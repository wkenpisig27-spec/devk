...
class dstring {
public:
	dstring() : m_buf(0), m_overwrite(true){}; // ȱʡ����
	dstring(dstring& ds) : m_buf(0), m_overwrite(true) {
		SetSize(ds.length() + 1);
		if (m_buf)
			strcpy(m_buf->getbuf(), ds);
	}
	dstring(cChar* str) : m_buf(0), m_overwrite(true) // ������
	{
		if (!str)
			return;
	...
	dstring(cwChar* str) : m_buf(0), m_overwrite(true) // ��������
	{
		if (!str)
			return;
	...
	explicit dstring(int i) : m_buf(0), m_overwrite(true) // ��������
	{
		char l_str[21];
		itoa(i, l_str, 10);
...
inline dstring& operator+=(dstring& str1, const LLong i) {
	char l_str[42];
	_i64toa(i, l_str, 10);
...
inline dstring& operator<<(dstring& str1, const LLong& i) { return str1 += i; }
inline dstring& operator<<(dstring&& str1, const LLong& i) { return str1 += i; }
