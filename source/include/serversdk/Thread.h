...
class TaskQue {
	...
	TaskQue(int max) : m_max(max), m_taskcount(0), m_head(0), m_tail(0) { // 池任务队列初始化
	...
	int GetTaskCount() { return m_taskcount; }

	Mutex m_mtxQue;
	int const m_max;
	int volatile m_taskcount;
...
public:
	TaskWait(uLong size = 0) : PreAllocStru(size) {
		...
		if (!m_semWait.Create(0, 1, 0))
			throw excpSync("Ceeate sync mutex failed(task wait)!");
	};

private:
	Sema m_semWait;
	int volatile m_retval;
