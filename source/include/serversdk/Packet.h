...
public:
	WPacket(const WPacket& wpkt)
		: rptr<rbuf>(wpkt), __tca(wpkt.__tca), m_offset(wpkt.m_offset), m_head(wpkt.m_head), m_wpos(wpkt.m_wpos), m_cpos(wpkt.m_cpos) //,m_appendant(wpkt.m_appendant)
	{}
	...
	bool WriteLongLong(unsigned long long ll);
...
public:
	RPacket(const RPacket& rpk)
		: rptr<rbuf>(rpk), __tca(rpk.__tca), m_offset(rpk.m_offset), m_head(rpk.m_head), m_rpos(rpk.m_rpos), m_len(rpk.m_len), m_tickcount(rpk.m_tickcount), m_revrpos(0) {}
