...
template <class T> class RunBiDirectChain;
template <class T> class RunCtrlMgr;
template <class T> class RunChainGetArmor;

//============================================================================================
template <class T>
class RunBiDirectItem {
	friend class RunBiDirectChain<T>;
	template <class U> friend class RunCtrlMgr;
...
protected:
	...
	int _BeginRun(RunBiDirectChain<T>* chain) {
...
class RunBiDirectChain {
	friend class RunBiDirectItem<T>;
	friend class RunChainGetArmor<T>;
...
private:
	int AddItem(T* item) {
		...
		if (item && !static_cast<RunBiDirectItem<T>*>(item)->m_chain) {
			...
			if (!m_head) {
				m_head = m_tail = item;
				static_cast<RunBiDirectItem<T>*>(m_head)->m_last = static_cast<RunBiDirectItem<T>*>(m_head)->m_next = 0;
			} else {
	...
	int DelItem(T* item) {
		...
		if (item && (static_cast<RunBiDirectItem<T>*>(item)->m_chain == this)) {
			...
			if (static_cast<RunBiDirectItem<T>*>(item)->m_last) {
				static_cast<RunBiDirectItem<T>*>(static_cast<RunBiDirectItem<T>*>(item)->m_last)->m_next = static_cast<RunBiDirectItem<T>*>(item)->m_next;
			}
			if (static_cast<RunBiDirectItem<T>*>(item)->m_next) {
				static_cast<RunBiDirectItem<T>*>(static_cast<RunBiDirectItem<T>*>(item)->m_next)->m_last = static_cast<RunBiDirectItem<T>*>(item)->m_last;
			}
			if (m_head == item)
				m_head = static_cast<RunBiDirectItem<T>*>(item)->m_next;
			if (m_tail == item)
				m_tail = static_cast<RunBiDirectItem<T>*>(item)->m_last;
