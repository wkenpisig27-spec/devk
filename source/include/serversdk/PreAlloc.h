...
public:
	PreAllocHeap(uLong unitsize)
		: m_unitsize(unitsize ? unitsize : 1), m_initnum(0), m_unitflag(unitsize ? unitsize : 1), m_numptr(0), m_freestru(0), m_usednum(0) {}
	PreAllocHeap(uLong unitsize, uLong initnum)
		: m_unitsize(unitsize ? unitsize : 1), m_initnum(initnum ? initnum : 1), m_unitflag(unitsize ? unitsize : 1), m_numptr(0), m_freestru(0), m_usednum(0) {}
	virtual ~PreAllocHeap() {
		Finally();
		*const_cast<uLong*>(&m_unitflag) = 0;
	};
...
private:
	...
	// Flattened from anonymous struct (GCC doesn't allow non-trivial members in anonymous aggregates)
	Mutex m_mtxfreeacc;
