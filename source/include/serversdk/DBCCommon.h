...
#define nil 0
// typedef type define
// CRITICAL: Long/uLong MUST be exactly 4 bytes on all platforms.
// On Linux x64 (LP64), 'unsigned int' is 8 bytes, breaking network packets.
// Use fixed-width types to ensure cross-platform binary compatibility.
typedef char Char;
typedef wchar_t wChar;
typedef short Short;       // signed 2-byte integer
typedef int32_t Long;      // signed 4-byte integer (was 'int' - 8 bytes on Linux x64!)
typedef int Int;           // system-native integer
...
typedef uint32_t uLong;          // unsigned 4-byte integer (was 'unsigned int' - 8 bytes on Linux x64!)
typedef unsigned int uInt;       // system-native unsigned integer
typedef const unsigned char cuChar;
typedef const unsigned short cuShort;
typedef const uint32_t cuLong;
typedef const unsigned int cuInt;
typedef wchar_t WChar;     // 2-byte character type
...
public:
	...
	LONG operator=(InterLockedLong& val) {
		Assign(val);
		return val;
	}
	...
	LONG SetZero() { return Assign(0); }			 // ����ԭʼֵ
	...
	LONG CompareAssign(LONG Comperand, LONG newval); // ���ʱ��Ÿ�ֵ,The return value is the initial value
private:
...
public:
	...
	LLong SetZero() { return Assign(0); }				// ����ԭʼֵ
	...
	LLong CompareAssign(LLong Comperand, LLong newval); // ���ʱ��Ÿ�ֵ,The return value is the initial value
private:
...
private:
	...
	LONG m_lMaximumCount; // ֻ��һ����־��û�п����������Ĺ���
};
