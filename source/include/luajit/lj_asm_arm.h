...
static uint32_t asm_fuseopm(ASMState* as, ARMIns ai, IRRef ref, RegSet allow) {
	...
	if (ra_hasreg(ir->r)) {
		...
		if (ir->o >= IR_BSHL && ir->o <= IR_BROR) {
			...
			ARMShift sh = ir->o == IR_BSHL ? ARMSH_LSL : ir->o == IR_BSHR ? ARMSH_LSR
													 : ir->o == IR_BSAR	  ? ARMSH_ASR
																		  : ARMSH_ROR;
			if (irref_isk(ir->op2)) {
...
static void asm_fusexref(ASMState* as, ARMIns ai, Reg rd, IRRef ref,
	...
	if (ra_noreg(ir->r) && canfuse(as, ir)) {
		int32_t lim = (!LJ_SOFTFP && (ai & 0x08000000)) ? 1024 : (ai & 0x04000000) ? 4096
																				   : 256;
		if (ir->o == IR_ADD) {
			int32_t ofs2;
			if (irref_isk(ir->op2) &&
...
#endif
	{
		...
		if (st >= IRT_I8 && st <= IRT_U16) { /* Extend to 32 bit integer. */
			...
			if ((as->flags & JIT_F_ARMV6)) {
				ARMIns ai = st == IRT_I8 ? ARMI_SXTB : st == IRT_U8 ? ARMI_UXTB
												   : st == IRT_I16	? ARMI_SXTH
																	: ARMI_UXTH;
...
static void asm_callround(ASMState* as, IRIns* ir, int id) {
	...
	emit_call(as, id == IRFPM_FLOOR ? (void*)lj_vm_floor_sf : id == IRFPM_CEIL ? (void*)lj_vm_ceil_sf
																			   : (void*)lj_vm_trunc_sf);
	/* Workaround to protect argument GPRs from being used for remat. */
	of = as->freeset;
...
static void asm_sfpmin_max(ASMState* as, IRIns* ir, int cc) {
	...
	if (!rset_test(as->freeset, RID_R2) &&
		regcost_ref(as->cost[RID_R2]) == args[2])
		rset_clear(drop, RID_R2);
	if (!rset_test(as->freeset, RID_R3) &&
		regcost_ref(as->cost[RID_R3]) == args[3])
...
static void asm_sfpcomp(ASMState* as, IRIns* ir) {
	...
	for (r = RID_R0; r <= RID_R3; r++)
		if (!rset_test(as->freeset, r) &&
			regcost_ref(as->cost[r]) == args[r - RID_R0])
			rset_clear(drop, r);
	ra_evictset(as, drop);
...
static void asm_ir(ASMState* as, IRIns* ir) {
	...
	case IR_BAND:
		asm_bitop(as, ir, ARMI_AND);
		break;
	case IR_BOR:
		asm_bitop(as, ir, ARMI_ORR);
		break;
	case IR_BXOR:
		asm_bitop(as, ir, ARMI_EOR);
		break;

	case IR_BSHL:
		asm_bitshift(as, ir, ARMSH_LSL);
		break;
	case IR_BSHR:
		asm_bitshift(as, ir, ARMSH_LSR);
		break;
	case IR_BSAR:
		asm_bitshift(as, ir, ARMSH_ASR);
		break;
	case IR_BROR:
		asm_bitshift(as, ir, ARMSH_ROR);
		break;
	case IR_BROL:
		lua_assert(0);
		break;
	...
	case IR_POW:
		...
		break;
	case IR_ABS:
		asm_fpunary(as, ir, ARMI_VABS_D);
		break;
	case IR_ATAN2:
		asm_callid(as, ir, IRCALL_atan2);
		break;
	case IR_LDEXP:
		asm_callid(as, ir, IRCALL_ldexp);
		break;
	case IR_FPMATH:
	...
	case IR_MIN:
		asm_min_max(as, ir, CC_GT, CC_HI);
		break;
	case IR_MAX:
		asm_min_max(as, ir, CC_LT, CC_LO);
		break;
