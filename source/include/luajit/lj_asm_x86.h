...
static void asm_conv(ASMState* as, IRIns* ir) {
	...
	if (irt_isfp(ir->t)) {
		...
		if (irt_isguard(ir->t)) {
			...
			Reg dest = ra_dest(as, ir, RSET_GPR);
			x86Op op = st == IRT_NUM ? ((ir->op2 & IRCONV_TRUNC) ? XO_CVTTSD2SI : XO_CVTSD2SI) : ((ir->op2 & IRCONV_TRUNC) ? XO_CVTTSS2SI : XO_CVTSS2SI);
			if (LJ_64 ? irt_isu64(ir->t) : irt_isu32(ir->t)) {
				...
				if (st == IRT_NUM)
					emit_rma(as, XO_ADDSD, tmp, lj_ir_k64_find(as->J, LJ_64 ? U64x(c3f00000, 00000000) : U64x(c1e00000, 00000000)));
				else
					emit_rma(as, XO_ADDSS, tmp, lj_ir_k64_find(as->J, LJ_64 ? U64x(00000000, df800000) : U64x(00000000, cf000000)));
				emit_sjcc(as, CC_NS, l_end);
...
static void asm_hrefk(ASMState* as, IRIns* ir) {
	...
	if (!irt_ispri(irkey->t)) {
		...
		/* Assumes -0.0 is already canonicalized to +0.0. */
		emit_loadu64(as, key, irt_isnum(irkey->t) ? ir_knum(irkey)->u64 : ((uint64_t)irt_toitype(irkey->t) << 32) | (uint64_t)(uint32_t)ptr2addr(ir_kgc(irkey)));
	} else {
...
static void asm_fxstore(ASMState* as, IRIns* ir) {
	...
	*/
	if (irt_isi16(ir->t) || irt_isu16(ir->t) || irt_isfp(ir->t) ||
		...
		RegSet allow8 = irt_isfp(ir->t) ? RSET_FPR : (irt_isi8(ir->t) || irt_isu8(ir->t)) ? RSET_GPR8
																						  : RSET_GPR;
...
static void asm_cnew(ASMState* as, IRIns* ir) {
	...
	CTSize sz = (ir->o == IR_CNEWI || ir->op2 == REF_NIL) ? lj_ctype_size(cts, ctypeid) : (CTSize)IR(ir->op2)->i;
	...
	if (ir->o == IR_CNEWI) {
		...
		if (irref_isk(ir->op2)) {
			IRIns* irk = IR(ir->op2);
			uint64_t k = irk->o == IR_KINT64 ? ir_k64(irk)->u64 : (uint64_t)(uint32_t)irk->i;
	...
	if (fpm == IRFPM_SQRT) {
		...
		if (as->flags & JIT_F_SSE4_1) { /* SSE4.1 has a rounding instruction. */
			...
			*--as->mcp = 0x66; /* 1st byte of ROUNDSD opcode. */
			...
			ra_evictset(as, drop);
			ra_destreg(as, ir, RID_XMM0);
			emit_call(as, fpm == IRFPM_FLOOR ? lj_vm_floor_sse : fpm == IRFPM_CEIL ? lj_vm_ceil_sse
																				   : lj_vm_trunc_sse);
		...
		case IRFPM_EXP:
			emit_call(as, lj_vm_exp_x87);
			break;
		case IRFPM_EXP2:
			emit_call(as, lj_vm_exp2_x87);
			break;
		case IRFPM_SIN:
			emit_x87op(as, XI_FSIN);
			break;
		case IRFPM_COS:
			emit_x87op(as, XI_FCOS);
			break;
		case IRFPM_TAN:
			emit_x87op(as, XI_FPOP);
			emit_x87op(as, XI_FPTAN);
			break;
		case IRFPM_LOG:
		case IRFPM_LOG2:
		case IRFPM_LOG10:
		...
		switch (fpm) {
		case IRFPM_LOG:
			emit_x87op(as, XI_FLDLN2);
			break;
		case IRFPM_LOG2:
			emit_x87op(as, XI_FLD1);
			break;
		case IRFPM_LOG10:
			emit_x87op(as, XI_FLDLG2);
			break;
		case IRFPM_OTHER:
			if (ir->o == IR_LDEXP)
				asm_x87load(as, ir->op2);
...
static void asm_ir(ASMState* as, IRIns* ir) {
	...
	case IR_BAND:
		asm_intarith(as, ir, XOg_AND);
		break;
	case IR_BOR:
		asm_intarith(as, ir, XOg_OR);
		break;
	case IR_BXOR:
		asm_intarith(as, ir, XOg_XOR);
		break;

	case IR_BSHL:
		asm_bitshift(as, ir, XOg_SHL);
		break;
	case IR_BSHR:
		asm_bitshift(as, ir, XOg_SHR);
		break;
	case IR_BSAR:
		asm_bitshift(as, ir, XOg_SAR);
		break;
	case IR_BROL:
		asm_bitshift(as, ir, XOg_ROL);
		break;
	case IR_BROR:
		asm_bitshift(as, ir, XOg_ROR);
		break;
...
#if LJ_64 && LJ_HASFFI
		if (!irt_isnum(ir->t))
			asm_arith64(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_divi64 : IRCALL_lj_carith_divu64);
		else
#endif
...
#if LJ_64 && LJ_HASFFI
		if (!irt_isint(ir->t))
			asm_arith64(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_modi64 : IRCALL_lj_carith_modu64);
		else
#endif
			asm_intmod(as, ir);
		break;
...
#if LJ_64 && LJ_HASFFI
		if (!irt_isnum(ir->t))
			asm_arith64(as, ir, irt_isi64(ir->t) ? IRCALL_lj_carith_powi64 : IRCALL_lj_carith_powu64);
		else
#endif
			asm_fppowi(as, ir);
		break;
