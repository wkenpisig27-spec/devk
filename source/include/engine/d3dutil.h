...
class CD3DArcBall
{
	...
	XMQUATERNION m_qDown;               // Quaternion before button down
	XMQUATERNION m_qNow;                // Composite quaternion for current drag
	XMMATRIX     m_matRotation;         // Matrix for arcball's orientation
	XMMATRIX     m_matRotationDelta;    // Matrix for arcball's orientation
	XMMATRIX     m_matTranslation;      // Matrix for arcball's position
	XMMATRIX     m_matTranslationDelta; // Matrix for arcball's position
	BOOL           m_bDrag;               // Whether user is dragging arcball

	XMVECTOR3 ScreenToVector( int sx, int sy );

public:
	LRESULT     HandleMouseMessages( HWND, UINT, WPARAM, LPARAM );

	XMMATRIX* GetRotationMatrix()         { return &m_matRotation; }
	XMMATRIX* GetRotationDeltaMatrix()    { return &m_matRotationDelta; }
	XMMATRIX* GetTranslationMatrix()      { return &m_matTranslation; }
	XMMATRIX* GetTranslationDeltaMatrix() { return &m_matTranslationDelta; }
...
class CD3DCamera
{
	XMVECTOR3 m_vEyePt;       // Attributes for view matrix
	XMVECTOR3 m_vLookatPt;
	XMVECTOR3 m_vUpVec;
	...
	XMMATRIX  m_matView;
	XMMATRIX  m_matBillboard; // Special matrix for billboarding effects
...
public:
	// Access functions
	XMVECTOR3 GetEyePt()           { return m_vEyePt; }
	XMVECTOR3 GetLookatPt()        { return m_vLookatPt; }
	XMVECTOR3 GetUpVec()           { return m_vUpVec; }
	XMVECTOR3 GetViewDir()         { return m_vView; }
	XMVECTOR3 GetCross()           { return m_vCross; }

	XMMATRIX  GetViewMatrix()      { return m_matView; }
	XMMATRIX  GetBillboardMatrix() { return m_matBillboard; }
	XMMATRIX  GetProjMatrix()      { return m_matProj; }

	VOID SetViewParams( XMVECTOR3 &vEyePt, XMVECTOR3& vLookatPt,
						XMVECTOR3& vUpVec );
