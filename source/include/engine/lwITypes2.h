...
// Pack lwTexInfo to 4-byte alignment for binary file compatibility with x86 format
// (void* data field is 4 bytes in x86 binary files, but 8 bytes on x64)
#pragma pack(push, 4)
...
struct lwTexInfo
{
    ...
    DWORD width; // ��ʹ��TEX_TYPE_DATA��ʱ��width��ʾdata size
    DWORD height;
    // file texture
    DWORD colorkey_type;
    lwColorValue4b colorkey;
    char file_name[LW_MAX_NAME];
    void* data;  // Runtime pointer - NOT serialized directly (file uses 4-byte placeholder)

    lwRenderStateAtom tss_set[LW_TEX_TSS_NUM];
};

#pragma pack(pop) // End 4-byte packing for lwTexInfo
