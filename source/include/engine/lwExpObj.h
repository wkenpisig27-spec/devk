...
// Pack structs to 4-byte alignment for binary file compatibility with x86 format
#pragma pack(push, 4)
...
struct lwTexInfo_0001 {
	...
	lwColorValue4b colorkey;
	char file_name[LW_MAX_NAME];
	DWORD data;  // Changed from void* to DWORD for x64 binary compatibility
...
#pragma pack(pop) // End 4-byte packing for binary file compatibility
