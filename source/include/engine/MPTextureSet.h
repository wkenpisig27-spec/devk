...
#define USE_MANAGED_TEXTURE
		...
		if (tex_info.file_name[l - 1] == 'a' && tex_info.file_name[l - 2] == 'g' && tex_info.file_name[l - 3] == 't') {
...
inline IDirect3DTextureX* GetTextureByID(int nID, BOOL bRequest = FALSE) // ??ID??Texture??
{
#ifdef USE_MANAGED_TEXTURE
	lwITex* tex = (lwITex*)MPTexSet::I()->GetRawData(nID, bRequest);
...
inline int GetTextureID(const char* pszDataName) // ??????Texture ID, ?????ID????????ID, ????????, ??-1????
{
	if (MPTexSet::I() != nullptr) {
...
inline MPTexInfo* GetTextureInfo(int nID) // ??ID??Texture????
{
	if (MPTexSet::I() != nullptr) {
