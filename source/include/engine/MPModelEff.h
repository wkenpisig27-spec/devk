...
public:
	...
	void		RotatingYaw(float fDeg)//��ˮƽ������ת
	{
		if(m_bRotating)
			return;
		m_SVerRota.z = fDeg;
	...
	void		RotatingPitch(float fDeg)//!��������ת
	{
		if(m_bRotating)
			return;
		m_SVerRota.x = fDeg;
	...
	void		RotatingRoll(float fDeg)//!���������ת
	{
		if(m_bRotating)
			return;
		m_SVerRota.y = fDeg;
...
#else
	void	CreateStrip(IDirect3DDeviceX*	pDev,const s_string& strTexName,CMPResManger* pResMagr)
#endif
	{
		m_pDev = pDev;
