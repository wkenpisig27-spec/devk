...
public:
	...
	// access functions to the pblock
	void SetRadius(TimeValue t, float r) {
		if (!pblock->GetController(collisionsphere_radius))
			pblock->SetValue(collisionsphere_radius, t, r);
	}
	void SetNode(TimeValue t, INode* n) {
		theHold.Suspend();
