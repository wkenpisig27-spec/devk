...
public:
	...
	MapDelta& operator=(MapDelta& from) {
		CopyMDChannels(from, MDELTA_ALL);
		return *this;
	}
...
public:
	...
	void SetVNum(int nv, BOOL keep = FALSE) {
		if (out)
			out->SetCount(nv, keep);
	}
	void Activate(int vnum, int vdID) {
		if (out == NULL)
			out = new PerData(vnum, VertexDataType(vdID));
	}
...
public:
	...
	void setMapSupport(int mp, bool val = true) {
		if (mp < 0)
			hmapSupport.Set(-1 - mp, val);
		else
			mapSupport.Set(mp, val);
	}
...
enum meshCommandMode { McmCreate,
					   McmAttach,
					   McmExtrude,
					   McmBevel,
					   McmChamfer,
...
enum meshUIParam { MuiSelByVert,
				   ...
				   MuiShowFNormals,
				   MuiSliceSplit,
				   MuiCutRefine, // end of integer values
				   MuiPolyThresh,
				   MuiFalloff,
				   MuiPinch,
				   MuiBubble,
