...
#define MakeFakeHandleClass(className, handleClassName) \
	class className;                                    \
	typedef className* handleClassName
...
public:
	...
	// mechanism, be sure that your clients are aware that they
	// must run your plugin with 3ds max version 4.2 or higher.
	virtual void* GetInterface(ULONG id) { return id == REFERENCE_MAKER_INTERFACE ? this : Animatable::GetInterface(id); }
...
public:
	...
	virtual void NotifyForeground(TimeValue t) { NotifyDependents(Interval(t, t), PART_PUT_IN_FG, REFMSG_FLAGDEPENDENTS); }
...
public:
	DependentIterator(ReferenceTarget* rtarg) {
		rt = rtarg;
		next = rt->refs.first;
	}
