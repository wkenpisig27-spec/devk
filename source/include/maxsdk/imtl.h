...
#define PSDIM(isz) ((isz == 0) ? PS_SMALL_SIZE : (isz == 1) ? PS_LARGE_SIZE \
															: PS_TINY_SIZE)
...
public:
	...
	DWORD extra;
	LightDesc() {
		affectDiffuse = affectSpecular = TRUE;
		ambientOnly = FALSE;
	}
...
#define SHADECONTEXT_SET_THREAD(sc, thread) \
...
#define SHADECONTEXT_SET_WORKER(sc, worker) \
...
#define SHADECONTEXT_SET_BOUNCE(sc, bounce) \
...
public:
	...
	void SetMeditObjType(int t) {
		mtlFlags &= ~MTL_MEDIT_OBJTYPE_MASK;
		mtlFlags |= t << MTL_OBJTYPE_SHIFT;
	}
	...
	void SetMeditTiling(int t) {
		mtlFlags &= ~MTL_MEDIT_TILING_MASK;
		mtlFlags |= t << MTL_TILING_SHIFT;
	}
