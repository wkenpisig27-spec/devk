...
public:
	...
	void ResetData() {
		vert = PATCH_UNDEFINED;
		patches.Delete(0, patches.Count());
	}
...
public:
	...
	void setMatID(MtlID id) {
		flags &= 0xFFFF;
		flags |= (DWORD)(id << FACE_MATID_SHIFT);
	}
...
public:
	...
	void setTVertChannel(int channel, int i, float x, float y, float z) {
		tVerts[channel ? 0 : 1][i].p.x = x;
		tVerts[channel ? 0 : 1][i].p.y = y;
		tVerts[channel ? 0 : 1][i].p.z = z;
	}
...
public:
	...
	CoreExport void SetVertexWeight(int i, float w) {
		assert(mpVertexWeights);
		if (i >= numVertexWeights)
			return;
		mpVertexWeights[i] = w;
	}
