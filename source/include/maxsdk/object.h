...
MakeTab(TextMapIndex) typedef TextMapIndexTab TextTab;
...
public:
	...
	View() {
		projType = -1;
		flags = RENDER_MESH_DISPLACEMENT_MAP;
	} // projType not set, this is to deal with older renderers.
};
//-------------------------------------------------------------
...
public:
	...
	virtual void ForceNotify(Interval& i) { NotifyDependents(i, PART_ALL, REFMSG_CHANGE); }
...
public:
	...
	void Reset() {
		NL = geometricAtten = shapeAtten = distanceAtten = shadowAtten = ambientAtten = diffuseAtten = specularAtten = 0.0f;
		rawColor.Black();
		filteredColor.Black();
...
public:
	...
	virtual void ForceNotify(Interval& i) { NotifyDependents(i, ChannelsChanged(), REFMSG_CHANGE); }
...
public:
	...
	int proc(Point3* p, int n) {
		gw->polyline(n, p, NULL, NULL, 0, NULL);
		return 0;
	}
