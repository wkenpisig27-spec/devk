...
public:
	...
	virtual BaseInterface* GetInterface(Interface_ID id) {
		if (id == FPINTERFACE_ID)
			return this;
		else if (id == FPINTERFACEDESC_ID)
...
public:
	...
	FPPropDef() {
		cbStruct = sizeof(FPPropDef);
		flags = 0;
		description = 0;
		getter_ID = setter_ID = FPS_NO_SUCH_FUNCTION;
		enumID = FP_NO_ENUM;
		setter_param = NULL;
	}
...
public:                                                            \
	FPStatus _dispatch_fn(FunctionID fid, TimeValue t,             \
						  FPValue& result, FPParams* p) {          \
		FPStatus status = Parent::_dispatch_fn(fid, t, result, p); \
		if (status == FPS_OK)                                      \
			return status;                                         \
		status = FPS_OK;                                           \
		switch (fid) {
