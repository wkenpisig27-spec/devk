...
public:
	...
	bool ZeroLength() const {
		return NullLink() ? true : (rigidExtend->GetIdentFlags() & POS_IDENT) ? true
																			  : false;
	}
...
public:
	...
	BaseInterface* GetInterface(Interface_ID id) {
		if (id == kHIIKGoalID)
			...
			return IIKGoal::GetInterface(id);
	}
...
inline Matrix3& RootLink::RotateByAxis(Matrix3& mat, unsigned i) const {
	switch (i) {
	case 0:
		mat.PreRotateX(rotXYZ[0]);
		return mat;
	case 1:
		mat.PreRotateY(rotXYZ[1]);
		return mat;
	case 2:
		mat.PreRotateZ(rotXYZ[2]);
		return mat;
	default:
		return mat;
	}
...
inline Matrix3& Link::DofMatrix(Matrix3& mat) const {
	...
	case TransZ: {
		...
		return mat;
	case RotX:
		mat.PreRotateX(dofValue);
		return mat;
	case RotY:
		mat.PreRotateY(dofValue);
		return mat;
	case RotZ:
		mat.PreRotateZ(dofValue);
		return mat;
	default:
		return mat;
	}
