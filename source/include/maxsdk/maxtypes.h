...
typedef struct {
	...
	static WORD clipColor(float c) { return c <= 0.0f ? 0 : c >= 1.0f ? 65535
																	  : int(c * 65535.0); }
} BMM_Color_fl;
...
#define TimeToTicksSec(time, ticks, secs)    \
	{                                        \
...
typedef enum { IO_OK = 0,
			   IO_END = 1,
			   IO_ERROR = 2 } IOResult;
typedef enum { NEW_CHUNK = 0,
			   CONTAINER_CHUNK = 1,
			   DATA_CHUNK = 2 } ChunkType;
typedef enum { IOTYPE_MAX = 0,
			   IOTYPE_MATLIB = 1 } FileIOType;

// Lock / licensing types  000817  --prs.

enum ProductVersionType { PRODUCT_VERSION_DEVEL,
