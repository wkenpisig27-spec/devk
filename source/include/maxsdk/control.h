...
public:
	...
	DelayedMatrix3::DelayedMatrix3(const DelayedMatrix3& src)
		: Matrix3Indirect(src.mat), mMatInitialized(src.mMatInitialized) {
...
public:
	...
	void LogHit(INode* nr, DWORD dist, ulong info, DWORD infoExtra) { first = new CtrlHitRecord(first, nr, dist, info, infoExtra); }
};
...
enum GetSetMethod { CTRL_RELATIVE,
					CTRL_ABSOLUTE };
...
enum SetXFormCommand { XFORM_MOVE,
					   XFORM_ROTATE,
					   XFORM_SCALE,
					   XFORM_SET };
...
public:
	...
	// XFORM_SET
	SetXFormPacket(const Matrix3& mat, const Matrix3& par = Matrix3(1)) { command = XFORM_SET, tmParent = par, tmAxis = mat; }
...
public:
	...
	SetMorphTargetPacket(Object* o, TSTR n, Matrix3& m, BOOL fc = FALSE) {
		obj = o;
		name = n;
		tm = m;
		forceCreate = fc;
	}
	SetMorphTargetPacket(Object* o, TSTR n, BOOL fc = FALSE) {
		obj = o;
		name = n;
		tm = Matrix3(1);
		forceCreate = fc;
...
template <class T>
T RepeatExtrapolate(Interval range, TimeValue t,
