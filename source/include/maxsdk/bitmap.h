...
enum ProjectionType { ProjPerspective = 0,
					  ProjParallel = 1 };
...
public:
	...
	bVirtual BMMRES GetImageName(BitmapInfo* bi, TCHAR* filename) {
		filename[0] = 0;
...
public:
	...
	inline static void ClampColor(BMM_Color_64& out, const BMM_Color_fl& in) {
		out.r = in.clipColor(in.r);
		out.g = in.clipColor(in.g);
		out.b = in.clipColor(in.b);
	}
	inline static void ClampColorA(BMM_Color_64& out, const BMM_Color_fl& in) {
		ClampColor(out, in);
		out.a = in.clipColor(in.a);
	}
	BMMExport static void ScaleColor(BMM_Color_64& out, BMM_Color_fl in);
	inline static void ScaleColorA(BMM_Color_64& out, const BMM_Color_fl& in) {
		ScaleColor(out, in);
		out.a = in.clipColor(in.a);
	}
	...
	bVirtual void* GetStoragePtr(int* type) {
		*type = BMM_NO_TYPE;
		return (NULL);
	}
	bVirtual void* GetAlphaPtr(int* type) {
		*type = BMM_NO_TYPE;
		return (NULL);
	}
	...
	bVirtual void DeleteChannels(ULONG channelIDs) {
		if (gbuffer)
			gbuffer->DeleteChannels(channelIDs);
	}
...
public:
	...
	inline int Paletted() {
		if (storage)
			return storage->Paletted();
		return 0;
	};
	inline int IsDithered() {
		if (storage)
			return storage->IsDithered();
		return 0;
	};
	inline int PreMultipliedAlpha() {
		if (storage)
			return storage->PreMultipliedAlpha();
		return 0;
	};
	inline int HasAlpha() {
		if (storage)
			return storage->HasAlpha();
		return 0;
	};
	inline int IsHighDynamicRange() {
		if (storage)
			return storage->IsHighDynamicRange();
		return 0;
	};
	inline int MaxRGBLevel() {
		if (storage)
			return storage->MaxRGBLevel();
		return 0;
	};
	inline int MaxAlphaLevel() {
		if (storage)
			return storage->MaxAlphaLevel();
		return 0;
	};
	inline void UseScaleColors(int on) {
		if (storage)
			storage->UseScaleColors(on);
	};
	inline int ScaleColors() {
		if (storage)
			return storage->ScaleColors();
		return 0;
	};
	...
	inline void ClampScaleColor(BMM_Color_64& out, const BMM_Color_fl& in) {
		if (storage)
			storage->ClampScaleColor(out, in);
		else
			ClampColor(out, in);
	}
	inline void ClampScaleColorA(BMM_Color_64& out, const BMM_Color_fl& in) {
		if (storage)
			storage->ClampScaleColorA(out, in);
		else
			ClampColorA(out, in);
	}
	...
	int Put16Gray(int x, int y, int pixels, WORD* ptr) {
		if (storage)
			return storage->Put16Gray(x, y, pixels, ptr);
		return 0;
	};
	...
	inline void* GetStoragePtr(int* type) {
		if (storage)
			return storage->GetStoragePtr(type);
		return NULL;
	};
	inline void* GetAlphaPtr(int* type) {
		if (storage)
			return storage->GetAlphaPtr(type);
		return NULL;
	};
	inline int Get16Gray(int x, int y, int pixels, WORD* ptr) {
		if (storage)
			return storage->Get16Gray(x, y, pixels, ptr);
		return 0;
	};
	...
	inline int GetPixels(int x, int y, int pixels, BMM_Color_64* ptr) {
		if (storage)
			return storage->GetPixels(x, y, pixels, ptr);
		return 0;
	};
	...
	inline int GetLinearPixels(int x, int y, int pixels, BMM_Color_64* ptr) {
		if (storage)
			return storage->GetLinearPixels(x, y, pixels, ptr);
		return 0;
	};
	...
	inline int GetIndexPixels(int x, int y, int pixels, BYTE* ptr) {
		if (storage)
			return storage->GetIndexPixels(x, y, pixels, ptr);
		return 0;
	};
	inline int PutIndexPixels(int x, int y, int pixels, BYTE* ptr) {
		if (storage)
			return storage->PutIndexPixels(x, y, pixels, ptr);
		return 0;
	};
	inline int CropImage(int width, int height, BMM_Color_64 fillcolor) {
		if (storage)
			return storage->CropImage(width, height, fillcolor);
		return 0;
	};
	...
	inline int CropImage(int width, int height, int fillindex) {
		if (storage)
			return storage->CropImage(width, height, fillindex);
		return 0;
	};
	inline int ResizeImage(int width, int height, int newpalette) {
		if (storage)
			return storage->ResizeImage(width, height, newpalette);
		return 0;
	};
	inline int CopyImage(Bitmap* from, int operation, BMM_Color_64 fillcolor, BitmapInfo* bi = NULL) {
		if (storage)
			return storage->CopyImage(from, operation, fillcolor, bi);
		return 0;
	};
	...
	inline int CopyImage(Bitmap* from, int operation, int fillindex) {
		if (storage)
			return storage->CopyImage(from, operation, fillindex);
		return 0;
	};
	inline int GetPalette(int start, int count, BMM_Color_48* ptr) {
		if (storage)
			return storage->GetPalette(start, count, ptr);
		return 0;
	};
	inline int SetPalette(int start, int count, BMM_Color_48* ptr) {
		if (storage)
			return storage->SetPalette(start, count, ptr);
		return 0;
	};
	...
	inline void* GetChannel(ULONG channelID, ULONG& chanType) {
		if (storage)
			return storage->GetChannel(channelID, chanType);
		return NULL;
	}
	...
	inline ULONG CreateChannels(ULONG channelIDs) {
		if (storage)
			return storage->CreateChannels(channelIDs);
		return 0;
	}
	inline void DeleteChannels(ULONG channelIDs) {
		if (storage)
			storage->DeleteChannels(channelIDs);
	}
	inline ULONG ChannelsPresent() {
		if (storage)
			return storage->ChannelsPresent();
		return 0;
	}
	inline RenderInfo* GetRenderInfo() {
		if (storage)
			return storage->GetRenderInfo();
		return NULL;
	}

	inline RenderInfo* AllocRenderInfo() {
		if (storage)
			return storage->AllocRenderInfo();
		return NULL;
	}
...
public:
	inline PixelBuf(int width) {
		buf = (BMM_Color_64*)calloc(width, sizeof(BMM_Color_64));
		this->width = width;
	};
	inline ~PixelBuf() {
		if (buf)
