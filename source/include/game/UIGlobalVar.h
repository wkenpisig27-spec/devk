...
#define FORM_LOADING_CHECK(a, b, c)               \
	...
	if (!a) {                                     \
		::Error(RES_STRING(CL_LANGUAGE_MATCH_577), b, c); \
		return false;                             \
	}
#define FORM_CONTROL_LOADING_CHECK(a, b, c, d, e)               \
	a = dynamic_cast<c*>(b->Find(e));                           \
	if (!a) {                                                   \
		::Error(RES_STRING(CL_LANGUAGE_MATCH_578), d, b->GetName(), e); \
		return false;                                           \
	}

// 锟饺硷拷锟斤拷锟斤拷FLAG锟斤拷SetEnableHotKey锟斤拷锟斤拷
#define HOTKEY_STORE (1 << 0) // 锟教筹拷
#define HOTKEY_BANK (1 << 1)  // 锟斤拷锟斤拷
#define HOTKEY_BOOTH (1 << 2) // 锟斤拷摊
#define HOTKEY_TRADE (1 << 3) // 锟斤拷锟斤拷
...
class CUIInterface // 锟矫伙拷锟斤拷锟斤拷锟斤拷息
{
public:
	CUIInterface();
...
protected:
	...
	static void _evtDragToGoodsEvent(CGuiData* pSender, CCommandObj* pItem, int nGridID, bool& isAccept); // 锟较讹拷锟斤拷锟斤拷锟斤拷锟斤拷
...
extern CBossTimerMgr g_stUIBossTimer;
