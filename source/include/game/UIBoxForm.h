...
public:
	// 交易用的对话框
	static stTradeBox* ShowTradeBox(FormMouseEvent evtForm = nullptr, __int64 fUnitMoney = 0, int nTotalNum = -1, const char* szName = nullptr);

	// 选择确定或取消的对话框
	static stSelectBox* ShowSelectBox(FormMouseEvent evtForm = nullptr, const char* szTitle = nullptr, bool bModal = false);

	// 输入数量对话框
	static stNumBox* ShowNumberBox(FormMouseEvent evtForm = nullptr, long long nTotalNum = -1, const char* szTitle = nullptr, bool IsMax = true, bool bModal = true);
