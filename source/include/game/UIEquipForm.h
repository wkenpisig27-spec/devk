...
public:
	...
	void RequestChestPreview(int chestItemID);
	void OnChestPreviewData(int chestItemID, const std::vector<int>& itemIDs, const std::vector<int>& quantities, const std::vector<int>& weights, int totalWeight);

private:
	struct SChestPreviewData {
	...
	void ShowChestPreviewByID(int chestItemID);
	void HideChestPreview();
	bool IsChestPreviewSupported(int itemID) const;
	void ClearChestPreview();
	...
	static void _evtExpandBagClick();
	static void _evtExpandBagConfirm(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey);
...
private:
	...
	CForm* frmChestPreview{};
	CGoodsGrid* grdChestPreview{};
	CLabelEx* labChestPreviewTitle{};
	std::unordered_map<int, SChestPreviewData> chestPreviewCache;
