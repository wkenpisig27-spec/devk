...
private:               // ��Ч
	...
	// ��תʱ��ʱ����
	int _nHelixCenterX, _nHelixCenterY;
	int _nHelixAngle; // �����Ƕ�����
	int _nHelixRadii;
...
private:
	...
	CEffectObj* CLOAKGlow[enumEQUIP_NUM]{nullptr}; // cloak glowing from iteminfo
...
protected:
	...
	int mParentBoneID;
	int CloakprevLevel{0}; // cloak previous level
...
inline bool CCharacter::IsDefaultSkill() {
	return _pReadySkillInfo == nullptr || _pReadySkillInfo == _pDefaultSkillInfo;
}
...
inline void CCharacter::CheckIsFightArea() {
	...
	if (_pScene && _pScene->GetTerrain()) {
		_InFight =
		    !(_pScene->GetTerrain()->GetTile(GetCurX() / 100, GetCurY() / 100)->sRegion & enumAREA_TYPE_NOT_FIGHT);
	}
}
...
inline void CCharacter::RefreshShadow() {
	if (_IsShowShadow && (GetDefaultChaInfo()->chTerritory != defCHA_TERRITORY_SEA) &&
	    GetActor()->GetState() == enumNormal && !IsHide()) {
