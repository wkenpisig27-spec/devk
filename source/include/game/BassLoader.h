...
struct BASS_INSTANCE {
	...
	(int device, DWORD freq, DWORD flags, HWND win);
	BOOL(WINAPI* BASS_Init2)
	(int device, DWORD freq, DWORD flags, HWND win, GUID* clsid);
	...
	(HMUSIC handle, DWORD amp);

	DWORD(WINAPI* BASS_SetConfig)
	(DWORD option, DWORD value);
	...
	(DWORD handle, int freq, int volume, int pan);
	...
	(BOOL mem, void* file, DWORD offset, DWORD length, DWORD flags);
	HMUSIC(WINAPI* BASS_MusicLoad2)
	(BOOL mem, void* file, DWORD offset, DWORD length, DWORD flags, DWORD freq);
	BOOL(WINAPI* BASS_MusicPlay)
	(HMUSIC handle);
	BOOL(WINAPI* BASS_MusicPlayEx)
	(HMUSIC handle, DWORD pos, int flags, BOOL reset);
	...
	(HSTREAM handle, BOOL flush, DWORD flags);
};
