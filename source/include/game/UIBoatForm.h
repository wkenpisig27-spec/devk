...
public:
	...
	bool GetIsValid() { return _chtBoat != nullptr; }	   // 获得角色信息
	void UnLink() { _chtBoat = nullptr; }
...
protected:
	...
	virtual void SwitchMap() {
		// Don't clear immediately - let the server resync handle it.
		// Only clear the display state (hide forms), not the boat references.
		for (int i = 0; i < defMaxBoat; i++) {
			if (_cBoats[i].GetForm())
				_cBoats[i].GetForm()->SetIsShow(false);
		}
	}
