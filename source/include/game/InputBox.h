...
public:
	...
	void HandleWindowMsg(DWORD dwMsgType, WPARAM wParam, LPARAM lParam);
...
inline void CInputBox::HandleWindowMsg(DWORD dwMsgType, WPARAM wParam, LPARAM lParam) {
	...
	if (dwMsgType == WM_KEYDOWN || dwMsgType == WM_KEYUP) {
		if (wParam == VK_UP)
			return; // 屏蔽掉UP键, 避免触发Edit控件的对应操作
					// ::SendMessage(_hEdit, WM_KEYDOWN, wParam, lParam);
	...
	::SendMessage(_hEdit, dwMsgType, wParam, lParam);
}
