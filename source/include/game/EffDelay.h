...
class CHitRepresent {
public:
	CHitRepresent() : _pTarget(nullptr), _pSkill(nullptr), _nAttackX(0), _nAttackY(0), _pAttack(nullptr) {}
...
inline void CHitRepresent::ActionExec(CServerHarm* pHarm, int nActionKey) {
	if (!_pSkill->IsActKeyHarm())
		return;

	if (_pSkill->chTargetEffectTime != nActionKey)
		return;
