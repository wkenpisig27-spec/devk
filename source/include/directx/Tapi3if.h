...
typedef enum TAPI_TONEMODE { TTM_RINGBACK = 0x2,
							 TTM_BUSY = 0x4,
							 TTM_BEEP = 0x8,
							 TTM_BILLING = 0x10
} TAPI_TONEMODE;

typedef enum TAPI_GATHERTERM { TGT_BUFFERFULL = 0x1,
							   TGT_TERMDIGIT = 0x2,
...
typedef enum ADDRESS_EVENT { AE_STATE = 0,
...
typedef enum ADDRESS_STATE { AS_INSERVICE = 0,
							 AS_OUTOFSERVICE = AS_INSERVICE + 1
} ADDRESS_STATE;

typedef enum CALL_STATE { CS_IDLE = 0,
...
typedef enum CALL_STATE_EVENT_CAUSE { CEC_NONE = 0,
...
typedef enum CALL_MEDIA_EVENT { CME_NEW_STREAM = 0,
...
typedef enum CALL_MEDIA_EVENT_CAUSE { CMC_UNKNOWN = 0,
...
typedef enum DISCONNECT_CODE { DC_NORMAL = 0,
							   DC_NOANSWER = DC_NORMAL + 1,
							   DC_REJECTED = DC_NOANSWER + 1
} DISCONNECT_CODE;

typedef enum TERMINAL_STATE { TS_INUSE = 0,
							  TS_NOTINUSE = TS_INUSE + 1
} TERMINAL_STATE;

typedef enum TERMINAL_DIRECTION { TD_CAPTURE = 0,
...
typedef enum TERMINAL_TYPE { TT_STATIC = 0,
							 TT_DYNAMIC = TT_STATIC + 1
} TERMINAL_TYPE;

typedef enum CALL_PRIVILEGE { CP_OWNER = 0,
							  CP_MONITOR = CP_OWNER + 1
} CALL_PRIVILEGE;

typedef enum TAPI_EVENT { TE_TAPIOBJECT = 0x1,
						  TE_ADDRESS = 0x2,
...
typedef enum CALL_NOTIFICATION_EVENT { CNE_OWNER = 0,
									   CNE_MONITOR = CNE_OWNER + 1,
									   CNE_LASTITEM = CNE_MONITOR
} CALL_NOTIFICATION_EVENT;

typedef enum CALLHUB_EVENT { CHE_CALLJOIN = 0,
...
typedef enum CALLHUB_STATE { CHS_ACTIVE = 0,
							 CHS_IDLE = CHS_ACTIVE + 1
} CALLHUB_STATE;

typedef enum TAPIOBJECT_EVENT { TE_ADDRESSCREATE = 0,
...
typedef enum TAPI_OBJECT_TYPE { TOT_NONE = 0,
								TOT_TAPI = TOT_NONE + 1,
...
typedef enum QOS_SERVICE_LEVEL { QSL_NEEDED = 1,
								 QSL_IF_AVAILABLE = 2,
								 QSL_BEST_EFFORT = 3
} QOS_SERVICE_LEVEL;

typedef enum QOS_EVENT { QE_NOQOS = 1,
...
typedef enum CALLINFOCHANGE_CAUSE { CIC_OTHER = 0,
...
typedef enum CALLINFO_LONG { CIL_MEDIATYPESAVAILABLE = 0,
...
typedef enum CALLINFO_STRING { CIS_CALLERIDNAME = 0,
...
typedef enum CALLINFO_BUFFER { CIB_USERUSERINFO = 0,
...
typedef enum ADDRESS_CAPABILITY { AC_ADDRESSTYPES = 0,
...
typedef enum ADDRESS_CAPABILITY_STRING { ACS_PROTOCOL = 0,
...
typedef enum FULLDUPLEX_SUPPORT { FDS_SUPPORTED = 0,
								  FDS_NOTSUPPORTED = FDS_SUPPORTED + 1,
								  FDS_UNKNOWN = FDS_NOTSUPPORTED + 1
} FULLDUPLEX_SUPPORT;

typedef enum FINISH_MODE { FM_ASTRANSFER = 0,
						   FM_ASCONFERENCE = FM_ASTRANSFER + 1
} FINISH_MODE;

typedef enum PHONE_PRIVILEGE { PP_OWNER = 0,
							   PP_MONITOR = PP_OWNER + 1
} PHONE_PRIVILEGE;

typedef enum PHONE_HOOK_SWITCH_DEVICE { PHSD_HANDSET = 0x1,
										PHSD_SPEAKERPHONE = 0x2,
										PHSD_HEADSET = 0x4
} PHONE_HOOK_SWITCH_DEVICE;

typedef enum PHONE_HOOK_SWITCH_STATE { PHSS_ONHOOK = 0x1,
									   PHSS_OFFHOOK_MIC_ONLY = 0x2,
									   PHSS_OFFHOOK_SPEAKER_ONLY = 0x4,
									   PHSS_OFFHOOK = 0x8
} PHONE_HOOK_SWITCH_STATE;

typedef enum PHONE_LAMP_MODE { LM_DUMMY = 0x1,
							   LM_OFF = 0x2,
							   LM_STEADY = 0x4,
							   LM_WINK = 0x8,
							   LM_FLASH = 0x10,
...
typedef enum PHONECAPS_LONG { PCL_HOOKSWITCHES = 0,
...
typedef enum PHONECAPS_STRING { PCS_PHONENAME = 0,
								PCS_PHONEINFO = PCS_PHONENAME + 1,
								PCS_PROVIDERINFO = PCS_PHONEINFO + 1
} PHONECAPS_STRING;

typedef enum PHONECAPS_BUFFER { PCB_DEVSPECIFICBUFFER = 0
} PHONECAPS_BUFFER;

typedef enum PHONE_BUTTON_STATE { PBS_UP = 0x1,
								  PBS_DOWN = 0x2,
								  PBS_UNKNOWN = 0x4,
								  PBS_UNAVAIL = 0x8
} PHONE_BUTTON_STATE;

typedef enum PHONE_BUTTON_MODE { PBM_DUMMY = 0,
...
typedef enum PHONE_BUTTON_FUNCTION { PBF_UNKNOWN = 0,
...
typedef enum PHONE_TONE { PT_KEYPADZERO = 0,
...
typedef enum PHONE_EVENT { PE_DISPLAY = 0,
...
/* interface __MIDL_itf_tapi3if_0425 */
/* [local] */

typedef enum TERMINAL_MEDIA_STATE { TMS_IDLE = 0,
									TMS_ACTIVE = TMS_IDLE + 1,
									TMS_PAUSED = TMS_ACTIVE + 1,
									TMS_LASTITEM = TMS_PAUSED
} TERMINAL_MEDIA_STATE;

typedef enum FT_STATE_EVENT_CAUSE { FTEC_NORMAL = 0,
									FTEC_END_OF_FILE = FTEC_NORMAL + 1,
									FTEC_READ_ERROR = FTEC_END_OF_FILE + 1,
									FTEC_WRITE_ERROR = FTEC_READ_ERROR + 1
} FT_STATE_EVENT_CAUSE;

extern RPC_IF_HANDLE __MIDL_itf_tapi3if_0425_v0_0_c_ifspec;
extern RPC_IF_HANDLE __MIDL_itf_tapi3if_0425_v0_0_s_ifspec;
...
/* interface __MIDL_itf_tapi3if_0491 */
/* [local] */
...
extern RPC_IF_HANDLE __MIDL_itf_tapi3if_0491_v0_0_c_ifspec;
extern RPC_IF_HANDLE __MIDL_itf_tapi3if_0491_v0_0_s_ifspec;
