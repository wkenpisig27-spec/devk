...
WINOLEAPI_(BOOL)
CoIsHandlerConnected(IN LPUNKNOWN pUnk);
...
CoLoadLibrary(IN LPOLESTR lpszLibName, IN BOOL bAutoFree);
WINOLEAPI_(void)
CoFreeLibrary(IN HINSTANCE hInst);
WINOLEAPI_(void)
 ...
 */
WINOLEAPI_(void)
CoFreeUnusedLibrariesEx(IN DWORD dwUnloadDelay, IN DWORD dwReserved);
#endif
...
WINOLEAPI_(BOOL)
CoIsOle1Class(IN REFCLSID rclsid);
...
WINOLEAPI_(int)
StringFromGUID2(IN REFGUID rguid, OUT LPOLESTR lpsz, IN int cchMax);
...
WINOLEAPI_(LPVOID)
CoTaskMemAlloc(IN SIZE_T cb);
WINOLEAPI_(LPVOID)
CoTaskMemRealloc(IN LPVOID pv, IN SIZE_T cb);
WINOLEAPI_(void)
CoTaskMemFree(IN LPVOID pv);
...
WINOLEAPI MonikerRelativePathTo(IN LPMONIKER pmkSrc, IN LPMONIKER pmkDest, OUT LPMONIKER FAR* ppmkRelPath, IN BOOL dwReserved);
