...
DECLARE_INTERFACE_(IPrintDialogCallback, IUnknown) {
	...
	(THIS_ HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT * pResult) PURE;
};
...
DECLARE_INTERFACE_(IPrintDialogServices, IUnknown) {
	...
	STDMETHOD(GetCurrentDevMode)
	(THIS_ LPDEVMODE pDevMode, UINT * pcbSize) PURE;
	STDMETHOD(GetCurrentPrinterName)
	(THIS_ LPTSTR pPrinterName, UINT * pcchSize) PURE;
	STDMETHOD(GetCurrentPortName)
	(THIS_ LPTSTR pPortName, UINT * pcchSize) PURE;
};
...
ISOLATION_AWARE_INLINE HRESULT CommdlgIsolationAwarePrivateJIaDCGbueEfhLg(void) {
...
ISOLATION_AWARE_INLINE HRESULT WINAPI IsolationAwarePrintDlgExA(LPPRINTDLGEXA unnamed1) {
	...
	const BOOL fActivateActCtxSuccess = IsolationAwarePrivateG_FqbjaLEiEL || IsolationAwarePrivatenCgIiAgEzlnCgpgk(&ulpCookie);
	if (!fActivateActCtxSuccess)
		return CommdlgIsolationAwarePrivateJIaDCGbueEfhLg();
	__try {
		if (s_pfn == NULL) {
			s_pfn = (PFN)CommdlgIsolationAwarePrivatetEgCebCnDDeEff_pbMDLGDC_DLL("PrintDlgExA");
			if (s_pfn == NULL) {
				result = CommdlgIsolationAwarePrivateJIaDCGbueEfhLg();
				__leave;
			}
		}
...
/* This function is shared by the other stubs in this header. */
{
	...
	const static IsolationAwarePrivatepBAFGnAG_zBqHyr_vAsB
		c = {IsolationAwarePrivatezlybADyIBeAeln, IsolationAwarePrivatezlybADyIBeAelJ, "Comdlg32.dll", L"Comdlg32.dll"};
	...
	__try {
		...
		proc = IsolationAwarePrivatezltEgCebCnDDeEff(&c, &m, pszProcName);
	} __finally {
