...
DECLARE_INTERFACE_(IDsAdminNewObj, IUnknown) {
	...
	STDMETHOD(SetButtons)
	(THIS_ /*IN*/ ULONG nCurrIndex, /*IN*/ BOOL bValid) PURE;
	STDMETHOD(GetPageCounts)
	(THIS_ /*OUT*/ LONG * pnTotal,
...
DECLARE_INTERFACE_(IDsAdminNewObjExt, IUnknown) {
	...
	STDMETHOD(AddPages)
	(THIS_ /*IN*/ LPFNADDPROPSHEETPAGE lpfnAddPage,
	 /*IN*/ LPARAM lParam) PURE;

	STDMETHOD(SetObject)
	(THIS_ /*IN*/ IADs * pADsObj) PURE;
	...
	STDMETHOD(GetSummaryInfo)
	(THIS_ /*OUT*/ BSTR * pBstrText) PURE;
};
...
DECLARE_INTERFACE_(IDsAdminNotifyHandler, IUnknown) {
	...
	STDMETHOD(Initialize)
	(THIS_ /*IN*/ IDataObject * pExtraInfo,
	 /*OUT*/ ULONG * puEventFlags) PURE;
	STDMETHOD(Begin)
	(THIS_ /*IN*/ ULONG uEvent,
	...
	STDMETHOD(Notify)
	(THIS_ /*IN*/ ULONG nItem, /*IN*/ ULONG uFlags) PURE;
