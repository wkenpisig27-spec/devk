...
#define IMMPID_START_LIST(name, start, guid)                 \
	struct __declspec(uuid(guid)) tagIMMPID_##name##_STRUCT; \
	typedef enum tagIMMPID_##name##_ENUM {                   \
		IMMPID_##name##_BEFORE__ = (start)-1,
#define IMMPID_END_LIST(name) \
	IMMPID_##name##_AFTER__   \
	}                         \
	IMMPID_##name##_ENUM;
...
#define IMMPID_DECLARE_ENTRY(name)         \
	{ &_uuidof(tagIMMPID_##name##_STRUCT), \
