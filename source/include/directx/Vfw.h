...
typedef struct {
	...
	LONG(CALLBACK* GetData)
	(LPARAM lInput, LONG lFrame, LPVOID lpBits, LONG len);
	LONG(CALLBACK* PutData)
	(LPARAM lOutput, LONG lFrame, LPVOID lpBits, LONG len);
} ICCOMPRESSFRAMES;
...
#define ICSTATUS_YIELD 4
	...
	LONG(CALLBACK* Status)
	(LPARAM lParam, UINT message, LONG l);
} ICSETSTATUSPROC;
...
DECLARE_INTERFACE_(IAVIStream, IUnknown) {
	...
	// *** IAVIStream methods ***
	STDMETHOD(Create)
	(THIS_ LPARAM lParam1, LPARAM lParam2) PURE;
	STDMETHOD(Info)
	(THIS_ AVISTREAMINFOW FAR * psi, LONG lSize) PURE;
	STDMETHOD_(LONG, FindSample)
	(THIS_ LONG lPos, LONG lFlags) PURE;
	STDMETHOD(ReadFormat)
	(THIS_ LONG lPos,
	...
	STDMETHOD(Read)
	(THIS_ LONG lStart, LONG lSamples,
	...
	STDMETHOD(Delete)
	(THIS_ LONG lStart, LONG lSamples) PURE;
	STDMETHOD(ReadData)
	(THIS_ DWORD fcc, LPVOID lp, LONG FAR * lpcb) PURE;
	STDMETHOD(WriteData)
	(THIS_ DWORD fcc, LPVOID lp, LONG cb) PURE;
#ifdef _WIN32
	STDMETHOD(SetInfo)
	(THIS_ AVISTREAMINFOW FAR * lpInfo,
	 LONG cbInfo) PURE;
#else
	STDMETHOD(Reserved1)
	(THIS) PURE;
...
DECLARE_INTERFACE_(IAVIEditStream, IUnknown) {
	...
	(THIS_ LONG FAR * plStart,
	 ...
	 PAVISTREAM FAR * ppResult) PURE;
	STDMETHOD(Paste)
	(THIS_ LONG FAR * plPos,
	 LONG FAR * plLength,
	 PAVISTREAM pstream,
	 LONG lStart,
	 LONG lEnd) PURE;
	STDMETHOD(Clone)
	(THIS_ PAVISTREAM FAR * ppResult) PURE;
	STDMETHOD(SetInfo)
	(THIS_ AVISTREAMINFOW FAR * lpInfo,
	 LONG cbInfo) PURE;
};
...
DECLARE_INTERFACE_(IGetFrame, IUnknown) {
	...
	STDMETHOD_(LPVOID, GetFrame)
	(THIS_ LONG lPos) PURE;
	//  STDMETHOD_(LPVOID,GetFrameData) (THIS_ LONG lPos) PURE;

	STDMETHOD(Begin)
	(THIS_ LONG lStart, LONG lEnd, LONG lRate) PURE;
	STDMETHOD(End)
	(THIS) PURE;

	STDMETHOD(SetFormat)
	(THIS_ LPBITMAPINFOHEADER lpbi, LPVOID lpBits, int x, int y, int dx, int dy) PURE;
...
STDAPI_(void)
AVIFileInit(void); // Call this first!
STDAPI_(void)
AVIFileExit(void);
...
STDAPI_(ULONG)
AVIStreamAddRef(PAVISTREAM pavi);
STDAPI_(ULONG)
...
STDAPI_(LONG)
AVIStreamStart(PAVISTREAM pavi);
STDAPI_(LONG)
AVIStreamLength(PAVISTREAM pavi);
STDAPI_(LONG)
AVIStreamTimeToSample(PAVISTREAM pavi, LONG lTime);
STDAPI_(LONG)
...
STDAPI_(LPVOID)
AVIStreamGetFrame(PGETFRAME pg, LONG lPos);
