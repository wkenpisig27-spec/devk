...
DECLARE_INTERFACE_(IKsAllocator, IUnknown) {
	...
	(
		THIS_
			PKSSTREAMALLOCATOR_STATUS AllocatorStatus) PURE;
	STDMETHOD_(VOID, KsSetAllocatorMode)
	(
		THIS_
			KSALLOCATORMODE Mode) PURE;
};
...
DECLARE_INTERFACE_(IKsAllocatorEx, IKsAllocator) {
	...
	(
		THIS_
			PALLOCATOR_PROPERTIES_EX) PURE;
	STDMETHOD_(VOID, KsSetAllocatorHandle)
	(
		THIS_
			HANDLE AllocatorHandle) PURE;
...
DECLARE_INTERFACE_(IKsPin, IUnknown) {
	...
	(
		THIS_
			PKSSTREAM_SEGMENT StreamSegment) PURE;
	STDMETHOD_(IMemAllocator*, KsPeekAllocator)
	(
		THIS_
			KSPEEKOPERATION Operation) PURE;
...
DECLARE_INTERFACE_(IKsDataTypeHandler, IUnknown) {
	...
	(
		THIS_
			...
			Sample,
		PVOID StreamHeader,
		KSIOOPERATION IoOperation) PURE;
	...
	(
		THIS_ const AM_MEDIA_TYPE* AmMediaType) PURE;
};
...
DECLARE_INTERFACE_(IKsDataTypeCompletion, IUnknown) {
	...
	(
		THIS_
			HANDLE FilterHandle,
		ULONG PinFactoryId,
		AM_MEDIA_TYPE * AmMediaType) PURE;
};
...
DECLSPEC_UUID("17CCA71B-ECD7-11D0-B908-00A0C9223196")
CLSID_Proxy;
#endif // defined(__cplusplus) && _MSC_VER >= 1100
...
DECLARE_INTERFACE_(IKsAggregateControl, IUnknown) {
	...
	(
		THIS_
			IN REFGUID AggregateClass) PURE;
