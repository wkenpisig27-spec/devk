...
// 操作背景音乐
inline void SetBkMusicNotify(HWND hWnd, UINT uMsgId) { bkg_snd_set_notify(hWnd, uMsgId); }
inline int PlayBkMusic(char const* filename, bool loop = false) { return int(bkg_snd_play(filename, loop)); }
...
inline int PlayMusic(char const* filename, int reserved = 0) { return int(cmn_snd_play(filename)); }
