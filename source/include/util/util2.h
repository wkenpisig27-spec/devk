...
// Use platform_compat.h for all Windows type definitions and POSIX equivalents
...
#ifdef WIN32
#define MSGSTR(str) MessageBox(nullptr, str, "msg", 0)
#define MSGNUM(num)           \
	{                         \
		char szNum[6];        \
		itoa(num, szNum, 10); \
...
inline int END_TICK(bool bLog = true, const char* pszTag = "") {
...
inline DWORD RGB24TORGB15(DWORD dwRGB) { return ((dwRGB & 0xf80000) >> 9) | ((dwRGB & 0xf800) >> 6) | ((dwRGB & 0xff) >> 3); }

inline DWORD RGB24TORGB12(DWORD dwRGB) { return ((dwRGB & 0xf00000) >> 12) | ((dwRGB & 0xf000) >> 8) | ((dwRGB & 0xf0) >> 4); }

inline bool OutRect(const RECT& Rect, const int nX, const int nY) { return nX < Rect.left || nX >= Rect.right || nY < Rect.top || nY >= Rect.bottom; }
...
public:
	...
	MPTimer() {
		if (dwFrequency == 1) {
			LARGE_INTEGER l;
			QueryPerformanceFrequency(&l); // current frequency
