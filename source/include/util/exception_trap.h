...
struct unhandled_policy {
	int operator()(EXCEPTION_POINTERS*) { return EXCEPTION_CONTINUE_SEARCH; }
};
...
private:
	int __stdcall filter(EXCEPTION_POINTERS*);

	static LPTOP_LEVEL_EXCEPTION_FILTER fake() { return (LPTOP_LEVEL_EXCEPTION_FILTER)0xffffffff; }
};
...
inline void exception_trap<P>::install(bool b) {
	...
	if (b) {
		m_fn_prev_filter = SetUnhandledExceptionFilter((LPTOP_LEVEL_EXCEPTION_FILTER)GetThunk());
		m_fn_prev_filter = !m_fn_prev_filter ? fake() : m_fn_prev_filter;
	}
}

template <typename P>
inline int __stdcall exception_trap<P>::filter(EXCEPTION_POINTERS* pex) {
