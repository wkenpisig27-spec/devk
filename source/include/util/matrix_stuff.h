...
// constructors, destructor, assignment operators
template <class T>
inline matrix_stuff<T>::matrix_stuff(size_t row, size_t col) { _m = new core_matrix(row, col, nullptr); }

template <class T>
inline matrix_stuff<T>::matrix_stuff(size_t circle) {
	size_t row_col = 1 + 2 * circle;
	_m = new core_matrix(row_col, row_col, nullptr);
}
...
inline void matrix_stuff<T>::_realloc(size_t row, size_t col) {
	if (row == _m->_row_size && col == _m->_col_size) {
		...
		_m->_col = _m->_col_size;
		return;
	}

	core_matrix* m1 = new core_matrix(row, col, nullptr);
...
inline T* matrix_stuff<T>::operator()(size_t row) {
	if (row >= _m->_row)
		throw std::logic_error("matrix_stuff<T>::operator(): index out of range!");
	if (_m->_ref_cnt > 1)
		_clone(); // copy on possiblely write
	return _m->_vp[row];
}
