...
public:
	...
	TDt& operator=(int lJulian);	  // dt = 36525 is 12-31-1999
	...
	friend bool operator<(int dt1, TDt& dt2);
	friend bool operator<=(int dt1, TDt& dt2);
	friend bool operator>(int dt1, TDt& dt2);
	friend bool operator>=(int dt1, TDt& dt2);
	friend bool operator==(int dt1, TDt& dt2);
	...
	bool operator<(int l) { return Julian() < l; }
	bool operator<=(int l) { return Julian() <= l; }
	bool operator>(int l) { return Julian() > l; }
	bool operator>=(int l) { return Julian() >= l; }
	bool operator==(int l) { return Julian() == l; }
	...
	bool IsNull();
	int Julian(); // returns julian date
	...
	int DifferenceInYears(int d1, int d2); // result = d1 - d2
	int DifferenceInMonths(int d1, int d2);

	static int m_nDisplayFormat;

private:
	void GetMDY(int& m, int& d, int& y, int jul);
...
public:
	...
	void SetWhere(const char* columnName, int value, char a_DataType = LONG);
...
public:
	...
	int lSQLGetData(int column, int a_eDataType);
