...
struct SDataSection {
	...
	DWORD_PTR dwParam;	// 参数 (DWORD_PTR for x64 pointer compatibility)

	SDataSection()
		: pData(nullptr), dwActiveTime(0), dwDataOffset(0), dwParam(0) {
	}
};

class CSectionDataMgr;
typedef int(CALLBACK* MAP_LOADING_PROC)(int nFlag, int nSectionX,
										 int nSectionY, DWORD_PTR dwParam, CSectionDataMgr* pMapData);
