...
extern LONG64 g_lMaxChaAttr[ATTR_MAX_NUM];
...
public:
	...
	void Clear();
	void Init(int lID, bool bFromFile = true);
	LONG64 GetAttr(int lNo);
	int SetAttr(int lNo, LONG64 lVal);
	LONG64 GetAttrMaxVal(int lNo);
	int DirectSetAttr(int lNo, LONG64 lVal); // 直接设置最大值，只用于客户端（因为客户端的数据来源于服务端，是合法数据）
	int AddAttr(int lNo, LONG64 lVal);
	int ReEvaluation(int lNo);
...
inline LONG64 CChaAttr::GetAttrMaxVal(int lNo) {
	T_B if (lNo >= ATTR_MAX_NUM) return -1;
...
inline int CChaAttr::SetAttr(int lNo, LONG64 lVal) {
	T_B if (lNo >= ATTR_MAX_NUM) return 0;

	int lRet = 2;
	if (lVal > g_lMaxChaAttr[lNo]) {
		lRet = 1;
		// if (!g_IsNosignChaAttr(lNo))
		{
			lVal = g_lMaxChaAttr[lNo];
			// LG("attr_error", "设置Cha[%s] Attrib[%d]超过最大值[%d]\n", m_szName, lNo, lVal);
			LG("attr_error", "set Cha[%s] Attrib[%d] greater than max value [%lld]\n", m_szName, lNo, lVal);
		}
	}
...
inline int CChaAttr::DirectSetAttr(int lNo, LONG64 lVal) {
	T_B if (lNo >= ATTR_MAX_NUM) return 0;
...
inline int CChaAttr::AddAttr(int lNo, LONG64 lVal) {
	T_B if (lNo >= ATTR_MAX_NUM) return 0;
...
inline int CChaAttr::ReEvaluation(int lNo) {
	T_B if (lNo >= ATTR_MAX_NUM) return 0;
...
inline void CChaAttr::SetChangeBitFlag(int lBit) {
...
inline bool CChaAttr::GetChangeBitFlag(int lBit) {
