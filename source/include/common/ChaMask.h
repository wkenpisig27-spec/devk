...
public:
	...
	// 从数据库读取的mask数据
	bool InitMaskData(char* pszMapName, const char* szBase64Data, int llen = 0);

	// 得到某个地图的mask数据
	BYTE* GetMapMask(const char* pszMapName, int& size);
	...
	// 得到结果数据
	// int	GetResultLength(){ return lResultSize;}
