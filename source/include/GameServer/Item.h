...
public:
	...
	// Check if item has expired and should be deleted
	// Used by GameItemRun for deferred deletion to avoid iterator invalidation
	bool ShouldExpire(dbc::uLong ulCurTick) const {
		return (m_ulOnTick != 0 && ulCurTick - m_ulStartTick >= m_ulOnTick);
	}
	
	// Get the submap for deletion safety check
	...
	void SetProtCha(dbc::uLong ulChaID, dbc::uLong ulChaHandle) {
		if (m_ulProtOnTick == 0)
			m_ulProtID = 0;
		else
			m_ulProtID = ulChaID, m_ulProtHandle = ulChaHandle;
	}
