...
public:
	...
	void WriteLong(int l) { _pkt.wl(l); }
	...
	int ReverseReadLong() { return _pkt.rrl(); }
...
inline void cfl_pkt::wstr(char const* str) { // add the '\0'
...
inline char cfl_pkt::rc() {
	...
	if (has_data(sizeof(char))) {
		...
		throw std::out_of_range("read char out of memory");
}
...
inline short cfl_pkt::rs() {
	...
	if (has_data(sizeof(short))) {
		...
		throw std::out_of_range("read short out of memory");
}
...
inline int cfl_pkt::ri() {
	...
	if (has_data(sizeof(int))) {
		...
		throw std::out_of_range("read int out of memory");
}

inline int cfl_pkt::rl() {
	int l;
	if (has_data(sizeof(int))) {
#ifdef USE_NBO
		memcpy((void*)&l, (void*)(buf + offset), sizeof(int));
		offset += sizeof(int);
		return ntohl(l);
#else
		l = *(int*)(buf + offset);
...
inline int cfl_pkt::rrl() {
	...
	offset_r -= sizeof(int);
#ifdef USE_NBO
	memcpy((void*)&l, (void*)(buf + offset_r), sizeof(int));
	return ntohl(l);
#else
	l = *(int*)(buf + offset_r);
	return l;
...
inline char const* cfl_pkt::rb(unsigned short& len) {
	...
	if (has_data(len)) {
		...
		throw std::out_of_range("read block out of memory");
}
