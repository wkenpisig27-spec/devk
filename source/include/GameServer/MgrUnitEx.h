...
public:
	CEntityListNode(unsigned int ulSize = 1) : PreAllocStru(1) {
		m_pCEntity = 0;
		m_pCNext = 0;
		m_pCLast = 0;
	}
...
public:
	CMgrUnit(unsigned int ulSize = 1) : PreAllocStru(1) {
		m_sPosX = 0;
		m_sPosY = 0;
		m_sAreaAttr = 0;
		m_chIslandID = 0;
	...
	int GetEntityNum(void) { return m_lEntityNum; }
	int GetStateNum(void) { return m_CSkillState.m_chStateNum; }
	...
	bool AddState(unsigned char uchFightID, unsigned int ulSrcWorldID, int lSrcHandle, char chEffType,
				  char chStateID, char chStateLv, unsigned int ulStartTick, int lOnTick, char chType);
	void DelState(char chStateID);
	bool AddStateToCharacter(char chStateNo, CCharacter* pCCha, int lOnTime, char chAddType, bool bNotice = true);
	...
	int m_lActiveNum; // 激活计数
	int m_lEntityNum; // 实体总数
...
public:
	CMgrNode(unsigned int ulSize = 1) : PreAllocStru(1) {
		m_pCMgrUnit = 0;
		m_pCEntityNode = 0;
		m_pCNext = 0;
		m_pCLast = 0;
