...
public:
	...
	bool IsValidPos(dbc::Short sUnitX, dbc::Short sUnitY) {
		if (sUnitX < 0 || sUnitX >= GetWidth() || sUnitY < 0 || sUnitY >= GetHeight())
			return false;
		return true;
	}
...
public:
	...
	// 视野单元 (members below were in an anonymous struct, removed for GCC compatibility)
	dbc::cShort m_csEyeshotCellWidth;
	dbc::cShort m_csEyeshotCellHeight;
	dbc::Short m_sEyeshotCellLin;
	dbc::Short m_sEyeshotCellCol;

	// 状态单元 (members below were in an anonymous struct, removed for GCC compatibility)
	...
	// 入口信息 (members below were in an anonymous struct, removed for GCC compatibility)
