...
inline int lua_GetChaAttrI(lua_State* L) {
	T_B return lua_GetChaAttr(L);
	T_E
}

// 设置角色的指定属性
inline int lua_SetChaAttrI(lua_State* L) {
	T_B return lua_SetChaAttr(L);
	T_E
}
...
inline int lua_AddWeatherRegion(lua_State* L) {
	// 参数合法性判别
	BOOL bValid = (lua_gettop(L) == 7 && lua_isnumber(L, 1) && lua_isnumber(L, 2) && lua_isnumber(L, 3) && lua_isnumber(L, 4) && lua_isnumber(L, 5) && lua_isnumber(L, 6) && lua_isnumber(L, 7));
	if (!bValid) {
		PARAM_ERROR;
		return 0;
	}
