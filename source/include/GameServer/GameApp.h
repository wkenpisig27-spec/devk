...
public:
	...
	CPlayer* GetPlayer(int lHandle);
	CPlayer* IsValidPlayer(int lID, int lHandle);
	...
	Entity* IsLifeEntity(unsigned int ulID, int lHandle);
	...
	int GetSStateTraOnTime(unsigned char uchStateID, unsigned char uchStateLv);
	...
	CCharacter* FindPlayerChaByID(unsigned int ulChaID);
	CCharacter* FindMainPlayerChaByID(unsigned int ulChaID);
	CCharacter* FindChaByID(unsigned int ulChaID);
	CCharacter* FindChaByName(const char* cszChaName);
	CPlayer* FindPlayerByDBChaID(unsigned int ulDBChaID);

	void WorldNotice(const char* szString); // 通知本组所有GameServer上的玩家
	void GuildNotice(unsigned int guildID, const char* szString);
...
private:
	...
	int m_lSStateTraOnTime[AREA_STATE_MAXID + 1][SKILL_STATE_LEVEL + 1]; // 从地表转移到角色身上的状态的持续时间
...
inline int CGameApp::GetSStateTraOnTime(unsigned char uchStateID, unsigned char uchStateLv) {
...
// 根据WorldID搜索本进程的所有角色
inline CCharacter* CGameApp::FindPlayerChaByID(unsigned int ulChaID) {
	BEGINGETGATE();
	CPlayer* pCPlayer;
	CCharacter* pCha = 0;
	GateServer* pGateServer;
...
// 根据数据ID搜索本进程的所有玩家
inline CPlayer* CGameApp::FindPlayerByDBChaID(unsigned int ulDBChaID) {
	BEGINGETGATE();
	CPlayer* pCPlayer;
	GateServer* pGateServer;
	while (pGateServer = GETNEXTGATE()) {
...
inline CCharacter* CGameApp::FindMainPlayerChaByID(unsigned int ulChaID) {
	BEGINGETGATE();
	CPlayer* pCPlayer;
	CCharacter* pCha = 0;
	GateServer* pGateServer;
...
// 根据WorldID搜索本进程的所有角色
inline CCharacter* CGameApp::FindChaByID(unsigned int ulChaID) {
	CCharacter* pCCha;
