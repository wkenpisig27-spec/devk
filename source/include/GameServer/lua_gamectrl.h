...
inline int lua_ChaUseSkill(lua_State* L) {
	...
	if (pCha && pTarget) {
		...
		if (bExecNow) {
			...
			pCha->m_CActCache.PushParam(&lSkillID, sizeof(int));
...
inline int lua_ChaUseSkill2(lua_State* L) {
	...
	if (pCha) {
		...
		int lPosX = (int)lua_tonumber(L, 4);
		int lPosY = (int)lua_tonumber(L, 5);
		if (bExecNow) {
			pCha->Cmd_BeginSkillDirect2(lSkillID, lSkillLv, lPosX, lPosY);
		} else {
			pCha->m_CActCache.AddCommand(enumCACHEACTION_SKILL2);
			pCha->m_CActCache.PushParam(&lSkillID, sizeof(int));
			pCha->m_CActCache.PushParam(&lSkillLv, sizeof(int));
			pCha->m_CActCache.PushParam(&lPosX, sizeof(int));
			pCha->m_CActCache.PushParam(&lPosY, sizeof(int));
		}
	}
...
inline int lua_GetChaByRange(lua_State* L) {
	...
	unsigned int ulMinDist2 = r * r, ulTempDist2;
	int lDistX, lDistY;
...
inline int lua_view(lua_State* L) {
	...
	extern int g_lViewAtMapX;
	extern int g_lViewAtMapY;
	g_lViewAtMapX = (int)lua_tonumber(L, 1);
	g_lViewAtMapY = (int)lua_tonumber(L, 2);
	return 0;
	T_E
}
...
inline int lua_Msg(lua_State* L) {
	...
	MessageBox(nullptr, pszContent, "msg", 0);
	return 0;
	T_E
}

inline int lua_Exit(lua_State* L) {
	T_B extern volatile BOOL g_bGameEnd;
	g_bGameEnd = TRUE;
	return 1;
	T_E
}
...
inline int lua_PRINT(lua_State* L) {
	...
	__try {
		...
		for (int i = 1; i <= count; i++) {
			...
			switch (luaType) {
			case LUA_TNIL:
				strncat(szBuf, "nil  ", sizeof(szBuf) - strlen(szBuf) - 1);
				break;
			case LUA_TBOOLEAN:
				strncat(szBuf, lua_toboolean(L, i) ? "TRUE  " : "FALSE  ", sizeof(szBuf) - strlen(szBuf) - 1);
				break;
			case LUA_TLIGHTUSERDATA:
			case LUA_TUSERDATA: {
				const void* p = lua_touserdata(L, i);
				snprintf(temp, sizeof(temp) - 1, "userdata:%p  ", p);
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			case LUA_TNUMBER: {
				snprintf(temp, sizeof(temp) - 1, "%g  ", lua_tonumber(L, i));
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			case LUA_TSTRING: {
				const char* pszData = lua_tostring(L, i);
				snprintf(temp, sizeof(temp) - 1, "%s  ", pszData ? pszData : "nil");
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			case LUA_TTABLE: {
				snprintf(temp, sizeof(temp) - 1, "table:%p  ", lua_topointer(L, i));
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			case LUA_TFUNCTION: {
				snprintf(temp, sizeof(temp) - 1, "function:%p  ", lua_topointer(L, i));
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			case LUA_TTHREAD: {
				snprintf(temp, sizeof(temp) - 1, "thread:%p  ", (void*)lua_tothread(L, i));
				strncat(szBuf, temp, sizeof(szBuf) - strlen(szBuf) - 1);
			} break;
			default:
				strncat(szBuf, "unknown  ", sizeof(szBuf) - strlen(szBuf) - 1);
				break;
			}
		}
		strncat(szBuf, "\r\n", sizeof(szBuf) - strlen(szBuf) - 1);
		printf("%s", szBuf);
	}
	__except(EXCEPTION_EXECUTE_HANDLER) {
		// Silently ignore access violations in PRINT
		return 0;
	}
	return 0;
}
...
inline int lua_LG(lua_State* L) {
	// Guard against null or invalid lua_State pointer
	if (!L) {
		return 0;
	}
	
	// Use Windows SEH to catch access violations from corrupted pointers
	__try {
		int count = lua_gettop(L);
		if (count <= 1) {
			return 0;
		...
		char szBuf[1024 * 2] = {0};
		const char* pszMsg = lua_tostring(L, 2);
		snprintf(szBuf, sizeof(szBuf) - 1, "%s ", pszMsg ? pszMsg : "(null)");
		...
		strncat(szBuf, "\n", sizeof(szBuf) - strlen(szBuf) - 1);
		LG((char*)pszFile, "%s", szBuf);
	}
	__except(EXCEPTION_EXECUTE_HANDLER) {
		// Silently ignore access violations in logging - don't crash the server
		return 0;
	}
	return 0;
}
...
inline int lua_EXLG(lua_State* L) {
	...
	for (int i = 3; i <= nNumParam; i++) {
		...
		strncpy_s(szData, sizeof(szData), pszTemp + sPos1, (sNum > 1020) ? 1020 : sNum);
