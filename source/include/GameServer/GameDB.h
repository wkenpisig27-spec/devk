...
public:
	...
	BOOL AddCreditByDBID(DWORD cha_id, int lCredit);
	BOOL IsChaOnline(DWORD cha_id, BOOL& bOnline);
	Long GetChaAddr(DWORD cha_id);
	bool SetChaAddr(DWORD cha_id, Long addr);

	bool SetGuildPermission(int cha_id, unsigned int perm, int guild_id);

	BOOL SaveStoreItemID(DWORD cha_id, int lStoreItemID);
	BOOL AddMoney(DWORD cha_id, __int64 money);
...
public:
	CTableMaster(cfl_db* pDB)
		: cfl_rs(pDB, "master", 6) { T_B

										 T_E }

	bool Init(void);
	unsigned int GetMasterDBID(CPlayer* pPlayer);
...
public:
	CTableLotterySetting(cfl_db* pDB)
		: cfl_rs(pDB, "LotterySetting", 10) { T_B
...
public:
	CTableAmphitheaterSetting(cfl_db* pDB)
		: cfl_rs(pDB, "AmphitheaterSetting", 10) { T_B
...
public:
	CTableAmphitheaterTeam(cfl_db* pDB)
		: cfl_rs(pDB, "AmphitheaterTeam", 10) { T_B
...
public:
	CTableResource(cfl_db* pDB)
		: cfl_rs(pDB, "resource", 10) { T_B

											T_E }

	bool Init(void);
	bool Create(int& lDBID, int lChaId, int lTypeId);
	...
	bool ReadKitbagTmpData(int lRecDBID, std::string& strData);
	bool SaveKitbagTmpData(int lRecDBID, const std::string& strData);
...
public:
	CTableMapMask(cfl_db* pDB)
		: cfl_rs(pDB, "map_mask", 10) { T_B

											T_E }

	bool Init(void);
	bool Create(int& lDBID, int lChaId);
	bool ReadData(CPlayer* pCPly);
	bool SaveData(CPlayer* pCPly, BOOL bDirect = FALSE);
	bool GetColNameByMapName(const char* szMapName, char* szColName, int lColNameLen);
...
protected:
	...
	std::list<std::string> _SaveMapMaskList; // 保存大地图的队列
};
...
public:
	...
	bool Init(void);
	int Create(CCharacter* pCha, char* guildname, cChar* passwd);
	...
	bool UpdateGuildBankGold(int guildID, long long money);
	...
	bool GetGuildName(int lGuildID, std::string& strGuildName);

	// 公会挑战
	bool Challenge(CCharacter* pCha, BYTE byLevel, long long dwMoney);
	bool Leizhu(CCharacter* pCha, BYTE byLevel, long long dwMoney);
	void ListChallenge(CCharacter* pCha);
	bool GetChallInfo(BYTE byLevel, DWORD& dwGuildID1, DWORD& dwGuildID2, long long& dwMoney);
	bool StartChall(BYTE byLevel);
	bool HasCall(BYTE byLevel);
	void EndChall(DWORD dwGuild1, DWORD dwGuild2, BOOL bChall);
	void ChallMoney(BYTE byLevel, BOOL bChall, DWORD dwGuildID, DWORD dwChallID, long long dwMoney);
...
public:
	...
	~CGameDB() {
		...
		// Fixed: Memory leaks - these tables were allocated in Init() but never deleted
		SAFE_DELETE(_tab_res);
		SAFE_DELETE(_tab_setting);
		SAFE_DELETE(_tab_ticket);
		SAFE_DELETE(_tab_winticket);
		SAFE_DELETE(_tab_amp_setting);
		SAFE_DELETE(_tab_amp_team);
	...
	unsigned int GetPlayerMasterDBID(CPlayer* pPlayer) {
	...
	Long SetGuildPermission(int cha_id, unsigned int perm, int guild_id) {
	...
	BOOL SaveMissionData(CPlayer* pPlayer, DWORD cha_id) // 角色任务信息存盘
	{
		T_B return _tab_cha->SaveMissionData(pPlayer, cha_id);
		T_E
	}

	// 船只存储
	BOOL Create(DWORD& dwBoatID, const BOAT_DATA& Data) {
		T_B return _tab_boat->Create(dwBoatID, Data);
		T_E
	}
	BOOL GetBoat(CCharacter& Boat) {
		T_B return _tab_boat->GetBoat(Boat);
		T_E
	}
	BOOL SaveBoat(CCharacter& Boat, char chSaveType) {
		T_B return _tab_boat->SaveBoat(Boat, chSaveType);
		T_E
	}
	BOOL SaveBoatDelTag(DWORD dwBoatID, BYTE byIsDeleted = 0) {
	...
	int CreateGuild(CCharacter* pCha, char* guildname, cChar* passwd) {
		return _tab_gld->Create(pCha, guildname, passwd);
	}

	int GetGuildBank(uLong guildid, CKitbag* bag) {
		return _tab_gld->GetGuildBank(guildid, bag);
	}

	int UpdateGuildBank(uLong guildid, CKitbag* bag) {
	...
	bool GetChall(BYTE byLevel, DWORD& dwGuildID1, DWORD& dwGuildID2, long long& dwMoney) {
		for (int i = 0; i < 100; i++) {
	...
	bool LoadOfflineStalls(const char* szMapName, class COfflineStallMgr* pMgr);
	
	// Create a new offline stall
	DWORD CreateOfflineStall(DWORD dwChaID, const char* szChaName, DWORD dwActID,
	                         ...
	                         const BYTE* pEquipLookData = nullptr, DWORD dwEquipLookDataSize = 0,
	                         BYTE byStallLevel = 1);
	
	// Delete an offline stall by stall ID
	bool DeleteOfflineStall(DWORD dwStallID);
	
	// Delete an offline stall by character ID (returns pending gold)
	__int64 DeleteOfflineStallByCharId(DWORD dwChaID);
	
	// Check if character has an offline stall
	bool HasOfflineStall(DWORD dwChaID);
	
	// Update stall items after purchase
	bool UpdateOfflineStallItems(DWORD dwStallID, BYTE byItemCount,
	                             const BYTE* pItemData, DWORD dwItemDataSize,
	                             __int64 llPendingGold);
	
	// Extend stall expiration time
	bool ExtendOfflineStallTime(DWORD dwStallID, int nExtendHours);
	
	// Get offline stall info by character ID
	bool GetOfflineStallByCharId(DWORD dwChaID, struct SOfflineStallInfo* pOutInfo);

	// Get player login IP and MAC from AccountServer database
	bool GetPlayerLoginInfo(const char* actName, std::string& outIP, std::string& outMAC);
