...
inline int lua_GetChaAttr(lua_State* pLS) {
	...
	}
	{
		...
		// nAttrVal = (int)pCCha->getAttr(sAttrIndex);
		lAttrVal = pCCha->getAttr(sAttrIndex);
	}
End:
	if (bSuccess) {
		if (lAttrVal < 0 && g_IsNosignChaAttr(sAttrIndex))
			lua_pushnumber(pLS, (double)(unsigned __int64)lAttrVal);
		else
			lua_pushnumber(pLS, (double)lAttrVal);
		return 1;
	} else
		return 0;
	T_E
...
inline int lua_SetChaAttr(lua_State* pLS) {
...

	{
		...
		int lSetRet = pCCha->setAttr(sAttrIndex, lValue);
		if (lSetRet == 0) {
			bSuccess = false;
			goto End;
		}
...
inline int lua_RemoveChaItem(lua_State* pLS) {
	...
	}
	{
		...
		{
			int lItemID = (int)lua_tonumber(pLS, 2);
			int lItemNum = (int)lua_tonumber(pLS, 3);
...
inline int lua_GetChaItem(lua_State* pLS) {
	...
	}
	{
		...
		pSItem = pCCha->GetItem2((char)lua_tonumber(pLS, 2), (int)lua_tonumber(pLS, 3));
	}
End:
	if (bSuccess) {
#ifdef defPARSE_LOG
...
inline int lua_GetChaItem2(lua_State* pLS) {
	...
	}
	{
		...
		pSItem = pCCha->GetItem((char)lua_tonumber(pLS, 2), (int)lua_tonumber(pLS, 3));
	}
End:
	if (bSuccess) {
#ifdef defPARSE_LOG
...
inline int lua_GetItemAttr(lua_State* pLS) {
	...
	}
	{
		...
		{
			int lAttrID = (int)lua_tonumber(pLS, 2);
...
// Get item grid validity status (for equipment like fairy/mount that can be invalid)
// Parameter: Item object (SItemGrid pointer)
...
inline int lua_GetItemGridValid(lua_State* pLS) {
	...
	if (!pSItem) {
		...
		lua_pushnumber(pLS, pSItem->IsValid() ? 1 : 0);
	}
	return 1;
}
...
inline int lua_GetItemForgeParam(lua_State* pLS) {
	...
	}
	{
		...
		{
			int lType = (int)lua_tonumber(pLS, 2);
			if (lType == 0)
...
// Get map entry by target map name
// Parameter: target map name (string)
// Returns: entry object (lightuserdata) or nil if not found
inline int lua_GetMapEntryByName(lua_State* pLS) {
#ifdef defPARSE_LOG
	g_pCLogObj->Log("Get map entry by name : GetMapEntryByName\n");
#endif
	...
	const char* szMapName = lua_tostring(pLS, 1);
	if (!szMapName) {
#ifdef defPARSE_LOG
		g_pCLogObj->Log("\tMap name is null, transfer failed\n");
#endif
		return 0;
	}

	CDynMapEntryCell* pEntry = g_CDMapEntry.GetEntry(szMapName);
	if (pEntry) {
		lua_pushlightuserdata(pLS, pEntry);
		return 1;
	}
...
inline int lua_GetGuildName(lua_State* pLS) {
	...
	{
		int lGuildID = (int)lua_tonumber(pLS, 1);
...
inline int lua_SetMapCopyParam(lua_State* pLS) {
	...
	}
	{
		...
		if (!pCCpyMgr->SetParam((char)lua_tonumber(pLS, 2) - 1, (int)lua_tonumber(pLS, 3))) {
			bSuccess = false;
			goto End;
		}
	}
...
inline int lua_SetMapCopyParam2(lua_State* pLS) {
...

	{
		...
		if (!pCMapCpy->SetInfoParam((char)lua_tonumber(pLS, 2) - 1, (int)lua_tonumber(pLS, 3))) {
			bSuccess = false;
			goto End;
		}
	}
...
inline int lua_GetMapEntryCopyObj(lua_State* pLS) {
	bool bSuccess = true;
	CMapEntryCopyCell* pCCopyCell = nullptr;
...
inline int lua_SetChaParam(lua_State* pLS) {
	...
	}
	{
		...
		if (!pCCha->SetScriptParam((char)lua_tonumber(pLS, 2) - 1, (int)lua_tonumber(pLS, 3))) {
			bSuccess = false;
			goto End;
		}
	}
...
inline int lua_GetCaptainByMapId(lua_State* pLS) {
	...
	DWORD Capid1 = 0;
	DWORD Capid2 = 0;
	CCharacter* pCCha1 = nullptr;
	CCharacter* pCCha2 = nullptr;
	CPlayer* player1 = nullptr;
	CPlayer* player2 = nullptr;
...
inline int lua_SetExpiration(lua_State* L) {
	...
	}
	{
		...
		int expiration = *static_cast<int*>(lua_touserdata(L, 2));
...
inline void RegisterLuaGameLogic(lua_State* L) {
	...
	if (!g_pCLogObj) {
		...
		throw std::runtime_error(RES_STRING(GM_EXPAND_H_00160));
	}
