...
inline const char* g_GameGateConnError(int error_code) {
	...
	case ERR_TM_OVERNAME:
		return RES_STRING(GM_GAMEAPPNET_H_00001);
	case ERR_TM_OVERMAP:
		return RES_STRING(GM_GAMEAPPNET_H_00002);
	case ERR_TM_MAPERR:
		return RES_STRING(GM_GAMEAPPNET_H_00003);
	default:
		return RES_STRING(GM_GAMEAPPNET_H_00004);
	}
}
...
#define VALIDRPACKET(pkt) (pkt != nullptr) ? true : false
