...
public:
	...
	int m_lSocketAlive;	  // Socket保持活动
	int m_nMaxPly;			  // 最大玩家数
	int m_nMaxCha;			  // 最大角色数
	int m_nMaxItem;			  // 最大道具数
	int m_nMaxTNpc;			  // 最大对话NPC
	unsigned int m_ulBaseID; // 服务器的ID基数
	int m_lItemShowTime;	  // 道具存在时间
	int m_lItemProtTime;	  // 道具保护时间
	int m_lSayInterval;	  // 喊话间隔
	char m_szResDir[255];	  // 运行资源所在的目录
	char m_szLogDir[255];	  // Log所在的目录
	char m_chMapMask;		  // 是否存取大地图
	int m_lDBSave;			  // 数据库定时存盘时间间隔
	...
	DWORD m_dwStallTime;
	BOOL m_bOfflineStall;  // Enable database-persisted offline stalls
	...
	BOOL m_bFastMapSwitch;  // Enable optimized fast map switching (single GameServer mode)
};
