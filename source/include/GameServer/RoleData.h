...
typedef struct _ROLE_FLAGINFO {
	...
	BOOL Load(LPBYTE pBuf, DWORD dwSize) {
		if (dwSize >= ROLE_MAXNUM_RECORDSIZE)
			return FALSE;
	...
	BOOL IsSet(WORD wFlag) {
		if (wFlag >= ROLE_MAXNUM_FLAGSIZE << 3)
			return FALSE;
...
typedef struct _ROLE_RECORDINFO {
	...
	BOOL Load(LPBYTE pBuf, DWORD dwSize) {
		T_B if (dwSize >= ROLE_MAXNUM_RECORDSIZE << 3) return FALSE;
	...
	BOOL IsValid(DWORD dwID) {
		T_B return dwID < ROLE_MAXNUM_RECORDSIZE << 3;
		T_E
	}

	BOOL IsSet(DWORD dwID) {
		T_B if (dwID >= ROLE_MAXNUM_RECORDSIZE << 3) return FALSE;
