...
typedef struct _RoleInfo_ {
	...
	int moBean;	  //	摩豆数量
	int rplMoney;	  //	代币数量
	int vip;		  //  VIP种类(20-50)，0为非vip
} RoleInfo, *pRoleInfo;

//  交易记录
typedef struct _HistoryInfo_ {
	int32_t tradeTime; //	交易时间 (unix timestamp, 32-bit for cross-platform compatibility)
	int comID;		  //	商品编号
...
typedef struct _AfficheInfo_ {
	...
	int32_t affiTime;	//  发布时间 (unix timestamp, 32-bit for cross-platform compatibility)
...
typedef struct _StoreInfo_ {
	...
	char comName[21];	 //	商品名称
	int comClass;		 //	商品分类
	int32_t comTime;		 //	商品上架时间 (unix timestamp, 32-bit)
	int comPrice;		 //	价格
	char comRemark[129]; //	商品备注
	int comNumber;		 //  商品剩余个数。-1不限制，0下架
	int32_t comExpire;	 //  商品到期时间。 (unix timestamp, 32-bit)
	_byte itemNum;		 //  道具个数
	_byte isHot;		 //  是否热卖
	int32_t beginTime;	 //  上架时间 (unix timestamp, 32-bit)
...
typedef struct _MailInfo_ {
	...
	int32_t sendTime;	   //	发送时间 (unix timestamp, 32-bit for cross-platform compatibility)
} MailInfo, *pMailInfo;
...
//  解析消息
extern int PeekNetMessage(pNetMessage* nm, unsigned char** buf, int& bufsize, void** tmpbuf, int& tmpsize);

//  释放消息
extern void FreeNetMessage(pNetMessage msg);

//  生成消息
extern bool BuildNetMessage(pNetMessage nm, int msgID, int subID, int extend, int section, unsigned char* body, int size);
...
public:
	...
	virtual bool PostInfoSend(int msgID, int subID, int extend, int section, unsigned char* body, int size, bool resnd = false); //  发送消息
