...
Sender::Sender(DataSocket* datasock)
	: m_datasock(datasock), __tca(0), m_p(0), m_send(0), m_head(0), m_tail(0) {
		...
		if (m_tail && m_tail->HasSpace()) {
...
#if 0
            RPacket Test(wpk);
            if(Test.ReadCmd() != 10000)
            {
                LG("DebugOut", "Send %s(%d) ==> %s(%d), cmd=%d\n",
                   m_datasock->GetLocalIP(), m_datasock->GetLocalPort(),
                   m_datasock->GetPeerIP(), m_datasock->GetPeerPort(), Test.ReadCmd());
            }
#endif
			// End
			...
			*m_tail << wpk;
			...
			try {
				...
				LG("comm", "Sender::operator<<(WPacket &wpk) exception, PeerIP:%s\n", m_datasock->GetPeerIP());
			}
			l_lockComb.lock();
		} // if(m_tail && m_tail->HasSpace())
	}	  // while(bool(wpk) && m_datasock->m_delflag ==0)
