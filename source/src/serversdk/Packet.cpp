...
WPacket::WPacket(const RPacket& rpk)
	: rptr<rbuf>(rpk), __tca(rpk.__tca), m_offset(rpk.m_offset), m_head(rpk.m_head), m_wpos(rpk.GetDataLen() - em_cmdsize), m_cpos(0) //,m_appendant(false)
{}
...
WPacket& WPacket::operator=(const RPacket& rpk) {
	if (!rpk) {
		/*
		 * NOTE(Ogge):
		 * - The RPacket rpk does not have any ownership of a buffer; there is nothing copy.
		 *	- Make this WPacket empty.
		 */
		*this = WPacket(nullptr);
		return *this;
	}
	...
	if (__tca) {
		if ((__tca == rpk.__tca) || (m_head == rpk.m_head)) {
			/*
			 * NOTE(Ogge):
			 * - I think __tca must match for the sync rpc (SESS) to work.
			 * 	- if __tca == rpk.__tca we want to retain the SESS value.
			 * 	- if __tca == rpk.__tca then header size should be same as its originate from same __tca?
			 * - I think header size (m_head) must match due to rptr<rbuf> being a reference counted shared ownership ( buffer ).
			 * 	- Misalignment issues if header size is different.
			 */
			...
			/*
			 * NOTE(Ogge):
			 * - Allocate a new buffer for this WPacket, and only copy the content data; no header data.
			 */
			m_offset = 0;
		...
		/*
		 * NOTE(Ogge):
		 * - Share ownership of buffer from RPacket with this WPacket.
		 * - Copy header size; header size is normally transferred from __tca to WPacket.
		 */
		rptr<rbuf>::operator=(rpk);
...
void WPacket::WritePktLen() const {
	...
	if (!bool(*this))
		return;
	if (GetPktLen() < (__tca->__len_offset + __tca->__len_size))
		return;
...
//--------------------------------------------------------------------
bool WPacket::WriteLongLong(unsigned long long ll) {
	if (!bool(*this))
		return false;
	if (m_wpos >= SIGN32)
		return false;
	if (m_offset + GetPktLen() + sizeof(LLong) > Size()) {
		unsigned long long l_newlen = (GetPktLen() + sizeof(LLong)) + (Size() - m_offset) + (__bufheap.GetUnitSize() - 1);
		l_newlen /= __bufheap.GetUnitSize();
	...
	ll = htonll(ll);
	MemCpy(const_cast<char*>(GetDataAddr()) + GetDataLen(), (cChar*)&ll, sizeof(LLong));
	m_wpos += sizeof(LLong);
	return true;
}
//--------------------------------------------------------------------
...
RPacket::RPacket(const WPacket& wpk)
	: rptr<rbuf>(wpk), __tca(wpk.__tca), m_head(wpk.m_head), m_offset(wpk.m_offset), m_rpos(0), m_revrpos(0), m_tickcount(GetTickCount()), m_len(wpk.GetDataLen() + wpk.m_head) {}
RPacket::RPacket(const TcpCommApp* tca)
	: __tca(tca), m_offset(0), m_rpos(0), m_revrpos(0), m_len(0), m_tickcount(GetTickCount()), m_head(tca ? tca->GetPktHead() : 0) {}
//--------------------------------------------------------------------
...
RPacket& RPacket::operator=(const WPacket& wpk) {
	if (!wpk) {
		/*
		 * NOTE(Ogge):
		 * - The RPacket wpk does not have any ownership of a buffer; there is nothing copy.
		 *	- Make this RPacket empty.
		 */
		*this = RPacket(nullptr);
		return *this;
	}
...
//--------------------------------------------------------------------
unsigned long long RPacket::ReadLongLong() {
	unsigned long long l_retval = 0;
	if (RemainData() >= sizeof(LLong)) {
		MemCpy((char*)&l_retval, GetDataAddr() + em_cmdsize + m_rpos, sizeof(LLong));
		l_retval = ntohll(l_retval);
		m_rpos += sizeof(LLong);
	}
	return l_retval;
}
...
unsigned long long RPacket::ReverseReadLongLong() {
	unsigned long long l_retval = 0;
	if (GetDataLen() >= m_revrpos + sizeof(LLong)) {
		m_revrpos += sizeof(LLong);
		MemCpy((char*)&l_retval, GetDataAddr() + GetDataLen() - m_revrpos, sizeof(LLong));
		l_retval = ntohll(l_retval);
	}
	return l_retval;
}
