...
void Point::move(short dir, int distance) {
	dir %= 360;
	if (dir >= 0 && dir < 90) {
		x += int(g_sin[dir] * distance + 0.01f); // Testing some changes Mdr.st May 2020 - FPO alpha
		y -= int(g_cos[dir] * distance + 0.01f); // Original: 0.5f
	} else if (dir >= 90 && dir < 180) {
		x += int(g_sin[180 - dir] * distance + 0.01f);
		y += int(g_cos[180 - dir] * distance + 0.01f);
	} else if (dir >= 180 && dir < 270) {
		x -= int(g_sin[dir - 180] * distance + 0.01f);
		y += int(g_cos[dir - 180] * distance + 0.01f);
	} else if (dir >= 270 && dir < 360) {
		x -= int(g_sin[360 - dir] * distance + 0.01f);
		y -= int(g_cos[360 - dir] * distance + 0.01f);
	}
}
...
bool operator^(const Square& S1, const Square& S2) { // 锟角凤拷锟洁交
	int x = S1.centre.x - S2.centre.x;
	int y = S1.centre.y - S2.centre.y;
	x = (x >= 0) ? x : -x;
	y = (y >= 0) ? y : -y;
	int radius = S1.radius + S2.radius;
	return (x < radius) && (y < radius);
}
bool operator^(const Circle& C1, const Circle& C2) { // 锟洁交锟斤拷锟斤拷
	int x = C1.centre.x - C2.centre.x;
	int y = C1.centre.y - C2.centre.y;
	x = (x >= 0) ? x : -x;
	y = (y >= 0) ? y : -y;
...
bool operator^(const Rect& R1, const Rect& R2) { // 锟洁交锟斤拷锟斤拷
	int x = (R1.ltop.x + R1.rbtm.x) - (R2.ltop.x + R2.rbtm.x);
	int y = (R1.ltop.y + R1.rbtm.y) - (R2.ltop.y + R2.rbtm.y);
	x = (x >= 0) ? x : -x;
	y = (y >= 0) ? y : -y;
	int xr = (R1.rbtm.x - R1.ltop.x) + (R2.rbtm.x - R2.ltop.x);
	int yr = (R1.rbtm.y - R1.ltop.y) + (R2.rbtm.y - R2.ltop.y);
	return (x < xr) && (y < yr);
}
...
short arctan(const Point& src, const Point& dest) {
	int l_dltx = dest.x - src.x;
	int l_dlty = dest.y - src.y;
	short l_dir;
	if (l_dltx != 0) {
