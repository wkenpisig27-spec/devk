...
IniFile::IniFile(const char* filename)
	: m_rw(false), m_update(false), m_sectcount(0), m_sectsize(0), m_sect(0) {
...
void IniFile::ReadFile(const char* filename) {
	...
	if (!l_file) {
		throw excpFile(std::string("file") + filename + " error!");
	}
	try {
		char l_szbuf[1024];
		int l_lino = 0;
		...
		while (!feof(l_file)) {
			...
			if ((l_str1 = ltrim(l_szbuf))[0] == '[') {
				...
				if (!l_str2) {
					throw excpFile(std::string("INI�ļ���ʽ����:") + std::to_string(l_lino) + "��");
				}
				*l_str2 = 0;
...
IniSection& IniFile::operator[](int i) {
	...
	if (i >= 0 && i < m_sectcount) {
		...
		throw std::runtime_error(std::string("IniFile�����±�:") + std::to_string(i) + "Խ��");
	}
	return *l_is;
}
...
IniSection& IniFile::operator[](const char* sectname) const {
	...
	for (i = 0; i < m_sectcount; i++) {
		...
		if (!_stricmp(m_sect[i]->m_sectname.c_str(), sectname)) {
			break;
		}
	}
	if (i < m_sectcount) {
		...
		throw excpIniF(std::string("Section:") + sectname + " û���ҵ�.");
	}
}
...
IniItem& IniSection::operator[](int i) {
	...
	if (i >= 0 && i < m_itemcount)
		...
		throw std::runtime_error(std::string("Section:") + m_sectname.c_str() + " index " + std::to_string(i) + " out of range");
	}
	return *l_it;
}
std::string& IniSection::operator[](const char* name) const {
	...
	for (i = 0; i < m_itemcount; i++) {
		...
		if (!_stricmp(m_item[i]->name.c_str(), name)) {
			break;
		}
	}
	if (i < m_itemcount) {
		...
		throw excpIniF(std::string("Section:") + m_sectname.c_str() + " Name:" + name + " can't find.");
	}
}
