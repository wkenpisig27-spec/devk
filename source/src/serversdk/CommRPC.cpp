...
int OnServeCall::Process() {
	try {
		...
		try {
			int l_retval = __tca->_SendData(m_datasock, l_retbuf);
			if (l_retval) {
...
RPCMGR::RPCMGR(TcpCommApp* tca) : __tca(tca) {
	if (!__tca) {
		throw std::runtime_error("�����ṩͨ��Ӧ�ö���");
	}
}
//==============================================================================
int RPCMGR::SendSESS(DataSocket* datasock, WPacket& sendbuf) {
	sendbuf.WriteSESS(SESSFLAG);
	return __tca->_SendData(datasock, sendbuf);
}
int RPCMGR::AsynCall(DataSocket* datasock, WPacket in_parameter, void* pointer) {
	TFNCall* l_fncall{};
	int l_retval{};
	RPCInfo* l_rpcinfo{};
	...
	if (!l_fncall) {
		throw std::runtime_error(std::string("l_fncall is null!\t") + __func__);
	}
...
RPacket RPCMGR::SyncCall(DataSocket* datasock, WPacket in_parameter, uLong ulMilliseconds) {
	...
	if (l_rpcinfo) {
		...
		static uLong s_lastWarnTime = 0;
		static int s_warnCount = 0;
		++s_warnCount;
		uLong now = GetTickCount();
		if (now - s_lastWarnTime > 5000) {
			printf("SyncCall warning: get rpc info null! (count=%d)\n", s_warnCount);
			s_lastWarnTime = now;
			s_warnCount = 0;
		}
		return 0;
...
bool RPCMGR::ProcessSESS(DataSocket* datasock, RPacket& recvbuf) {
	...
	if (l_sessid > SESSFLAG) {
		...
		TFNCall *l_fncall, *l_prefncall;
		for (l_prefncall = 0, l_fncall = l_rpcinfo->m_fncall; l_fncall; l_prefncall = l_fncall, l_fncall = l_fncall->m_nextfnc) {
			if (l_fncall->m_sessid == l_sessid)
				break;
		}
		if (l_fncall) {
