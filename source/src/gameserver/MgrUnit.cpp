...
bool CMgrUnit::AddState(unsigned char uchFightID, unsigned int ulSrcWorldID, int lSrcHandle, char chEffType,
...
bool CMgrUnit::AddStateToCharacter(char chStateNo, CCharacter* pCCha, int lOnTime, char chAddType, bool bNotice) {
	if (!pCCha->IsLiveing())
		return false;
	CCharacter* pCSrcCha;
	Entity* pCSrcEnt;
...
bool CMgrUnit::AddStateToCharacter(SSkillStateUnit* pSStateUnit, CCharacter* pCCha, int lOnTime, char chAddType, bool bNotice) {
	if (!pCCha->IsLiveing())
		return false;
	CCharacter* pCSrcCha;
	Entity* pCSrcEnt;
...
void CMgrUnit::StateRun(unsigned int ulCurTick, SubMap* pCMap) {
	char chCount = 0;
