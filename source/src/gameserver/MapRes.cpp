...
CMapRes::CMapRes()
	: m_pCLeftMap(0), m_pCTopMap(0), m_pCRightMap(0), m_pCBelowMap(0), m_pCMonsterSpawn(0), m_pCMapSwitchEntitySpawn(0), m_pNpcSpawn(0), m_csEyeshotCellWidth(800), m_csEyeshotCellHeight(800), m_csStateCellWidth(200), m_csStateCellHeight(200), m_csBlockUnitWidth(50), m_csBlockUnitHeight(50), m_byMapID(0), m_bCanStall(true), m_bCanGuild(true) {
	T_B
		m_bValid = false;
...
bool CMapRes::Init() {
	...
	if (!g_MapID.GetID(m_strMapName.c_str(), m_byMapID)) {
		...
		// MessageBox(nullptr, szData, "错误", IDOK );
		MessageBox(nullptr, szData, RES_STRING(GM_MAPRES_CPP_00002), IDOK);
		return false;
	} else {
	...
	if (!m_pCMonsterSpawn->Init(m_szMonsterSpawnFile, 200)) {
		...
		throw std::runtime_error(RES_STRING(GM_MAPRES_CPP_00003));
	}
	...
	if (!m_pCMapSwitchEntitySpawn->Init(m_szMapSwitchFile, 100)) {
		...
		throw std::runtime_error(RES_STRING(GM_MAPRES_CPP_00004));
	}
	m_pNpcSpawn = new CNpcSpawn();
	if (!m_pNpcSpawn->Init(m_szNpcSpawnFile, 200)) {
		// THROW_EXCP( excpMem, "装载地图怪物NPC件错误!" );
		throw std::runtime_error(RES_STRING(GM_MAPRES_CPP_00005));
	}

	if (!InitCtrl())
		// THROW_EXCP( excpMem, "初始化地图配置错误!" );
		throw std::runtime_error(RES_STRING(GM_MAPRES_CPP_00006));
...
SubMap* CMapRes::GetCopy(dbc::Short sCpyNO) {
	if (sCpyNO < 0)
		return m_pCMapCopy;
	if (sCpyNO >= m_sMapCpyNum)
		return 0;
	return m_pCMapCopy + sCpyNO;
}

BOOL CMapRes::SummonNpc(USHORT sAreaID, const char szNpc[], USHORT sTime) {
	T_B return m_pNpcSpawn->SummonNpc(szNpc, sAreaID, sTime);
	T_E
}

// 根据地图控制脚本初始化一些控制信息
bool CMapRes::InitCtrl(void) {
	T_B if (g_IsFileExist(GetResPath(m_szCtrlFile)))
	...
	if (!m_pCMapCopy)
		...
		throw std::runtime_error(RES_STRING(GM_MAPRES_CPP_00007));
...
bool CMapRes::CreateEntry(void) {
	T_B if (!m_pfEntryFile) return false;
	...
	while (!feof(m_pfEntryFile)) {
		...
		if ((pszPos = strstr(szLine, "--")) != nullptr)
			*pszPos = '\0';
		if (!strcmp(szLine, ""))
			continue;
		if ((pszPos = strstr(szLine, "\x0a")) != nullptr)
			*pszPos = ' ';
		if ((pszPos = strstr(szLine, "\x0d")) != nullptr)
			*pszPos = ' ';
...
bool CMapRes::SubEntryPlayer(dbc::Short sCopyNO) {
	T_B if (!strcmp(m_szEntryMapName, "")) return true;
...
bool CMapRes::Open(void) {
	T_B if (m_chState == enumMAP_STATE_OPEN) return true;
...
bool CMapRes::Close(void) {
	T_B if (m_chState == enumMAP_STATE_CLOSE) {
		if (g_cchLogMapEntry)
			// LG("地图入口流程", "已经关闭地图：%s\n", GetName());
...
void CMapRes::Run(DWORD dwCurTime) {
	T_B if (!m_timeRun.IsOK(dwCurTime)) return;
...
bool CMapRes::OpenEntry(void) {
	T_B if (m_chEntryState == enumMAPENTRY_STATE_OPEN) return true;
...
bool CMapRes::CloseEntry(void) {
	T_B if (m_chEntryState == enumMAPENTRY_STATE_ASK_OPEN) return false;
...
bool CMapRes::CopyClose(dbc::Short sCopyNO) {
	T_B if (sCopyNO >= GetCopyNum()) return false;
...
bool CMapRes::ReleaseCopy(dbc::Short sCopyNO) {
	T_B return SubEntryCopy(sCopyNO);
	T_E
}

void CMapRes::CheckEntryState(dbc::Char chState) {
	T_B if (chState == enumMAPENTRY_STATE_OPEN) {
...
SubMap* CMapRes::GetNextUsedCopy(void) {
	T_B if (!m_pCMapCopy) return nullptr;
...
Long CAreaData::Init(_TCHAR* chFile) {
	T_B if ((m_nID = s_openAttribFile(chFile)) == -1) return 0;
...
BOOL CMapID::AddInfo(const char szMap[], BYTE byID) {
	T_B if (m_byNumMap >= MAX_MAP) return FALSE;
...
BOOL CMapID::GetID(const char szMap[], BYTE& byID) {
	T_B for (BYTE b = 0; b < m_byNumMap; b++) {
