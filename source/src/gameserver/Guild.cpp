...
BOOL Guild::lua_CreateGuild(CCharacter* pCha) // 创建公会,1-海军,2-海盗
{
...
void Guild::cmd_CreateGuild(CCharacter* pCha, bool confirm, cChar* guildname, cChar* passwd) {
	...
	char l_guildname[32];
	strcpy_s(l_guildname, sizeof(l_guildname), guildname);
	int l_guildid = game_db.CreateGuild(pCha, l_guildname, passwd);
	if (!l_guildid) {
		return;
	}
	...
	char l_str[512];
	_snprintf_s(l_str, sizeof(l_str), _TRUNCATE, "Player [%s] has founded [%s] guild. All Pirates may now apply!",
			pCha->GetName(),
			l_guildname);
...
void Guild::cmd_ListAllGuild(CCharacter* pCha) // 调用一次返回20行，由客户端命令分次调用，直到返回所有行
{
