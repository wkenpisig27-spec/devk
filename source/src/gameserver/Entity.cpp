...
void Entity::ReadPK(RPACKET& rpk) // 重构玩家本身及其所有附加结构(如召唤兽等)
{
	...
	// Fixed: Potential buffer overflow - use strncpy with size limit
	const char* name = READ_STRING(rpk);
	strncpy_s(m_name, sizeof(m_name), name, _TRUNCATE);

	Char szLogName[1024];
	const char* logName = READ_STRING(rpk);
	strncpy_s(szLogName, sizeof(szLogName), logName, _TRUNCATE);
...
void Entity::ActiveEyeshot(bool bActive) {
	T_B if (m_bActiveEyeshot == bActive) return;
	if (!m_submap)
		return;
...
bool Entity::overlap(int& xdist, int& ydist) {
	bool b_retval = false;
...
bool Entity::EdgeOverlap(int& xdist, int& ydist) {
	T_B bool l_retval = false;

	// 备份下来作为一个移动方向的记录
	const int lc_xdist = xdist;
	const int lc_ydist = ydist;
	// 检查是否越出地图
	...
	int l_xdlt = (l_shape.centre.x - l_shape.radius) - area.ltop.x;
	if (l_xdlt < 0) {
		l_flag = true;
		xdist -= l_xdlt;
	} else {
	...
	int l_ydlt = (l_shape.centre.y - l_shape.radius) - area.ltop.y;
	if (l_ydlt < 0) {
		l_flag = true;
		ydist -= l_ydlt;
	} else {
...
// bool Entity::ObstacleOverlap(int &xdist, int &ydist)
//{
