...
//=============================================================================
// Security: Validate item ID before broadcasting to prevent client crashes
// Returns true if item ID is valid, false otherwise (and logs security warning)
//=============================================================================
static bool ValidateEquipmentItemID(Short itemID, int slotIndex, const char* chaName) {
	// Item ID 0 is valid (empty slot)
	if (itemID == 0) {
		return true;
	}
	
	// Special reserved IDs for dual-wield and totem placeholders
	if (itemID == enumEQUIP_BOTH_HAND || itemID == enumEQUIP_TOTEM) {
		return true;
	}
	
	// Negative IDs are invalid
	if (itemID < 0) {
		LG("security", "[ALERT] Character '%s' has negative item ID %d in slot %d - possible attack attempt\n", 
			chaName ? chaName : "Unknown", itemID, slotIndex);
		return false;
	}
	
	// Check if item exists in the item database
	CItemRecord* pItemRec = GetItemRecordInfo(itemID);
	if (!pItemRec) {
		LG("security", "[ALERT] Character '%s' has invalid item ID %d in slot %d (not in ItemInfo) - possible attack attempt\n", 
			chaName ? chaName : "Unknown", itemID, slotIndex);
		return false;
	}
...
void CCharacter::WriteLookData(WPACKET& WtPk, Char chLookType, Char chSynType) {
	...
	if (m_CChaAttr.GetAttr(ATTR_CHATYPE) == enumCHACTRL_PLAYER && IsBoat()) {
		...
		// Security validation: Check hair ID before broadcasting
		Short safeHairID = m_SChaPart.sHairID;
		if (safeHairID != 0 && !ValidateEquipmentItemID(safeHairID, -1, GetName())) {
			// Invalid hair ID - use default hair (ID 0) or get from character record
			LG("security", "[ALERT] Character '%s' has invalid hair ID %d - resetting to default\n", GetName(), safeHairID);
			safeHairID = 0; // Reset to default - client will use CharacterRecord default
		}
		WRITE_SHORT(WtPk, safeHairID);
		SItemGrid* pItem;
		...
		for (int i = 0; i < nItemCnt; i++) {
			...
			// Security validation: Check item ID before broadcasting to other clients
			Short safeItemID = pItem->sID;
			if (!ValidateEquipmentItemID(pItem->sID, i, GetName())) {
				// Invalid item ID detected - replace with 0 to prevent client crashes
				safeItemID = 0;
			}
			
			WRITE_SHORT(WtPk, safeItemID);
			WRITE_LONG(WtPk, pItem->dwDBID);
			WRITE_SHORT(WtPk, pItem->sNeedLv);
			if (safeItemID == 0)
				continue;
...
bool CCharacter::WriteAppendLook(CKitbag& CKb, WPACKET& pk, bool bInit) {
	...
	for (int i = 0; i < defESPE_KBGRID_NUM; i++) {
		...
		// Security validation: Check item ID before broadcasting
		Short safeItemID = pGridCont->sID;
		if (!ValidateEquipmentItemID(pGridCont->sID, i, GetName())) {
			safeItemID = 0;
		}
		
		WRITE_SHORT(pk, safeItemID);
