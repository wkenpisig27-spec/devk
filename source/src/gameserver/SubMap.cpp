...
bool SubMap::Init(CMapRes* pCMapRes, dbc::Short sCopyNO) {
	...
	if (!m_pCStateCell)
		...
		throw std::runtime_error(RES_STRING(GM_SUBMAP_CPP_00001));
	for (short i = 0; i < GetStateCellLin(); i++) {
	...
	if (!m_pCEyeshotCell)
		...
		throw std::runtime_error(RES_STRING(GM_SUBMAP_CPP_00002));
...
//=============================================================================
CCharacter* SubMap::ChaSpawn(Long lChaInfoID, Char chCtrlType, Short sAngle, Point* pSPos, bool bEyeshotAbility, dbc::cChar* cszChaName, const int clSearchRadius) {
	T_B
		CCharacter* pCCha;
	...
	// BossTimer: Check if spawn is allowed for tracked bosses
	if (!BossTimer::CanSpawn(static_cast<unsigned int>(lChaInfoID), GetName(),
		pSPos->x, pSPos->y, sAngle))
	{
		// Boss is still on respawn cooldown - block spawn
		return 0;
	}
	...
	if (!cszChaName)
		strcpy_s(pCCha->m_name, sizeof(pCCha->m_name), pCCha->m_pCChaRecord->szName);
	else
		strcpy_s(pCCha->m_name, sizeof(pCCha->m_name), cszChaName);
...
BOOL SubMap::LoadNpc() {
	T_B if (m_pCMapRes->m_pNpcSpawn->Load(*this) == -1) {
		...
		throw std::runtime_error(RES_STRING(GM_SUBMAP_CPP_00003));
		return FALSE;
	}
...
void SubMap::Add(Entity* pCEnt) {
	...
	if (pCCha) {
		if (pCCha->IsPlayerCha()) {
			...
			// [FIX 4] Enable guild PK for SideID-based and CTF maps so guild members
			// can attack each other without needing the Ctrl key
			short mapType = GetMapRes()->GetType();
			pCCha->Cmd_SetPKGuild(mapType == 3 || mapType == 5 || mapType == 17);
		}
	...
	if (pCCha) {
		...
		for (int i = l_range.ltop.y; i <= l_range.rbtm.y; i++)
			for (int j = l_range.ltop.x; j <= l_range.rbtm.x; j++) {
...
bool SubMap::Enter(Square* pSEntShape, Entity* ent, cLong clSearchRadius) {
		...
		if (ent->overlap(l_xdlt, l_ydlt)) {
			...
			while (l_overflag) // 直到确定位置为止
			{
				...
				for (short dir = 0; dir < 4; dir++) {
					l_pt1 = l_pt;
					// int l_radius	=2 * lStep * l_dist;
					int l_radius = GetBlockCellWidth() * l_dist;
	...
	if (l_retval) {
		...
		for (int y = l_rect.ltop.y; y <= l_rect.rbtm.y; y++) {
			for (int x = l_rect.ltop.x; x <= l_rect.rbtm.x; x++) {
				if (bActive)
...
void SubMap::GoOut(Entity* ent) {
	...
	if (!ent) {
		LG("SubMap", "GoOut called with null entity\n");
		return;
	}
	
	// Check if entity is still valid and has a shape
	Point l_pt;
	try {
		l_pt = ent->GetShape().centre;
	} catch (...) {
		LG("SubMap", "GoOut: Exception getting entity shape for %s\n", ent->GetLogName());
		return;
	}
	...
	// Bounds check before accessing eyeshot cells
	int maxY = GetEyeshotCellLin() - 1;
	int maxX = GetEyeshotCellCol() - 1;
	
	for (int y = l_rect.ltop.y; y <= l_rect.rbtm.y && y <= maxY; y++) {
		for (int x = l_rect.ltop.x; x <= l_rect.rbtm.x && x <= maxX; x++) {
			if (y >= 0 && x >= 0 && m_pCEyeshotCell) {
				if (bActive)
...
Rect SubMap::GetEyeshot(Point& pt) const {
	...
	l_rect.ltop.x = std::max<decltype(pt.x)>(pt.x - GetEyeshotWidth(), 0);
	l_rect.ltop.y = std::max<decltype(pt.y)>(pt.y - GetEyeshotWidth(), 0);
	l_rect.rbtm.x = std::min<decltype(pt.x)>(pt.x + GetEyeshotWidth(), GetEyeshotCellCol() - 1);
	l_rect.rbtm.y = std::min<decltype(pt.y)>(pt.y + GetEyeshotWidth(), GetEyeshotCellLin() - 1);
	return l_rect;
	T_E
}
		...
		for (int j = lUnitSX; j <= lUnitEX; j++) {
			...
			if (m_pCBaseRange->RectHitTest(sx, sy, ex, ey)) // 管理单元在范围内
			{
				...
				if (m_sRangeMgrUnitNum >= defRANGE_MGRUNIU_NUM) {
					// MessageBox(nullptr, "区域搜索的范围超过设定值", "错误", MB_OK);
					MessageBox(nullptr, RES_STRING(GM_SUBMAP_CPP_00004), RES_STRING(GM_SUBMAP_CPP_00005), MB_OK);
					return;
				}
			}
		}
...
void SubMap::MoveTo(Entity* pCEnt, const Point& STar) {
	...
	if (l_src != l_dst) // 视角不重叠
	{
		...
		////beginsee and endsee
		// int	lXDist, lYDist, lXStep, lYStep, lSrcXStart, lTarXStart, lSrcYStart, lTarYStart;
		...
		//	for (int x = 0; x < lXDist; x++)
		//	{
		//		lXStep = lSrcXStart + x;
		//		if ((lXStep >= 0) && (lXStep <= GetEyeshotCellCol() - 1))
		//		{
		//			for (int y = l_srcEyeshot.ltop.y; y <= l_srcEyeshot.rbtm.y; y++)
		//			{
		//				if (bActive)
		...
		//		if ((lXStep >= 0) && (lXStep <= GetEyeshotCellCol() - 1))
		//		{
		//			for (int y = l_dstEyeshot.ltop.y; y <= l_dstEyeshot.rbtm.y; y++)
		//			{
		//				if (bActive)
		...
		const int l_xmin = min(l_srcEyeshot.ltop.x, l_dstEyeshot.ltop.x);
		const int l_xmax = max(l_srcEyeshot.rbtm.x, l_dstEyeshot.rbtm.x);
		const int l_ymin = min(l_srcEyeshot.ltop.y, l_dstEyeshot.ltop.y);
		const int l_ymax = max(l_srcEyeshot.rbtm.y, l_dstEyeshot.rbtm.y);
		for (int y = l_ymin; y <= l_ymax; y++) {
			for (int x = l_xmin; x <= l_xmax; x++) {
				const int l_srcx = abs(x - l_src.x);
				const int l_srcy = abs(y - l_src.y);
				const int l_dstx = abs(x - l_dst.x);
				const int l_dsty = abs(y - l_dst.y);
...
void COutMapCha::Run(unsigned int ulCurTick) {
	T_B

		int lTickDist = ulCurTick - m_ulTick;
					   ...
					   [&](std::unique_ptr<SMgrUnit>& pSCarrier) {
						   ...
						   case 2: {
							   ...
							   if (pSCarrier->lLeftTick2 <= 0) {
								   // BossTimer: Check if this is a tracked boss still on cooldown
								   // If so, keep it in the queue and retry later instead of respawning
								   CCharacter* pBossCha = pSCarrier->pCCha;
								   if (pBossCha && !pBossCha->IsPlayerCha())
								   {
									   unsigned int bossChaId = static_cast<unsigned int>(pBossCha->GetCat());
									   if (BossTimer::IsOnCooldown(bossChaId))
									   {
										   // Still on cooldown - keep in queue, retry later
										   return false;
									   }
								   }
								   ExecTimeCha(pSCarrier.get());
								   return true;  // Remove from queue after step 2 completes
							   }
						   } break;
						   default:
							   // Step 3+ means processing is complete, remove from queue
							   return true;
						   }
...
void COutMapCha::ExecTimeCha(SMgrUnit* pChaInfo) {
	T_B if (pChaInfo->chStep == 1) {
		...
		case enumCHA_TIMEER_ENTERMAP: {
			...
			// Temporary monsters (with LifeTime set) should be freed, not respawned
			if (pCCha->GetLifeTime() != 0) {
				pCCha->SetInOutMapQueue(false);
				pCCha->Free();  // Return to entity pool
				break;
			}
