...
// 总循环入口
void CCharacter::Run(uLong dwCurTime) {
	T_B
		MPTimer t;
	Char chCount = 0;
	...
	if(IsPlayerCha() && ((!(GetAreaAttr() & enumAREA_TYPE_NOT_FIGHT)) || IsBoat()) && !GetPlyCtrlCha()->GetSubMap()->GetMapRes()->CanPK())
	{
	...
	// 定时器检查
	t.Begin();
	if (m_timerAI.IsOK(dwCurTime))
		OnAI(dwCurTime);
	m_dwCellRunTime[chCount++] = t.End();
	t.Begin();
	if (m_timerAreaCheck.IsOK(dwCurTime))
		OnAreaCheck(dwCurTime);
	m_dwCellRunTime[chCount++] = t.End();
	t.Begin();
	if (m_timerDie.IsOK(dwCurTime))
		OnDie(dwCurTime);
	m_dwCellRunTime[chCount++] = t.End();
	t.Begin();
	if (m_timerMission.IsOK(dwCurTime))
		OnMissionTime();
	m_dwCellRunTime[chCount++] = t.End();
	t.Begin();
	if (m_timerTeam.IsOK(dwCurTime))
		OnTeamNotice(dwCurTime);
	m_dwCellRunTime[chCount++] = t.End();
...
void CCharacter::RunEnd(DWORD dwCurTime) {
	T_B if (m_byExit == CHAEXIT_BEGIN && m_timerExit.IsOK(dwCurTime)) {
		// 正式退出
		Exit();
	}
	T_E
...
void CCharacter::StartExit() {
	T_B
		...
		LG("time too int exit", "StartExit: Name = %s,exitcode = %d\n", this->GetName(), m_byExit);
...
void CCharacter::CancelExit() {
	T_B
		...
		LG("time too int exit", "CancelExit: Name = %s,exitcode = %d\n", this->GetName(), m_byExit);
	if (m_byExit == CHAEXIT_BEGIN) {
...
void CCharacter::Exit() {
	T_B
		...
		LG("time too int exit", "Exit: Name = %s, exitcode = %d\n", this->GetName(), m_byExit);
	WPACKET l_wpk = GETWPACKET();
...
void CCharacter::OnScriptTimer(DWORD dwExecTime, bool bNotice) {
	T_B if (!IsPlayerCha()) return;

	Long lOldHP = (int)getAttr(ATTR_HP);
