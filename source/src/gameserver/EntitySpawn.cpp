...
bool CChaSpawn::Init(char* szSpawnTable, int lRegionNum) {
	m_pCMap = 0;

	if (lRegionNum <= 0)
		// THROW_EXCP(excpArr, "刷怪的区域数目出错");
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00001));
	...
	if (!m_pCMonRefRecordSet)
		...
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00002));
	...
	if (!m_pSMonInfo)
		...
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00003));
...
int CChaSpawn::Load(SubMap* pCMap) {
	T_B
		m_pCMap = pCMap;
		...
		for (int j = 0; j < defMAX_REGION_MONSTER_TYPE; j++) {
			for (int k = 0; k < pMonRefRecord->lMonster[j][1]; k++) {
				...
				if (pCCha = pCMap->ChaSpawn(pMonRefRecord->lMonster[j][0], enumCHACTRL_NONE, sAngle, &l_pos)) {
					...
					// If this is a tracked boss blocked by BossTimer, save its resume time
					// so the deferred spawn can set it correctly later
					if (BossTimer::IsTrackedBoss(static_cast<unsigned int>(pMonRefRecord->lMonster[j][0])))
					{
						BossTimer::SetPendingResumeTime(
							static_cast<unsigned int>(pMonRefRecord->lMonster[j][0]),
							pMonRefRecord->lMonster[j][3] * 1000);
					}
					else
					{
...
bool CMapSwitchEntitySpawn::Init(char* szSpawnTable, int lRecordNum) {
	m_pCMap = 0;

	if (lRecordNum <= 0)
		// THROW_EXCP(excpArr, "记录数目出错");
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00005));
	strcpy(m_szSpawnTable, szSpawnTable);
	...
	if (!m_pCSwitchMapRecSet)
		...
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00006));
...
int CMapSwitchEntitySpawn::Load(SubMap* pCMap) {
	m_pCMap = pCMap;
...
int CMapSwitchEntitySpawn::Reload() {
	return 0;
}
...
bool CNpcSpawn::Init(char* szSpawnTable, int lRecordNum) {
	if (lRecordNum <= 0 || szSpawnTable == nullptr) {
		char szTemp[128];
	...
	if (!m_pNpcRecordSet) {
		...
		throw std::runtime_error(RES_STRING(GM_ENTITYSPAWN_CPP_00008));
	}
...
int CNpcSpawn::Load(SubMap& submap) {
	// 初始化该地图装载的NPC指针列表
	...
	for (int i = 0; i < m_lRecordNum; i++) {
		...
		case mission::CNpc::TALK: {
			...
			//
			Square SShape = {{static_cast<int>(pNpcRecord->dwxPos0), static_cast<int>(pNpcRecord->dwyPos0)}, pCharRecord->sRadii};
