...
// External declarations for graceful shutdown
extern volatile BOOL g_bGameEnd;
extern volatile BOOL g_bShutdownInitiated;
extern void InitiateGracefulShutdown(DWORD dwSecondsCountdown);

// Map view coordinates (needed by lua_gamectrl even without WIN32_DLG)
int g_lViewAtMapX = 0;
int g_lViewAtMapY = 0;
...
HWND g_ReportView;
HWND g_MapView;
int g_lMouseAtMapX = 0;
int g_lMouseAtMapY = 0;
BOOL g_bShowView = 0;
...
INT_PTR CALLBACK DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_CLOSE: {
		// If shutdown already completed (g_bGameEnd is true), just quit
		if (g_bGameEnd) {
			PostQuitMessage(0);
			break;
		}
		...
		// Don't call PostQuitMessage here - let the graceful shutdown handle it
		break;
	}
	}
	return FALSE;
...
void DrawMgrUnit(HDC dc, int map_sx, int map_sy, int view_w, int view_h) {
	T_B int cnt_x = view_w / UNIT_DRAW_SIZE + 1;
...
*/
	...
	for (int i = 0; i < pMap->GetEyeshotCellLin(); i++) {
		for (int j = 0; j < pMap->GetEyeshotCellCol(); j++) {
	...
	for (int i = 0; i < pMap->GetEyeshotCellLin(); i++) {
		for (int j = 0; j < pMap->GetEyeshotCellCol(); j++) {
			...
			while (pObj) {
				...
				if (IsMouseLocation(mx / 100, my / 100))
					TextOut(dc, x, y, pObj->GetName(), (int)strlen(pObj->GetName()));
...
static int g_lLastViewAtMapX = 0;
static int g_lLastViewAtMapY = 0;
	...
	case WM_MOUSEMOVE: {
		if (g_bDragMap) {
			...
			if (g_lViewAtMapY < 0)
				g_lViewAtMapY = 0;
			if (g_bShowView)
				InvalidateRect(g_MapView, nullptr, FALSE);
		}
		break;
	}
	case WM_PAINT: {
...
void CreateMainDialog(HINSTANCE hInst, HWND hParent) {
	...
	SetWindowPos(g_MapView, nullptr, 65, 340, 0, 0, SWP_NOSIZE);
	T_E
}
...
void SystemReport(DWORD dwTimeParam) {
	...
	if (dwTimeParam - dwLastReportTime < 1000)
		return;
	...
	if (dwLastReportTime == 0) // 只执行一次
	{
		...
		if (game_db.m_bInitOK)
			SetWindowText(hDB, "ok");
		else
			SetWindowText(hDB, "fail");
	...
	if (rand() % 2 == 0) {
		...
		if (pCMap) {
			...
			if (hPKCnt)
				SetWindowText(hPKCnt, szFPS); // 显示总数
		}
	}
