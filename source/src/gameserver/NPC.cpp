...
BOOL CTalkNpc::InitScript(const char szFunc[], const char szName[]) {
	T_B
	// Null pointer checks to prevent access violations
	if (!szFunc || !szName) {
		LG("NpcInit", "InitScript called with null parameter: szFunc=%p, szName=%p\n", szFunc, szName);
		return FALSE;
	}
	...
	if (!g_pLuaState) {
		LG("NpcInit", "InitScript called before Lua initialization for NPC [%s]\n", szName);
		return FALSE;
	}
...
BOOL CTalkNpc::MissionProc(CCharacter& character, BYTE& byState) {
	T_B if (!m_bHasMission) return TRUE;
...
BOOL CTalkNpc::AddNpcTrigger(WORD wID, mission::TRIGGER_EVENT e, WORD wParam1, WORD wParam2, WORD wParam3, WORD wParam4) {
	T_B if (m_byNumTrigger >= ROLE_MAXNUM_NPCTRIGGER) return FALSE;
...
void CTalkNpc::ClearTrigger(WORD wIndex) {
	T_B if (wIndex >= m_byNumTrigger) return;
