// StackWalker.cpp — Windows-only (DbgHelp symbol walking)
#include "stdafx.h"
#ifdef _WIN32
...
#pragma pack(pop)
	...
	BOOL GetModuleListPSAPI(HANDLE hProcess) {
		...
		if (!pEPM(hProcess, hMods, TTBUFLEN, &cbNeeded)) {
			//_ftprintf(fLogFile, _T("%u: EPM failed, GetLastError = %u\n"), g_dwShowCount, gle );
			goto cleanup;
		}

		if (cbNeeded > TTBUFLEN) {
			//_ftprintf(fLogFile, _T("%u: More than %u module handles. Huh?\n"), g_dwShowCount, lenof( hMods ) );
			goto cleanup;
		}
		...
		if (s.AddrPC.Offset != 0) {
			...
			if (this->m_sw->GetModuleInfo(this->m_hProcess, s.AddrPC.Offset, &Module) != FALSE) { // got module info OK
				...
				case 8: // SymVirtual:
					...
					break;
				default:
					//_snprintf( ty, sizeof ty, "symtype=%d", (int) Module.SymType );
