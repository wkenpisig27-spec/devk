...
// IOCP is Windows-only. On Linux, USE_IOCP is not defined and select()-based path is used.
#ifdef _WIN32
...
// I/O Operation word
enum IO_OPERATION { CLIENT_IO_ACCEPT,
					CLIENT_IO_READ,
					CLIENT_IO_WRITE,
					CLIENT_IO_CONNECTED };

// special interface
bool PER_SOCKET_CONTEXT::send(char const* buf, int buf_len) { return app->send(this, buf, buf_len); }

// iocp app base class
bool cfl_iocpapp::startup() {
	// init the winsock
	WSADATA wd;
	if (WSAStartup(0x202, &wd) != 0)
		return false;
	else
		return true;
}
...
// iocp client
cfl_iocpclt::cfl_iocpclt(int maxconn) try : skpool(maxconn), iopool(100), ctxlist(nullptr), ctxlist_lock(new cfl_spinlock),
	...
	SYSTEM_INFO si;
	GetSystemInfo(&si);
	thread_count = std::min<decltype(thread_count)>(MAX_WORKER_THREAD, si.dwNumberOfProcessors * 2 + 2);
	...
	DWORD dwThreadId;
	mgr_thrd_handle = CreateThread(nullptr, 0, mgr_thrd, this, 0, &dwThreadId);

	HANDLE handle;
	for (DWORD i = 0; i < thread_count; ++i) {
		handle = CreateThread(nullptr, 0, wrk_thrd, this, 0, &dwThreadId);
...
PER_SOCKET_CONTEXT* cfl_iocpclt::connect(char const* ipaddr, unsigned short port) {
	...
	s = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_IP, nullptr, 0, WSA_FLAG_OVERLAPPED);
	if (s == INVALID_SOCKET) {
...
DWORD WINAPI cfl_iocpclt::wrk_thrd(LPVOID wrk_thrd_ctx) {
	...
	PER_SOCKET_CONTEXT* sk_ctx = nullptr;
	PER_IO_CONTEXT* io_ctx = nullptr;
	...
	while (true) {
		...
		if (sk_ctx == nullptr) {
			cfl_printf("wrk_thrd: sk_ctx = nullptr \n");
			continue;
		}
		...
		case CLIENT_IO_WRITE:
			...
			if (io_ctx->sent_bytes < io_ctx->total_bytes) {
				...
				nRet = WSASend(sk_ctx->sk, &buffSend, 1, &send_bytes,
							   flags, &(io_ctx->overlapped), nullptr);
...
int cfl_iocpclt::on_connect(PER_SOCKET_CONTEXT* sk_ctx) { return 0; }

int cfl_iocpclt::on_recvdat(PER_SOCKET_CONTEXT* sk_ctx, DWORD io_size) { return 0; }

int cfl_iocpclt::on_sendfsh(PER_SOCKET_CONTEXT* sk_ctx, DWORD io_size) { return 0; }

int cfl_iocpclt::on_disconn(PER_SOCKET_CONTEXT* sk_ctx) { return 0; }
...
bool cfl_iocpclt::send(PER_SOCKET_CONTEXT* sk_ctx, char const* buf, int buf_len) {
	if (!sk_ctx->connected || (buf == nullptr) || (buf_len > MAX_BUFF_SIZE)) {
		cfl_printf("socket not connect or buffer is NULL or length too int: [%dBytes]\n", buf_len);
		return false;
	}
...
void cfl_iocpclt::close_client(PER_SOCKET_CONTEXT* sk_ctx, bool graceful) {
	...
	try {
		if (sk_ctx == nullptr) {
			cfl_printf("close_client: sk_ctx is NULL\n");
			return;
		}
		if (_verbose) {
		...
		if (!graceful) { // force the subsequent closesocket to be abortative.
			LINGER lingerStruct;
		...
		LG("iocp_error", "Exception in close_client()\n");
	}
	ctxlist_lock->unlock();
}
...
PER_SOCKET_CONTEXT* cfl_iocpclt::ctxlist_alloc(SOCKET sk, IO_OPERATION client_io) {
	...
	try {
		...
		LG("iocp_error", "Exception in ctxlist_alloc()\n");
	}
	ctxlist_lock->unlock();
...
void cfl_iocpclt::ctxlist_add(PER_SOCKET_CONTEXT* sk_ctx) {
	...
	try {
		...
		LG("iocp_error", "Exception in ctxlist_add()\n");
	}
	ctxlist_lock->unlock();
}
...
void cfl_iocpclt::ctxlist_del(PER_SOCKET_CONTEXT* sk_ctx) {
	...
	try {
		...
		if (pBack == nullptr && pForward == nullptr) { // This is the only node in the list to delete
			ctxlist = nullptr;
		} else if (pBack == nullptr && pForward != nullptr) { // This is the end node in the list to delete
			pForward->back = nullptr;
		} else if (pBack != nullptr && pForward == nullptr) { // This is the start node in the list to delete
			pBack->forward = nullptr;
			ctxlist = pBack;
		} else if (pBack && pForward) { // Neither start node nor end node in the list
		...
		LG("iocp_error", "Exception in ctxlist_del()\n");
	}
	ctxlist_lock->unlock();
}
...
void cfl_iocpclt::ctxlist_free() {
	...
	try {
		...
		LG("iocp_error", "Exception in ctxlist_free()\n");
	}
	ctxlist_lock->unlock();
}
