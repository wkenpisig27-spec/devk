...
void CGameConfig::SetDefault() {
	...
	m_lWeather = 120;
	m_dwStallTime = 48;
	m_bOfflineStall = FALSE;  // Disabled by default
	m_cSaveState[32] = {0};
	m_bFastMapSwitch = TRUE;  // Enable fast map switching by default (single GameServer mode)
}

bool CGameConfig::Load(char* pszFileName) {
	// Pre-read log_dir from config FIRST - before any LG() calls
	{
		ifstream preread(pszFileName);
		if (preread.is_open()) {
			string line;
			char szLine[255];
			while (!preread.eof()) {
				preread.getline(szLine, 255);
				line = szLine;
				...
				if (eqPos != std::string::npos) {
					...
					if (key == "log_dir" && !value.empty()) {
						strcpy(m_szLogDir, value.c_str());
						LG_SetDirWithTimestamp(m_szLogDir);
						break;
					}
				}
			}
	...
	while (!in.eof()) {
		...
		} else if (strKey == "info") // 解析InfoServer IP和port
		{
			...
			size_t stPos = 0;
			if ((stPos = strValue.find("0x")) != std::string::npos || (stPos = strValue.find("0X")) != std::string::npos) // 十六进制值
		...
		} else if (strKey == "fast_map_switch") {
			m_bFastMapSwitch = Str2Int(strValue);
		}
	}
	in.close();
