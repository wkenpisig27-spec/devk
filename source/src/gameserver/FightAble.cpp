...
void CFightAble::BeginFight() {
	...
	int lDistX2 = (GetShape().centre.x - STarShape.centre.x) * (GetShape().centre.x - STarShape.centre.x);
	int lDistY2 = (GetShape().centre.y - STarShape.centre.y) * (GetShape().centre.y - STarShape.centre.y);
	if (lDistX2 + lDistY2 <= lReqDist * lReqDist) {
		if (m_SFightInit.pCSkillRecord->chOperate[0] == 0) // 普通技能
		{
			...
			SkillGeneral((int)sqrt((double)lDistX2 + lDistY2));
		} else {
			m_SFightProc.sState = enumFSTATE_OFF;
			NotiSkillSrcToSelf();
		}
			...
			if (lResumeDist > lResumeT) {
				...
				if (m_SFightInit.pCSkillRecord->chOperate[0] == 0) // 普通技能
				{
					...
					for (i = 0; i < sExecTime; i++) {
						if (!SkillGeneral((int)sqrt((double)lDistX2 + lDistY2)))
							break;
					}
...
void CFightAble::SkillTarEffect(SFireUnit* pSFireSrc) {
	...
	lNowHP = (int)m_CChaAttr.GetAttr(ATTR_HP);
	BeUseSkill(lOldHP, lNowHP, pSrcCha, pSFireSrc->pCSkillRecord->chHelpful);

	// 判断是否是采矿技能
	if (m_CChaAttr.GetAttr(ATTR_CHATYPE) != enumCHACTRL_MONS_MINE && m_CChaAttr.GetAttr(ATTR_CHATYPE) != enumCHACTRL_MONS_TREE && m_CChaAttr.GetAttr(ATTR_CHATYPE) != enumCHACTRL_MONS_FISH && m_CChaAttr.GetAttr(ATTR_CHATYPE) != enumCHACTRL_MONS_DBOAT) {
		//
...
bool CFightAble::RectifyAttr() {
	bool bRectify = false;
	Long lMaxHP = (int)m_CChaAttr.GetAttr(ATTR_MXHP);
	if (m_CChaAttr.GetAttr(ATTR_HP) > lMaxHP) {
		bRectify = true;
		m_CChaAttr.ResetChangeFlag();
		setAttr(ATTR_HP, lMaxHP);
	}
	Long lMaxSP = (int)m_CChaAttr.GetAttr(ATTR_MXSP);
...
Long CFightAble::setAttr(int nIdx, LONG64 lValue, int nType) {
	if (nIdx == ATTR_GD && lValue < 0)
		return m_CChaAttr.SetAttr(nIdx, 0);

	LONG64 lOldVal = m_CChaAttr.GetAttr(nIdx);
...
Long CFightAble::GetSkillTime(CSkillTempData* pCSkillTData) {
	if (pCSkillTData->lResumeTime == 0) // 天生技能
		return (int)m_CChaAttr.GetAttr(ATTR_ASPD);
	else
	...
	if (sAttrChangeNum > 0) {
		for (int i = 0; i < ATTR_CLIENT_MAX; i++) {
			if (m_CChaAttr.GetChangeBitFlag(i)) // 如果该属性有变，则压包
			{
				...
				WRITE_CHAR(pk, i);
				if (i == ATTR_NLEXP || i == ATTR_CLEXP || i == ATTR_CEXP || i == ATTR_GD)
					WRITE_LONGLONG(pk, m_CChaAttr.GetAttr(i));
				else
					WRITE_LONG(pk, (unsigned int)m_CChaAttr.GetAttr(i));
				sTempChangeNum++;
			}
		}
	}
...
void CFightAble::SynAttr(Short sType) {
	T_B short sAttrChangeNum = m_CChaAttr.GetChangeNumClient();
	if (sAttrChangeNum == 0)
		return;
	...
	if (sAttrChangeNum > 0) {
		for (int i = 0; i < ATTR_CLIENT_MAX; i++) {
			if (m_CChaAttr.GetChangeBitFlag(i)) // 如果该属性有变，则压包
			{
				...
				{
					WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(i));
				}
			}
		}
	}
...
void CFightAble::WriteMonsAttr(WPACKET& pk, Short sSynType) {
	...
	WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(ATTR_LV));
	WRITE_CHAR(pk, ATTR_HP);
	WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(ATTR_HP));
	WRITE_CHAR(pk, ATTR_MXHP);
	WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(ATTR_MXHP));
	WRITE_CHAR(pk, ATTR_ASPD);
	WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(ATTR_ASPD));
	WRITE_CHAR(pk, ATTR_MSPD);
	WRITE_LONG(pk, (int)m_CChaAttr.GetAttr(ATTR_MSPD));
}
...
bool CFightAble::SkillInBoat(CCharacter* pCBoat) // 上船
{
	...
	if (GetSubMap()) {
		...
		m_pCPlayer->SetLoginCha(enumLOGIN_CHA_BOAT, (int)pCBoat->getAttr(ATTR_BOAT_DBID));
		pCBoat->SynPKCtrl();
	}
...
bool CFightAble::IsRightSkill(CSkillRecord* pSkill) {
	T_B if (IsCharacter()->IsPlayerCha()) {
...
bool CFightAble::IsRightSkillSrc(Char chSkillEffType) {
	T_B if ((GetAreaAttr() & enumAREA_TYPE_NOT_FIGHT) && (chSkillEffType != enumSKILL_EFF_HELPFUL)) // 角色在非战斗区域且非有益技能
		return false;
	else return true;
	T_E
}
...
bool CFightAble::IsRightSkillTar(CFightAble* pSkillSrc, Char chSkillObjType, Char chSkillObjHabitat, Char chSkillEffType, bool bIncHider) {
	T_B
		...
		//	return false;
		if (!bIncHider) if (IsCharacter()->IsHide()) return false;
	...
	int nCheckRet = g_IsRightSkillTar((int)m_CChaAttr.GetAttr(ATTR_CHATYPE), !IsLiveing(), IsCharacter()->GetActControl(enumACTCONTROL_BEUSE_SKILL), GetAreaAttr(),
									  (int)pSkillSrc->m_CChaAttr.GetAttr(ATTR_CHATYPE), chSkillObjType, chSkillObjHabitat, chSkillEffType, bIsTeammate, bIsFriend, pSkillSrc == this);
...
void CFightAble::CountLifeLevel() {
	T_B if (!IsLiveing()) return;
	Long lOldLevel, lCurLevel;
	LONG64 lCurExp = m_CChaAttr.GetAttr(ATTR_CLIFEEXP);

	lOldLevel = lCurLevel = (int)m_CChaAttr.GetAttr(ATTR_LIFELV);
...
void CFightAble::AddExp(LONG64 ulAddExp) {
...
bool CFightAble::AddExpAndNotic(LONG64 lAddExp, Short sNotiType) {
	T_B
		m_CChaAttr.ResetChangeFlag();
	AddExp(lAddExp);
	SynAttr(sNotiType);
...
void CFightAble::ItemCount(CCharacter* pAtk) {
	...
	// Boss last-hit drop: For bosses in BossLastHit.txt, the killer (last hitter) gets drop ownership
	bool bBossLastHitDrop = false;
	if (pThis && !pThis->IsPlayerCha())
	{
		unsigned int uBossChaId = static_cast<unsigned int>(pThis->GetCat());
		if (BossLastHit::IsLastHitBoss(uBossChaId))
		{
			// Override item host to be the killer (last hitter), not the first attacker
			pCItemHCha = pAtk;
			bBossLastHitDrop = true;
			LG("BossLastHit", "Boss %u last-hit drop assigned to killer: %s\n", uBossChaId, pAtk->GetLogName());
		}
	}
	...
	if (dwEndTime > 20)
		...
		LG("script_time", "script [%s]cost time too int, time = %d\n", szItemScript, dwEndTime);
	...
	if (lFallNum > lItemNum)
		...
		for (int i = 0; i < lFallNum; i++) {
			int itemID = m_pCChaRecord->lItem[lItem[i] - 1][0];
			...
			if (pCItem)
				pCItem->SetProtType(bBossLastHitDrop ? enumITEM_PROT_OWN : enumITEM_PROT_TEAM);
		}
	}
	...
	if (lFallNum > lItemNum)
		...
		for (int i = 0; i < lFallNum; i++) {
			...
			if (pCItem && bBossLastHitDrop)
				pCItem->SetProtType(enumITEM_PROT_OWN);
		}
	}
	T_E
}
...
void CFightAble::OnSkillState(DWORD dwCurTick) {
	...
	if (m_CSkillState.GetStateNum() > 0) {
		...
		while (pSStateUnit = m_CSkillState.GetNextState()) {
			...
			lOldHP = (int)m_CChaAttr.GetAttr(ATTR_HP);
			if (pSStateUnit->lOnTick > 0) // 有限持续时间
			{
				if (dwCurTick - pSStateUnit->ulStartTick > (unsigned int)pSStateUnit->lOnTick * 1000) {
			...
			BeUseSkill(lOldHP, (int)m_CChaAttr.GetAttr(ATTR_HP), pCCha, pSStateUnit->chEffType);
...
void CTimeSkillMgr::Add(SFireUnit* pSFireSrc, uLong ulLeftTick, SubMap* pCMap, Point* pStarget, Long* plRangeBParam) {
	...
	} else // 分配新的载体
	{
		...
		if (!pSCarrier) {
			...
			throw std::runtime_error(RES_STRING(GM_FIGHTALBE_CPP_00004));
		}
	}
...
void CTimeSkillMgr::Run(unsigned int ulCurTick) {
	T_B unsigned int ulTickDist = ulCurTick - m_ulTick;
