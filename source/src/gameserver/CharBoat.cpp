...
void CCharBoat::UpdateBoat(const BOAT_DATA& Data) { T_B
...
BOOL CCharBoat::Create(CCharacter& owner, USHORT sBoatID, USHORT sBerthID) {
	T_B if (owner.GetPlayer()->IsLuanchOut()) {
...
BOOL CCharBoat::MakeBoat(CCharacter& owner, RPACKET& packet) {
	...
	if (pszData) {
		...
		// 校验船名是否正确
		if (!common::conformity::character::name::is_valid(pszData, (USHORT)nLen) || !CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, pszData)) {
			pBoat->Free();
			owner.SetBoat(nullptr);
...
CCharacter* CCharBoat::SummonBoat(CCharacter& owner, USHORT sBoatID) {
	T_B
		CGameApp* pApp = owner.GetOwnerApp();
		if (!pApp) {
			LG("monsterboat_error", "SummonBoat: owner has no app context\n");
			return FALSE;
		}
		CCharacter* pBoat = pApp->GetNewCharacter();
	if (pBoat == nullptr) {
...
BOOL CCharBoat::CreateBoat(CCharacter& owner, DWORD dwBoatID, char chType) {
	T_B
	// Fix for duplicated boat deeds causing "Blue Map" crashes:
	// Check if this boat ID is already loaded for this player.
	// If it is, we simply return TRUE to bypass creating a duplicate entity.
	if (owner.GetPlayer()->GetBoat(dwBoatID)) {
		return TRUE;
	}
