...
volatile BOOL g_bGameEnd = FALSE;  // volatile for cross-thread visibility
...
CResourceBundleManage g_ResourceBundleManage("Locale.loc"); // Add by lark.li 20080304

#include <signal.h>

// Flag to prevent multiple shutdown triggers (global, used by SystemDialog.cpp)
volatile BOOL g_bShutdownInitiated = FALSE;
...
BOOL WINAPI ConsoleCtrlHandler(DWORD dwCtrlType) {
	...
	case CTRL_SHUTDOWN_EVENT:
		...
		g_bGameEnd = TRUE;
		// Return FALSE to let Windows close the console immediately after we're done
		// Returning TRUE would make Windows wait, but console I/O is blocked
		return FALSE;
...
int main(int argc, char* argv[]) {
	...
	if (!SetConsoleCtrlHandler(ConsoleCtrlHandler, TRUE)) {
		C_PRINT("Warning: Failed to set console control handler\n");
	}
#endif

	// DisableCloseButton(); // Removed - we now handle close events gracefully
...
#ifdef PKO_PLATFORM_WINDOWS
	MSG msg;
	LG("init", "Entering main message loop\n");
	while (!g_bGameEnd) {
		...
		// Check g_bGameEnd at end of every loop iteration
		if (g_bGameEnd) {
			break;
		}
	}
#else
	LG("init", "Entering main loop (Linux)\n");
	while (!g_bGameEnd) {
		SystemReport(GetTickCount());
		Sleep(50);
	}
...
#else
ThreadPool* l_proc = nullptr;
ThreadPool* l_comm = nullptr;
#endif
...
BOOL GameServer_Begin() {
	...
	DWORD dwThreadID;
	hGameT = CreateThread(nullptr, 0, g_GameLogicProcess, 0, 0, &dwThreadID);
	...
	CreateMainDialog(hInst, nullptr);
