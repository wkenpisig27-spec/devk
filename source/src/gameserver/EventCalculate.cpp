...
void CEventHandler::Event_ChaDie(CCharacter* pDead, CCharacter* pAtk) {
	...
	// Check if this is a tracked boss and record death
	if (pDead && !pDead->IsPlayerCha())
	{
		unsigned int chaId = static_cast<unsigned int>(pDead->GetCat());
		if (BossTimer::IsTrackedBoss(chaId))
		{
			const char* mapName = pDead->GetSubMap() ? pDead->GetSubMap()->GetName() : "";
			BossTimer::OnBossDeath(chaId, mapName);
		}
	}

	// 记录相关的所有角色, 包括曾经攻击过怪物的人和他们的队友
	CCharacter* pValidCha[25] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};

	int nValidCha = 0;

	CPlayer* pPlayer = pAtk->GetPlayer();
	if (pPlayer == nullptr) // 非玩家角色
	{
		MPTimer t;
		t.Begin();
		// 死亡后计算经验扣除
		...
		if (dwEndTime > 10) {
			...
			LG("script_time", "when player dead transfer experience assign script,account time too int! time = %d\n", dwEndTime);
		}
		return; // 怪物攻击玩家, 不往下执行
	}
	...
	for (int i = 0; i < nValidCha; i++) {
		...
		if (pCur != pAtk)
			pCur->GetPlyMainCha()->m_CChaAttr.ResetChangeFlag();
	}
	...
	for (int i = 0; i < nValidCha; i++) {
		CCharacter* pCur = pValidCha[i];
		if (pCur != pAtk)
			pCur->GetPlyMainCha()->SynAttr(enumATTRSYN_ATTACK);
	}
	...
	if (dwEndTime > 10) {
		...
		LG("script_time", "the flow of assign experience when bugbear dead, calculate time too int, time = %d, exp = %d, upgrade = %d, item = %d!\n", dwEndTime, tLua.GetTimeCount(), tMission.GetTimeCount(), tItem.GetTimeCount());
	}
}
