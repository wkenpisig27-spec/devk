...
unsigned int g_ulElapse;
unsigned int g_ulDist;
...
CMoveAble::CMoveAble() {
	...
	// Initialize speed hack tracker
	m_speedTracker = SSpeedHackTracker();
	T_E
}
...
// bool CMoveAble::AreaOverlap(int &xdist, int &ydist)
//{
//	if (!m_submap)
//		return false;
//
		...
		for (Short x = sUnitSX; x <= sUnitEX; x++) {
			for (Short y = sUnitSY; y <= sUnitEY; y++) {
				...
				if (!g_IsMoveAble((char)m_CChaAttr.GetAttr(ATTR_CHATYPE), m_pCChaRecord->chTerritory, usAreaAttr)) {
					...
					if (LineIntersection(v0, v1, p1, p2, FALSE) || LineIntersection(v0, v1, p2, p3, FALSE) || LineIntersection(v0, v1, p3, p4, FALSE) || LineIntersection(v0, v1, p4, p1, FALSE)) {
						xdist = 0, ydist = 0;
						return true;
					}
				}
...
bool CMoveAble::overlap(int& xdist, int& ydist) {
	T_B bool b_retval = false;
...
void CMoveAble::BeginMove(uLong ulElapse) {
	T_B if (GetPos() != m_SMoveInit.SInflexionInfo.SList[0] && IsCharacter()->IsRangePoint2(m_SMoveInit.SInflexionInfo.SList[0], 25 * 25 * 2)) // 客户端会把当前位置调整到单元中点
	{
	...
	if (m_SMoveInit.STargetInfo.chType > 0) // 需要目标判断
	{
		...
		ulDistXY2 = (SPos.x - STarShape.centre.x) * (SPos.x - STarShape.centre.x) + (SPos.y - STarShape.centre.y) * (SPos.y - STarShape.centre.y);
	...
	uLong ulAttemptDist = m_SMoveRedu.ulLeftTime * (int)m_CChaAttr.GetAttr(ATTR_MSPD) / 1000;
...
void CMoveAble::OnMove(uLong dwCurTime) {
	T_B if (!m_bOnMove || !m_submap) return;
	...
	if (m_SMoveProc.chRequestState == 1 // 终端要求停止，则约定结束点
		...
		ulAttemptDist = ulWillElapse * (int)m_CChaAttr.GetAttr(ATTR_MSPD) / 1000;
	...
	if ((int)ulCurTick - (int)m_ulHeartbeatTick - (int)m_usHeartbeatFreq < -50)
		return;
...
Char CMoveAble::AttemptMove(double dPreMoveDist, bool bNotiInflexion) {
	...
	Long lPreMoveDist = 0;
	if (!bNotiInflexion)
		lPreMoveDist = m_SMoveRedu.ulLeftTime * (int)m_CChaAttr.GetAttr(ATTR_MSPD) / 1000;
	MPTimer t;
	t.Begin();
	while (true) {
		...
		if (m_SMoveInit.STargetInfo.chType > 0) // 需要距离判断
		{
			int lDistX2 = (SPos.x - SReqShape.centre.x) * (SPos.x - SReqShape.centre.x);
			int lDistY2 = (SPos.y - SReqShape.centre.y) * (SPos.y - SReqShape.centre.y);
		...
		dLeftDist -= sqrt(double(SSrc.x - SPos.x) * (SSrc.x - SPos.x) + (SSrc.y - SPos.y) * (SSrc.y - SPos.y));
		if (dLeftDist < 1)
			break;
		SSrc = SPos;
	}
	...
	if (dwMoveTime + dwEyeMoveTime >= 60)
		...
		LG("map_time", "\t\troll[%s]move cost time too int, time = %u（thereinto position cost%u，eye shot cost%u）\n", GetLogName(), dwMoveTime + dwEyeMoveTime, dwMoveTime, dwEyeMoveTime);
...
Char CMoveAble::LinearAttemptMove(Point STar, double distance, uLong* ulElapse) {
	T_B
		uLong ulMoveSpd = (int)m_CChaAttr.GetAttr(ATTR_MSPD);
	if (ulMoveSpd == 0) {
		ulElapse = 0;
		return 2;
	}
	...
	Char l_retval = 1;
	int l_elapse = int((distance * 1000) / m_CChaAttr.GetAttr(ATTR_MSPD)); // 折算成移动distance厘米距离所需的毫秒时间。
	...
	int l_xdist = lc_xdist; // 保存x方向能移动的距离，随着计算会被修改
	int l_ydist = lc_ydist; // 保存y方向能移动的距离，随着计算会被修改
	...
	{
		l_xdist = int(sqrt((l_dist2 * l_xdist2) / l_xydist2)) * chDirX;
		l_ydist = int(sqrt((l_dist2 * l_ydist2) / l_xydist2)) * chDirY;
	}
	if (bIs45Dir) {
	...
	if (l_elapse > 0) // 检查碰撞情况
	{
		...
		if (l_lap) // 重叠
		{
			int l_x2y2 = l_xdist * l_xdist + l_ydist * l_ydist;
...
void CMoveAble::NotiMovToEyeshot() {
	...
	for (int i = 0; i < m_SMoveProc.SNoticePoint.sNum; i++) {
		...
		if (i > 0)
			lDist += (pSPoint->SList[i].x - pSPoint->SList[i - 1].x) * (pSPoint->SList[i].x - pSPoint->SList[i - 1].x) + (pSPoint->SList[i].y - pSPoint->SList[i - 1].y) * (pSPoint->SList[i].y - pSPoint->SList[i - 1].y);
	}
	m_CLog.Log("Elapse: %u\n", m_SMoveProc.ulElapse);
	m_CLog.Log("Distance: %d\n", (int)sqrt(double(lDist)));
...
bool CMoveAble::SetMoveOnInfo(SMoveInit* pSMoveI) {
	if (GetMoveState() != enumMSTATE_ON)
		return false;
	if (m_SMoveInit.STargetInfo.chType != 0 && m_SMoveInit.STargetInfo.chType == (*pSMoveI).STargetInfo.chType && m_SMoveInit.STargetInfo.lInfo1 == (*pSMoveI).STargetInfo.lInfo1 && m_SMoveInit.STargetInfo.lInfo2 == (*pSMoveI).STargetInfo.lInfo2) {
		m_SMoveInit = *pSMoveI;
		m_SMoveProc.sCurInflexion = 1;
		return true;
	}
...
Point CMoveAble::NearlyPointFromPointToLine(const Point* pSPort1, const Point* pSPort2, const Point* pSReference) {
	...
	} else // 线段存在斜率，且不为0
	{
		...
		// 找最近点
		if (SNearlyPoint.x < lMinX || SNearlyPoint.x > lMaxX || SNearlyPoint.y < lMinY && SNearlyPoint.y > lMaxY) // 垂足不在线段上，则取距离垂足最近的端点
		{
			if (double(SNearlyPoint.x - pSPort1->x) * double(SNearlyPoint.x - pSPort1->x) + double(SNearlyPoint.y - pSPort1->y) * double(SNearlyPoint.y - pSPort1->y) < double(SNearlyPoint.x - pSPort2->x) * double(SNearlyPoint.x - pSPort2->x) + double(SNearlyPoint.y - pSPort2->y) * double(SNearlyPoint.y - pSPort2->y))
...
void CMoveAble::LogSpeedViolation(double actualDist, double expectedDist, uLong elapsedTime) {
	...
	if (currentTick - m_speedTracker.ulLastViolationTick > SPEED_HACK_VIOLATION_DECAY) {
		m_speedTracker.sViolationCount = 0;
	}
	
	// Check if this is a suspicious movement (over threshold)
	if (ratio > SPEED_HACK_LOG_THRESHOLD) {
		m_speedTracker.sViolationCount++;
		m_speedTracker.sTotalViolations++;
		m_speedTracker.ulLastViolationTick = currentTick;
		
		// Track worst ratio
		if (ratio > m_speedTracker.fWorstRatio) {
			m_speedTracker.fWorstRatio = ratio;
		}
		
		// Get character info for logging
		CCharacter* pCha = IsCharacter();
		const char* chaName = pCha ? pCha->GetLogName() : "Unknown";
		const char* mapName = m_submap ? m_submap->GetName() : "Unknown";
		Point pos = GetShape().centre;
		uLong moveSpeed = (uLong)m_CChaAttr.GetAttr(ATTR_MSPD);
		
		// Log to dedicated speed hack file
		LG("speed_hack", "[SUSPICIOUS] Character: %s | Map: %s | Pos: (%d, %d) | "
			"Actual: %.1f cm | Expected: %.1f cm | Ratio: %.2fx | "
			"Speed: %u | Elapsed: %u ms | "
			"Violations: %d (session: %d) | Worst: %.2fx\n",
			chaName, mapName, pos.x, pos.y,
			actualDist, expectedDist, ratio,
			moveSpeed, elapsedTime,
			m_speedTracker.sViolationCount, m_speedTracker.sTotalViolations,
			m_speedTracker.fWorstRatio);
		
		// Extra warning for likely hackers (5+ consecutive violations)
		if (m_speedTracker.sViolationCount >= 5) {
			LG("speed_hack", "[WARNING] Character %s has %d consecutive speed violations! "
				"Total session violations: %d, Worst ratio: %.2fx - LIKELY SPEED HACK\n",
				chaName, m_speedTracker.sViolationCount, 
				m_speedTracker.sTotalViolations, m_speedTracker.fWorstRatio);
		}
		
		// Extreme violation (3x+ speed) - very likely hacker
		if (ratio > 3.0f) {
			LG("speed_hack", "[CRITICAL] Character %s moved at %.2fx speed! "
		...
		if (m_speedTracker.sViolationCount > 0 && 
			currentTick - m_speedTracker.ulLastValidationTick > 5000) {
			m_speedTracker.sViolationCount--;
		}
	}
