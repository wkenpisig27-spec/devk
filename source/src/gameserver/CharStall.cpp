...
namespace mission {
#define MAX_STALL_MONEY 100000000000LL // 摆摊单个物品总价不可以超过1000亿
...
void CStallSystem::StartStall(CCharacter& staller, RPACKET& packet) {
	...
	// SANITIZE: Validate stall name
	size_t nameLen = strlen(pszName);
	if (nameLen == 0 || nameLen > 32) {
		LG("Security", "[Stall] Invalid stall name length %zu from character %s\n", nameLen, staller.GetName());
		staller.SystemNotice(RES_STRING(GM_CHARSTALL_CPP_00008));
		return;
	}
	if (!PS::IsSafeString(pszName)) {
		LG("Security", "[Stall] Dangerous characters in stall name from character %s\n", staller.GetName());
		staller.SystemNotice(RES_STRING(GM_CHARSTALL_CPP_00008));
		return;
	}
   ...
   */
	...
	strncpy_s(pData->m_szName, sizeof(pData->m_szName), pszName, ROLE_MAXNUM_STALL_NUM - 1);
	...
	for (BYTE i = 0; i < pData->m_byNum; i++) {
		...
		__int64 gold = packet.ReadLongLong();  // Changed from ReadLong to ReadLongLong for 64-bit
		int count = packet.ReadShort();  // Changed from ReadChar to ReadShort
		int index = packet.ReadChar();

		// validation for item stall.
		if (gold > 100000000000LL) {
			__int64 price = gold - 100000000000LL;
		...
		if (pItem->sType == enumItemTypeBoat) {
			...
			_snprintf_s(szTemp, sizeof(szTemp), _TRUNCATE, RES_STRING(GM_CHARSTALL_CPP_00025), pData->m_Goods[i].byCount, pItem->szName, pData->m_Goods[i].llMoney);
		}
		strncat_s(szLog, sizeof(szLog), szTemp, _TRUNCATE);
	}
...
void CStallSystem::OpenStall(CCharacter& character, RPACKET& packet) {
	...
	// Check if this is an offline stall NPC
	if (pStaller->IsOfflineStallNPC()) {
		COfflineStallNPC* pOfflineStall = static_cast<COfflineStallNPC*>(pStaller);
		pOfflineStall->SendStallDataTo(character);
		return;
	}
				...
				for (BYTE i = 0; i < pData->m_byNum; ++i) {
					if (pData->m_Goods[i].sItemID == itemID) {
						...
						WRITE_STRING(WtPk, pCha->GetStallName());
									char buf[16];
						_snprintf_s(buf, sizeof(buf), _TRUNCATE, "%d,%d", (int)pCha->GetPos().x / 100, (int)pCha->GetPos().y / 100);
						WRITE_STRING(WtPk, buf);
						WRITE_LONG(WtPk, pData->m_Goods[i].byCount);
						WRITE_LONGLONG(WtPk, pData->m_Goods[i].llMoney);
...
void CStallSystem::BuyGoods(CCharacter& character, RPACKET& packet) {
	...
	USHORT byCount = packet.ReadShort();  // Changed from ReadChar to ReadShort
	
	// SANITIZE: Validate stall grid and count
	if (!PS::ValidateRange(static_cast<int>(byGrid), 0, static_cast<int>(ROLE_MAXNUM_STALL_GOODS - 1))) {
		LG("Security", "[Stall] Invalid stall grid %d from character %s\n", byGrid, character.GetName());
		return;
	}
	if (!PS::ValidateStackCount(static_cast<int>(byCount))) {
		LG("Security", "[Stall] Invalid buy count %d from character %s\n", byCount, character.GetName());
		return;
	}
	...
	// Handle offline stall purchase
	if (pStaller->IsOfflineStallNPC()) {
		COfflineStallNPC* pOfflineStall = static_cast<COfflineStallNPC*>(pStaller);
		pOfflineStall->BuyItem(character, byGrid, (BYTE)byCount);
		return;
	}
	...
	if (pData->m_Goods[byIndex].llMoney > 100000000000LL) {
		...
		__int64 n64Temp = (__int64)(pData->m_Goods[byIndex].llMoney) * byCount;
		...
		// Seller receives gold with cap check
                ...
                __int64 saleAmount = (__int64)pData->m_Goods[byIndex].llMoney * byCount;
                ...
                pStaller->setAttr(ATTR_GD, stallerGold);
...
void CStallSystem::DelGoods(CCharacter& staller, BYTE byGrid, BYTE byCount) {
	...
	WRITE_CHAR(packet, byCount);  // Must be CHAR to match client SC_StallDelGoods
	staller.NotiChgToEyeshot(packet, true);  // Include seller so their stall UI updates too
}
