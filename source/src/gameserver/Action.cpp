...
bool CAction::Add(dbc::Short sActionType, void* pActionData) {
	T_B if (m_sCurAction != -1 || m_sActionNum >= defMAX_ACTION_NUM) {
		return false;
	}
...
bool CAction::DoNext(Short sActionType, Short sActionState) {
	T_B if (m_sActionNum == 0 || m_pCEntity == nullptr) {
		return false;
	}
...
void CAction::End() {
	T_B if (m_sActionNum == 0 || m_sCurAction == -1 || m_pCEntity == nullptr) {
		return;
	}
...
bool CAction::Has(Short sActionType, void* pActionData) {
	// if (m_sActionNum == 0 || m_pCEntity == nullptr)
	//	return false;
	// if (m_sCurAction >= m_sActionNum - 1)
	//	return false;
...
void CActionCache::AddCommand(Short sCommand) {
	...
	if (m_pSFreeQueue) {
		...
		if (!pSCarrier) {
			...
			throw std::runtime_error(RES_STRING(GM_ACTION_CPP_00001));
		}
	}
...
void CActionCache::PushParam(void* pParam, Char chSize) {
	T_B if (m_pSExecQueue && (m_pSExecQueue->chParamPos + chSize <= defMAX_CACHE_ACTION_PARAM_LEN)) {
