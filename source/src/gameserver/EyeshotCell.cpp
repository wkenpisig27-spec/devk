...
void CEyeshotCell::EnterEyeshot(Entity* pCEnt) {
	T_B for (short i = 0; i < m_sStateCellNum; i++) if (*m_pCStateCell[i])(*m_pCStateCell[i])->StateBeginSeen(pCEnt);
...
void CEyeshotCell::OutEyeshot(Entity* pCEnt) {
	...
	for (short i = 0; i < m_sStateCellNum; i++) {
		if (m_pCStateCell && m_pCStateCell[i] && *m_pCStateCell[i]) {
	...
	CCharacter* pCCellCha = m_pCChaL;
	while (pCCellCha) {
		// Cache next pointer before any operations that might invalidate it
		Entity* pNextEntity = pCCellCha->m_pCEyeshotCellNext;
		CCharacter* pNextCha = pNextEntity ? pNextEntity->IsCharacter() : nullptr;
		...
		if (pCEnt != pCCellCha) {
			try {
				...
				// Log but continue iteration
				LG("EyeshotCell", "Exception in OutEyeshot character iteration\n");
			}
		}
		pCCellCha = pNextCha;
	}
	...
	CItem* pCCellItem = m_pCItemL;
	while (pCCellItem) {
		// Cache next pointer before any operations
		Entity* pNextEntity = pCCellItem->m_pCEyeshotCellNext;
		CItem* pNextItem = pNextEntity ? pNextEntity->IsItem() : nullptr;
		
		try {
			pCEnt->EndSee(pCCellItem);
		} catch (...) {
			LG("EyeshotCell", "Exception in OutEyeshot item iteration\n");
		}
		pCCellItem = pNextItem;
	}
	T_E
}
...
void CEyeshotCell::RefreshEyeshot(Entity* pCEnt, bool bToEyeshot, bool bToNoHide, bool bToNoShow) {
	...
	if (!pCCha)
		return;
	if (pCCha->GetActControl(enumACTCONTROL_EYESHOT) == bToEyeshot && pCCha->GetActControl(enumACTCONTROL_NOHIDE) == bToNoHide && pCCha->GetActControl(enumACTCONTROL_NOSHOW) == bToNoShow)
		return;
...
CCharacter* CEyeshotCell::GetNextCha(void) {
	...
	if (m_pCChaSearch)
		m_pCChaSearch = m_pCChaSearch->m_pCEyeshotCellNext ? m_pCChaSearch->m_pCEyeshotCellNext->IsCharacter() : nullptr;
