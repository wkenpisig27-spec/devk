...
void CPlayer::Initially() {
	...
	m_pMakingBoat = nullptr;
	m_pStallData = nullptr;
	m_bOfflineStallPending = false;

	SetChallengeType(enumFIGHT_NONE);
	SetInRepair(false);
	SetInForge(false);
	m_pCRepairman = nullptr;
	m_pCForgeman = nullptr;
...
BOOL CPlayer::HasAllBoatInBerth(USHORT sBerthID) {
	// Modified: Allow all boats to be accessible from any harbor
	// Simply check if the player has any boats.
	return m_byNumBoat > 0;
}

BOOL CPlayer::HasBoatInBerth(USHORT sBerthID) {
	// Modified: Allow all boats to be accessible from any harbor
	T_B for (int i = 0; i < m_byNumBoat; i++) {
		// Removed berth ID check - boats can be accessed from any harbor
		if (m_Boat[i]->getAttr(ATTR_BOAT_ISDEAD) == 0) {
			return TRUE;
		}
	}
	return FALSE;
...
BOOL CPlayer::HasDeadBoatInBerth(USHORT sBerthID) {
	// Modified: Allow all boats to be accessible from any harbor
	for (int i = 0; i < m_byNumBoat; i++) {
		// Removed berth ID check - boats can be salvaged from any harbor
		if (m_Boat[i]->getAttr(ATTR_BOAT_ISDEAD) != 0) {
			return TRUE;
		}
	}
...
void CPlayer::GetAllBerthBoat(USHORT sBerthID, BYTE& byNumBoat, BOAT_BERTH_DATA& Data) {
	...
	for (BYTE i = 0; i < m_byNumBoat; i++) {
		// Removed berth ID check - all boats shown at any harbor
		Data.byID[byIndex] = i;
...
void CPlayer::GetBerthBoat(USHORT sBerthID, BYTE& byNumBoat, BOAT_BERTH_DATA& Data) {
	...
	T_B
		BYTE byIndex = 0;
	for (BYTE i = 0; i < m_byNumBoat; i++) {
		// Removed berth ID check - all non-dead boats shown at any harbor
		if (m_Boat[i]->getAttr(ATTR_BOAT_ISDEAD) == 0) {
			Data.byID[byIndex] = i;
...
void CPlayer::GetDeadBerthBoat(USHORT sBerthID, BYTE& byNumBoat, BOAT_BERTH_DATA& Data) {
	// Modified: Allow all boats to be salvaged from any harbor
	BYTE byIndex = 0;
	for (BYTE i = 0; i < m_byNumBoat; i++) {
		// Removed berth ID check - all dead boats shown at any harbor for salvage
		if (m_Boat[i]->getAttr(ATTR_BOAT_ISDEAD) != 0) {
			Data.byID[byIndex] = i;
			Data.byState[byIndex] = 0;
			strncpy(Data.szName[byIndex], m_Boat[i]->GetName(), BOAT_MAXSIZE_BOATNAME - 1);
...
BOOL CPlayer::AddBoat(CCharacter& Boat) {
	T_B if (IsBoatFull()) return FALSE;
...
void CPlayer::NoticeTeamMemberData() {
	T_B int nTMemberCnt = GetTeamMemberCnt();
	if (nTMemberCnt == 0)
		return;
	...
	WRITE_LONG(wpk, (int)pCha->getAttr(ATTR_HP));
	WRITE_LONG(wpk, (int)pCha->getAttr(ATTR_MXHP));
...
bool CPlayer::RefreshMapMask(const char* szMapName, int lPosX, int lPosY) {
...
bool CPlayer::OpenGuildBank() {
	...
	// Set guild bank UI state to open
	pCCtrlCha->SetGuildBankUIState(true);
	return true;
}
