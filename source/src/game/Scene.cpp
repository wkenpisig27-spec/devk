...
void operator<<(FILE* file, const ReallyBigObjectInfo& info) {
	fprintf(file, "%d %f %f %f %f %f %f %f %f\n", &info.typeID, &info.position.x, &info.position.y, &info.position.z,
	        &info.orientation.w, &info.orientation.x, &info.orientation.y, &info.orientation.z, &info.terrainHeight);
}

bool operator<(const ReallyBigObjectInfo& info1, const ReallyBigObjectInfo& info2) {
	return (info1.typeID < info2.typeID || info1.position.x < info2.position.x || info1.position.y < info2.position.y ||
	        info1.position.z < info2.position.z || info1.orientation.w < info2.orientation.w ||
	        info1.orientation.x < info2.orientation.x || info1.orientation.y < info2.orientation.y ||
	        info1.orientation.z < info2.orientation.z || info1.terrainHeight < info2.terrainHeight);
}
...
void CGameScene::_RecordRBO() {
	...
	for (std::set<CSceneObj*>::iterator itr = _reallyBigObjectList.begin(); itr != _reallyBigObjectList.end(); ++itr) {
		(*itr)->SetValid(false);
	}
}
	...
	for (int i = 0; i < _nSceneItemCnt; i++) {
		...
		if (pObj->IsValid() && pObj->IsHide() == FALSE && pObj->getAttachedCharacterID() == -1) {
			if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
				if ((pObjXXX == nullptr) || (dis > info.dis)) {
					pObjXXX = pObj;
					dis = info.dis;
				}
			}
...
bool CGameScene::_CreateMemory() {
	LG("memory", "Create - Map %s, Eff: %d, Cha: %d, Obj: %d, Item: %d\n", _stInit.strMapFile.c_str(), _stInit.nMaxEff,
	   _stInit.nMaxCha, _stInit.nMaxObj, _stInit.nMaxItem);
...
bool CGameScene::_ClearMemory() {
	...
	// Must reset CSynchroManage BEFORE clearing characters.
	// CAttackEffect objects hold _pServerHarm pointers to CServerHarm (owned by CActor).
	// If characters are destroyed first, those pointers become dangling, and
	// ~CAttackEffect() -> DelHarm() will crash accessing freed memory.
...
long CGameScene::AddCharacterBlock(int nCurX, int nCurY, int nDist, BYTE* byBlockBuff, int nBuffWidth, int sRadii) {
	...
	if (_pChaArray == nullptr || byBlockBuff == nullptr)
		return 0;
			...
			for (j = nStartY; j <= nEndY; j++) {
				for (k = nStartX; k <= nEndX; k++) {
					...
					if (nRange >= 0 && nRange < 65535) {
						...
						LG("pathfind",
						   "msgCGameScene::AddCharacterBlock nRange[%d] isn't Valid, Cha name[%s], pos:[%d, %d]",
...
bool CGameScene::_HandleSuperKey() {
	...
	} else if (g_pGameApp->IsKeyDown(DIK_F9)) // ?????jactive??section????????????????
	{
		/*
		// ??J??h????????g?
		...
		const int	ciRefreshWidth = 3;			// ?????�?section?????????section
		const int	ciRefreshHeight = 3;		// ?????�?section????????section
		...
		for (i = 0; i < ciRefreshHeight; i ++)
		{
		    ...
		    for (j = 0; j < ciRefreshWidth; j ++)
		    {
		        ...
		        if (iCurSectionX < 0)
		            continue;
		        g_ObjInfluence.RefreshSectionObjInfluenceInfo(iCurSectionY * _pTerrain->GetSectionCntX() +
		iCurSectionX);
		    }
		}
...
bool CGameScene::GetIsBlockWalk(CCharacter* pCha, int nX, int nY) {
	if (_pTerrain) {
		return _pTerrain->IsGridBlock(nX / 50, nY / 50) ||
		       !g_IsMoveAble(pCha->getChaCtrlType(), pCha->GetDefaultChaInfo()->chTerritory,
...
bool CGameScene::SwitchMap(int nMapID) {
	...
	// PKO FIX: Pause the background resource loading thread pool before destroying scene resources.
	// Without this, worker threads loading textures/meshes can access freed memory during _ClearMemory(),
	// causing crashes on map transitions (portals). SetPoolEvent(TRUE) blocks worker threads.
	lwIThreadPool* tp = g_Render.GetInterfaceMgr()->tp_loadres;
	if (tp) {
	...
	// PKO FIX: Resume background resource loading after new scene is fully initialized.
	if (tp) {
		tp->SetPoolEvent(FALSE);
	}
...
void CGameScene::Reload() {
	// PKO FIX: Pause background resource loading during scene reload to prevent use-after-free
	lwIThreadPool* tp = g_Render.GetInterfaceMgr()->tp_loadres;
	if (tp) {
		tp->SetPoolEvent(TRUE);
	}
	...
	// PKO FIX: Resume background resource loading
	if (tp) {
		tp->SetPoolEvent(FALSE);
	}
}
...
bool CGameScene::LoadMap(const char* file) {
	...
	extern long CALLBACK TerrainNotice(int nFlag, int nSectionX, int nSectionY, DWORD_PTR dwParam,
	                                   MPTerrain* pThis);
...
bool CGameScene::_Init() {
	...
	// Initialize shadow mapping from display settings.
	const bool bShadowOn = g_stUISystem.m_sysProp.m_videoProp.nShadowMode > 0;
	RecreateShadowMapFromQuality(g_stUISystem.m_sysProp.m_videoProp.nQuality, bShadowOn);
