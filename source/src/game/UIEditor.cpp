...
bool CEditor::Init() {
	...
	if (!frmEditor) {
		LG("gui", RES_STRING(CL_LANGUAGE_MATCH_527));
		return false;
	}
	...
	if (!trvEditor)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "trvEditor");
	...
	if (!imgEditor)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgEditor");

	btnColor = dynamic_cast<CTextButton*>(frmEditor->Find("btnColor"));
	if (!btnColor)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "btnColor");
	...
	if (!chkSize01)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize01");

	chkSize02 = dynamic_cast<CCheckBox*>(frmEditor->Find("chkSize02"));
	if (!chkSize02)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize02");

	chkSize03 = dynamic_cast<CCheckBox*>(frmEditor->Find("chkSize03"));
	if (!chkSize03)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSize03");

	chgGroup = dynamic_cast<CCheckGroup*>(frmEditor->Find("chgGroup"));
	if (!chgGroup)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chgGroup");
	chgGroup->evtSelectChange = _CheckSelectChange;

	chkWireFrame = dynamic_cast<CCheckBox*>(frmEditor->Find("chkWireFrame"));
	if (!chkWireFrame)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkWireFrame");
	chkWireFrame->evtCheckChange = _WireCheckChange;

	chkEnabledAlpha = dynamic_cast<CCheckBox*>(frmEditor->Find("chkEnabledAlpha"));
	if (!chkEnabledAlpha)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkEnabledAlpha");
	...
	if (!chkSmooth)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkSmooth");
	...
	if (!chkHideTree)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkHideTree");
	...
	if (!imgTreeFrame)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgTreeFrame");

	edtSetMainChaPos = dynamic_cast<CEdit*>(frmEditor->Find("edtSetMainChaPos"));
	if (!edtSetMainChaPos)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "edtSetMainChaPos");
	edtSetMainChaPos->evtKeyChar = _evtSetChaPosKeyChar;

	CEdit* edtShotMiniMap = dynamic_cast<CEdit*>(frmEditor->Find("edtShotMiniMap"));
	if (edtShotMiniMap)
		edtShotMiniMap->evtKeyChar = _evtShotMiniMap;
	...
	if (!edtSetBrushHeight)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "edtSetBrushHeight");
	...
	if (!chkModifyHeight)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkModifyHeight");
	...
	if (!chkColor)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkColor");
	...
	if (!chkShowLightBrush)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkShowLightBrush");
	...
	if (!ckhHideOptionEx)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "ckhHideOptionEx");
	...
	if (!imgEditorEx)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "imgEditorEx");

	lblSetMainChaPos = dynamic_cast<CLabel*>(frmEditor->Find("lblSetMainChaPos"));
	if (!lblSetMainChaPos)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetMainChaPos");

	lblSetBrushHeight = dynamic_cast<CLabel*>(frmEditor->Find("lblSetBrushHeight"));
	if (!lblSetBrushHeight)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetBrushHeight");

	ckhShowCompass = dynamic_cast<CCheckBox*>(frmEditor->Find("ckhShowCompass"));
	if (!ckhShowCompass)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "ckhShowCompass");
	...
	if (!edtSetIslandIndex)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "edtSetIslandIndex");
	...
	if (!lblSetIslandIndex)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblSetIslandIndex");
	lblSetIslandIndex->SetIsShow(false);

	chkEraseAttrib = dynamic_cast<CCheckBox*>(frmEditor->Find("chkEraseAttrib"));
	if (!chkEraseAttrib)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "chkEraseAttrib");
	...
	if (!lblEraseAttrib)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmEditor->GetName(), "lblEraseAttrib");
...
void CEditor::_TreeMouseDown(CGuiData* pSender, int x, int y, DWORD key) {
	...
	bool press = (tree->GetSelect()->GetItem() != nullptr);
...
void CEditor::_TreeEvent(CTreeView* tree, bool press) {
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_528)) == 0) {
		...
		if (p)
			g_Editor.SetCha(press, p->nTag);
		return;
	}

	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_529)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p)
			g_Editor.SetEff(press, p->nTag);
		return;
	}

	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_530)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p) {
			if (press) {
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_532)) == 0) {
		...
		if (p && press) {
			...
			g_stUIEditor.trvEditor->SetSelectNode(nullptr);
		}
		return;
	}
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_533)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p)
			g_Editor.SetChaPose(press, p->nTag);
		return;
	}

	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_534)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p) {
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_535)) == 0) {
		...
		if (p) {
			...
			case 1:
				...
				g_pGameApp->AddTipText(RES_STRING(CL_LANGUAGE_MATCH_537));
				break;
			}
		}
		return;
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_538)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p)
			g_Editor.ChangePart(press, p->nTag);
		return;
	}
	...
	if (stricmp(item->GetString(), RES_STRING(CL_LANGUAGE_MATCH_539)) == 0) {
		CGraph* p = dynamic_cast<CGraph*>(tree->GetSelect()->GetItem());
		if (p)
			g_Editor.SetTerr(press, p->nTag);
		return;
	}
...
void CEditor::_ColorMouseDown(CGuiData* pSender, int x, int y, DWORD key) {
	...
	if (UIGetChooseColor(color))
		g_stUIEditor.btnColor->GetImage()->SetColor(color);
...
bool CEditor::_evtSetChaPosKeyChar(CGuiData* pSender, char& key) {
	if (!CGameApp::GetCurScene() || !CGameApp::GetCurScene()->GetMainCha())
		return false;
	...
	if (key == ',' || isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
		if (key == VK_RETURN) {
			...
			if (pos != string::npos) {
				...
				g_pGameApp->AddTipText(RES_STRING(CL_LANGUAGE_MATCH_542));
			}
		}
		return true;
	}
...
bool CEditor::_evtShotMiniMap(CGuiData* pSender, char& key) {
	...
	if (key == ',' || isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
		if (key == VK_RETURN) {
			...
			if (n != 4) {
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_543));
				return true;
			}
	...
	if (isdigit(key) || key == VK_RETURN || key == VK_BACK || key == '-') {
		if (key == VK_RETURN) {
			...
			if (str.empty()) {
				...
				g_pGameApp->AddTipText(RES_STRING(CL_LANGUAGE_MATCH_546));
...
void CEditor::_HideOptionExCheckChange(CGuiData* pSender) {
	...
	if (g_stUIEditor.lblShotMiniMap)
		g_stUIEditor.lblShotMiniMap->SetIsShow(isShow);
}
...
void CEditor::_EraseAttribCheckChange(CGuiData* pSender) {
	...
	if (chk)
		g_Editor.enable_erase();
	else
		g_Editor.enable_erase(false);
}
