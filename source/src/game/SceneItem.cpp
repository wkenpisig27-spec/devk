...
void CSceneItem::_UpdateValid(BOOL bValid) {
	if (bValid == FALSE) {
		...
		// PKO FIX: Null-check GetScene() before HandleSceneMsg during item destroy
		if (GetScene())
...
void CSceneItem::_UpdatePos() {
	...
	// PKO FIX: Null-check _pScene before GetGridHeight
	_fTerrainHeight = _pScene ? _pScene->GetGridHeight(_vPos.x, _vPos.y) : 0.0f;
...
void CSceneItem::SetForgeEffect(DWORD value, int nCharID) {
	if (_dwForgeValue == value || !_pItemInfo)
		return;
	_dwForgeValue = value;
