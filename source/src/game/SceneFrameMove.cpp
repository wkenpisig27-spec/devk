...
void CGameScene::_FrameMove(DWORD dwTimeParam) {
	...
	for (i = 0; i < _nChaCnt; i++) {
		...
		if (pCha->IsValid() && !pCha->GetIsForUI()) {
			// Skip FrameMove for non-main characters that are off-screen (exclude mounts - their position is owner-controlled)
			if (!pCha->IsMainCha() && !pCha->GetIsMount() &&
				!IsPointVisible((float)pCha->GetCurX() / 100.0f, (float)pCha->GetCurY() / 100.0f)) {
				continue;
			}
	...
	// if(ResMgr.GetCanFrame() == 1)
	{
		if (_nEffCnt > 0) {
			...
			static CTimeWork time(1 * 60 * 1000);
