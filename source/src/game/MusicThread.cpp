...
bool CMusicThread::Begin() {
	if (!m_hThread) {
		...
		m_hThread = CreateThread(nullptr, 0, ThreadStartRoutine, this, 0, &m_dwThreadID);
		// if( m_hThread )
...
void CMusicThread::SetThreadPriority(EThreadPriority NewPriority) {
	if (NewPriority != m_ThreadPriority) {
		...
		::SetThreadPriority(m_hThread,
							m_ThreadPriority == TPri_AboveNormal ? THREAD_PRIORITY_ABOVE_NORMAL : m_ThreadPriority == TPri_BelowNormal ? THREAD_PRIORITY_BELOW_NORMAL
...
UINT CMusicThread::Run() {
	...
	try {
		...
		LG("music_error", "Exception playing music ID: %d\n", m_dwMusicID);
	}
