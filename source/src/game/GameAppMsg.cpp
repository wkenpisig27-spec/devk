		...
		if (g_ObjFile.ReadSectionObjInfo(nSectionNO, infoex, &nSectionObjCnt)) {
			for (int i = 0; i < nSectionObjCnt; i++) {
				...
				if (infoex[i].GetType() == 0) // ???????
				{
					...
					} else if (g_Config.m_bEnableLGMsg) // ?????????????????????????
					{
						_stprintf(tcsPrint,
								  _TEXT(RES_STRING(CL_LANGUAGE_MATCH_101)),
								  infoex[i].GetID());
						if (IDYES == MessageBox(nullptr, tcsPrint, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), MB_YESNO)) {
				...
				} else if (infoex[i].GetType() == 1) // ??????
				{
					...
					} else if (g_Config.m_bEnableLGMsg) // ?????????????????????????
					{
						_stprintf(tcsPrint,
								  _TEXT("???? ID ???%d ???????????????????????????.obj?l??????"),
								  infoex[i].GetID());
						if (IDYES == MessageBox(nullptr, tcsPrint, _TEXT("????"), MB_YESNO)) {
							for (int j = i; j < nSectionObjCnt - 1; j++)
								infoex[j] = infoex[j + 1];
							i--;
							nSectionObjCnt--;
...
void CGameApp::HandleKeyDown(DWORD dwKey) {
	...
	if (g_Config.m_bEditor && IsKeyDown(DIK_TAB)) // ????|??????
	{
...
void CGameApp::MouseButtonDown(int nButton) {
	...
	try {
		...
		LG("input_error", "Exception in MouseButtonDown, button=%d\n", nButton);
...
void CGameApp::MouseContinue(int nButton) {
	// Time-based mouse tracking (native 60 FPS)
...
void CGameApp::MouseMove(int nOffsetX, int nOffsetY) {
	...
	try {
		...
		LG("input_error", "Exception in MouseMove\n");
...
void CGameApp::MouseButtonDB(int nButton) {
	...
	try {
		...
		LG("input_error", "Exception in MouseButtonDB\n");
...
void CGameApp::MouseScroll(int nScroll) {
	...
	try {
		...
		LG("input_error", "Exception in MouseScroll\n");
...
void CGameApp::HandleSuperKey() {
	...
	try {
		...
		LG("input_error", "Exception in HandleSuperKey\n");
...
#include "resource.h"
bool CGameApp::HandleWindowMsg(DWORD dwMsg, WPARAM wParam, LPARAM lParam) {
	if (dwMsg > WM_USER)
		_HandleMsg(dwMsg, wParam, lParam);
	else if (!_IsSceneOk())
		return false;
	...
	case WM_KEYDOWN: {
		...
		if (g_Config.IsPower() || (CGameScene::GetMainCha() && CGameScene::GetMainCha()->getGMLv())) {
			GetConsole()->OnKeyDownEvent(wParam);
		}
	...
	case WM_CHAR: {
		...
		if (g_Config.IsPower() || (CGameScene::GetMainCha() && CGameScene::GetMainCha()->getGMLv())) {
			if (GetConsole()->OnCharEvent((TCHAR)wParam, lParam))
				return false;
		}
		// g_InputBox.HandleWindowMsg(dwMsg, wParam, lParam);
	...
	case WM_SYSKEYDOWN: {
		...
		if (IsAltPress()) {
			...
			if (CFormMgr::s_Mgr.GetEnableHotKey()) // ??????????
				CFormMgr::s_Mgr.OnHotKey((char)wParam, 0);
	...
	default: {
		// g_InputBox.HandleWindowMsg(dwMsg, wParam, lParam);
		if (CFormMgr::s_Mgr.HandleWindowMsg(dwMsg, wParam, lParam))
			return false;
		break;
	}
	}
...
const char* ConsoleCallback(const char* pszCmd) {
	...
	} else if (strCmd == "ui") // ???UI??????????? 0??? 1??	-added by Arcol
	{
		...
		*p = 0;
		...
		if (pMain) {
			...
			if (pItem) {
				...
				pItem->lTag = RefineID * 10 + Level; // ????copy????
		...
		for (int j(0); j < nTestCount; j++) {
			// g_pGameApp->AutoTestInfo( "??%d?????...", j );

			// ??????????????
			{
				// g_pGameApp->AutoTestInfo( "??????????????" );
				...
				for (int i = nStart; i < nEnd; i++) {
					...
					if (!pEffect->Create(i)) {
						// g_pGameApp->AutoTestInfo( "Error, ??????????:%d", i );
						continue;
					}
		...
		// ???????????????????
		{
			...
			CSkillRecord* _pSkillInfo = nullptr;
		...
		for (int k = nStart; k < nEnd; k++) {
			...
			if (_pSkill->IsAttackArea()) {
				if (_pSkill->sTargetEffectID > 0)
					_pAttack->SelfEffect(_pSkill->sTargetEffectID, _pSkill->sTargetDummyLink, false, -1, nAngle);
			} else if (_pTarget) {
				_pTarget->HitEffect(nAngle);
				if (_pSkill->sTargetEffectID > 0)
					_pTarget->SelfEffect(_pSkill->sTargetEffectID, _pSkill->sTargetDummyLink, false, -1, nAngle);
			}
