...
struct wield_state {
	...
	wield_state(BYTE m, BYTE l, BYTE r) : mode(m), left(l), right(r) {
	}
...
const static wield_state __wield_state[__wield_state_num] = {
    wield_state(S_MELEE, LINK_ITEM_NULL, LINK_ITEM_BURSE),     wield_state(S_MELEE, LINK_ITEM_NULL, LINK_ITEM_GLOVE),
    wield_state(S_MELEE, LINK_ITEM_SHIELD, LINK_ITEM_NULL),

    wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_SWORD),    wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_BLADE),
    wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_STICK),    wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_HAMMER),
    ...
    wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_AX),       wield_state(S_MELEE2, LINK_ITEM_NULL, LINK_ITEM_SHOVEL),
...
// ==================================
CCharacterModel::CCharacterModel() : _ModelType(MODEL_INVALID), _PoseType(0), cha_type_id(-1) {
	_UIScaleDis = 20.0f;
	_UIYaw = 0;
	_UIPitch = 0;
	_UIColor[0] = 255;
...
int CCharacterModel::ChangePart(DWORD part_id, DWORD tab_id) {
	...
	if (_PoseTabID != 0xffffffff) {
		if ((_ModelType == MODEL_WARSHIP && part_id == 0) || (_ModelType == MODEL_TOWER && part_id == 1)) {
			// ��Ҫ����load pose
			LoadPose(_PoseTabID);
...
void CCharacterModel::RenderForUI(int x, int y, bool bShowLingItem /*= true*/) {
	...
	if (bShowLingItem) {
		MPSceneItem* pLItem = GetLinkItem(LINK_ID_LEFTHAND);
		if (pLItem) {
			pLItem->FrameMove();
		}
		MPSceneItem* pRItem = GetLinkItem(LINK_ID_RIGHTHAND);
		if (pRItem) {
			pRItem->FrameMove();
		}

		MPSceneItem* back1 = GetLinkItem(21);
		if (back1)
			back1->FrameMove();

		MPSceneItem* back2 = GetLinkItem(22);
		if (back2)
			back2->FrameMove();

		MPSceneItem* back3 = GetLinkItem(23);
		if (back3)
			back3->FrameMove();
...
float CCharacterModel::GetPoseVelocity() {
	...
	switch (_ModelType) {
	case MODEL_CHARACTER:
		v = GetPlayPoseInfo()->velocity; // Testing Mdr.st
		break;
	case MODEL_WARSHIP:
