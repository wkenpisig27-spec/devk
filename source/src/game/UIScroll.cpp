...
CTextButton* CScroll::_pContinueButton = nullptr;
DWORD CScroll::_dwStartTime = 0;

CScroll::CScroll(CForm& frmOwn)
	: CCompent(frmOwn), _eStyle(btVertical), evtChange(nullptr), _fPageNum(2.0), _IsAutoHide(true) {
...
CScroll::CScroll(const CScroll& rhs)
	: CCompent(rhs), _up(new CTextButton(*rhs._up)), _down(new CTextButton(*rhs._down)), _pScroll(new CDragTitle(*rhs._pScroll)), _pImage(new CGuiPic(*rhs._pImage)) {
	_Copy(rhs);
...
bool CScroll::MouseRun(int x, int y, DWORD key) {
	...
	if (InRect(x, y)) {
		...
		if (_up->MouseRun(x, y, key))
			return true;
		if (_down->MouseRun(x, y, key))
			return true;
		...
		if (key & Mouse_LDown) {
			...
			case btVertical:
				if (y > _pScroll->GetY2()) {
					PageUp();
				} else if (y < _pScroll->GetY())
					PageDown();
				break;
			case btHorizontal:
				if (x > _pScroll->GetX2())
					PageUp();
				else if (x < _pScroll->GetX())
					PageDown();
				break;
			}
			_RefreshMidst();
...
void CScroll::_CheckScrollDrag() {
	...
	case btVertical:
		...
		if (pDrag->GetDragY() < _nMinDrag)
			pDrag->SetDragY(_nMinDrag);
		if (pDrag->GetDragY() > _nMaxDrag)
			pDrag->SetDragY(_nMaxDrag);
		_UpdataStep(pDrag->GetDragY() + _pScroll->GetY());
		break;
	case btHorizontal:
		pDrag->SetDragY(0);
		if (pDrag->GetDragX() < _nMinDrag)
			pDrag->SetDragX(_nMinDrag);
		if (pDrag->GetDragX() > _nMaxDrag)
			pDrag->SetDragX(_nMaxDrag);
