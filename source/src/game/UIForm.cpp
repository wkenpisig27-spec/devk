...
CForm* CForm::_pActive = nullptr;
int CForm::_nDragOffX = 0;
int CForm::_nDragOffY = 0;

CForm::CForm()
	: CGuiData(), _isModal(false), _nActiveCompentID(0), _modalResult(mrNone), _nOffWidth(0), _nOffHeight(0), evtShow(nullptr), evtHide(nullptr), evtActive(nullptr), evtLost(nullptr), evtMouseDown(nullptr), evtHotkeyHandler(nullptr), evtEntrustMouseEvent(nullptr), evtKeyDown(nullptr), evtKeyChar(nullptr), _cHotKey(0), _formStyle(fsNone), _IsEscClose(true), evtMouseDragEnd(nullptr), evtOnSetScreen(nullptr), _pPointer(nullptr), _pPopMenu(nullptr), evtClose(nullptr), _pEnterButton(nullptr), evtBeforeShow(nullptr), evtEscClose(nullptr) {
...
void CForm::OnSetScreen() {
	...
	case fsRightBottom:
		...
		break;
	case fsBottomXCenter:
		SetPos(sw / 2 - GetWidth() / 2 + _nOffWidth, sh - GetHeight() - _nOffHeight);
		break;
	default:
		SetPos(GetLeft(), GetTop());
		break;
...
void CForm::PopMenu(CMenu* pMenu, int x, int y) {
	...
	if (_pPopMenu) {
		if (_pPopMenu->GetParent() && _pPopMenu->GetParent() != this) {
			...
			old_form->PopMenu(nullptr);
		}

		// Hide disabled menu items
		for (int i = 0; i < _pPopMenu->GetCount(); ++i) {
			CMenuItem* item = _pPopMenu->GetMenuItem(i);
...
int CForm::ClearChild() {
	int i = 0;
	while (!_childs.empty()) {
		_childs.back()->SetParent(nullptr);
		i++;
	}
	return i;
}
