...
CSkillCommand::CSkillCommand(CSkillRecord* pSkill)
	: CCommandObj(), _pAniClock(nullptr), _pSkill(pSkill), _dwPlayTime(0) {
	if (!_pSkill)
		LG("error", "msgCSkillCommand::CSkillCommand(CSkillRecord * pSkill) pSkill is NULL");
...
CSkillCommand::CSkillCommand(const CSkillCommand& rhs)
	: _pImage(new CGuiPic(*rhs._pImage)), _pAniClock(nullptr), _dwPlayTime(0) {
}
