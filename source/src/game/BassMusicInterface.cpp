...
#define BASS2_MUSIC_RAMP 0x200 // normal ramping
...
BassMusicInterface::BassMusicInterface(HWND theHWnd) {
	...
	if (gBass->mVersion2) {
		success = gBass->BASS_Init2(1, 44100, 0, theHWnd, nullptr);
...
bool BassMusicInterface::LoadMusic(int theSongId, const string& theFileName) {
...
// Fade music volume when window is in background
// 1.0f = full volume (foreground), 0.0f = muted (minimized), 0.5f = half volume (partial)
void BassMusicInterface::FadeByWindow(float fFade) {
	if (m_fFadeByWindow != fFade) {
		...
		// Apply faded volume to all playing music
		BassMusicMap::iterator anItr = mMusicMap.begin();
		while (anItr != mMusicMap.end()) {
			BassMusicInfo* aMusicInfo = &anItr->second;
			int fadedVolume = (int)(aMusicInfo->mVolume * m_fFadeByWindow * 100);
			gBass->BASS_ChannelSetAttributes(aMusicInfo->GetHandle(), -1, fadedVolume, -101);
			++anItr;
		}
	}
}