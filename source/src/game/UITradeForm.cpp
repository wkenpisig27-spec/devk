...
bool CTradeMgr::Init() {
	...
	if (!grdSale)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "grdSale");
	...
	if (!grdBuy)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "grdBuy");
	...
	if (!labOtherGold)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "labOtherGold");

	labSelfGold = dynamic_cast<CLabelEx*>(frmPlayertrade->Find("labSelfGold"));
	if (!labSelfGold)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "labSelfGold");

	labOtherIMP = dynamic_cast<CLabelEx*>(frmPlayertrade->Find("labOtherIMP"));
	if (!labOtherIMP)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "labOtherIMP");

	labSelfIMP = dynamic_cast<CLabelEx*>(frmPlayertrade->Find("labSelfIMP"));
	if (!labSelfIMP)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "labSelfIMP");

	chkYes = (CCheckBox*)frmPlayertrade->Find("chkYes");
	if (!chkYes)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "chkYes");

	chkTrade = (CCheckBox*)frmPlayertrade->Find("chkTrade");
	if (!chkTrade)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "chkTrade");

	btnYes = (CTextButton*)frmPlayertrade->Find("btnYes");
	if (!btnYes)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "btnYes");

	btnTrade = (CTextButton*)frmPlayertrade->Find("btnTrade");
	if (!btnTrade)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "btnTrade");

	btnGold = (CTextButton*)frmPlayertrade->Find("btnGold");
	if (!btnGold)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "btnGold");

	btnIMP = (CTextButton*)frmPlayertrade->Find("btnIMP");
	if (!btnIMP)
		return Error(RES_STRING(CL_LANGUAGE_MATCH_45), frmPlayertrade->GetName(), "btnIMP");
...
void CTradeMgr::ShowCharTradeRequest(BYTE byType, DWORD dwRequestID) {
	...
	if (byType == mission::TRADE_CHAR)
		sprintf(szBuf, RES_STRING(CL_LANGUAGE_MATCH_779), pCha->getHumanName());
	else
		sprintf(szBuf, RES_STRING(CL_LANGUAGE_MATCH_780), pCha->getHumanName());
...
void CTradeMgr::_MainMousePlayerTradeEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	if (name == "btnNo" || name == "btnClose") // ???????,??????
	{
		...
		g_stUIBox.ShowNumberBox(_evtGoldFormEvent, g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_GD), RES_STRING(CL_LANGUAGE_MATCH_781), false);
	} else if (name == "btnIMP") {
...
void CTradeMgr::ShowCharTrade(BYTE byType, DWORD dwAcceptID, DWORD dwRequestID) {
	...
	CCharacter* pRequestCha(nullptr);
	CCharacter* pAcceptCha(nullptr);
	...
	if (grdSale)
		grdSale->Clear();
	if (labOtherGold)
		labOtherGold->SetCaption("0");
	if (labSelfGold)
		labSelfGold->SetCaption("0");
	if (labOtherIMP)
		labOtherIMP->SetCaption("0");
	if (labSelfIMP)
		labSelfIMP->SetCaption("0");
	...
	string strPlayTradeLabName(""); // labPlayertradeName??Caption
...
void CTradeMgr::DragTradeToItem(DWORD dwCharID, BYTE byIndex, BYTE byItemIndex) {
	...
	if (g_stUITrade.m_dwMainID == dwCharID) {
		...
		if (pObj)
			pObj->SetIsValid(true);
	} else {
		tradeGrd = g_stUITrade.GetPlayertradeBuyGrid();
	}

	if (tradeGrd)
		tradeGrd->DelItem(byIndex);
}
...
void CTradeMgr::ShowCharTradeMoney(DWORD dwCharID, long long llMoney) {
	if (!frmPlayertrade)
		return;
	if (!labOtherGold)
		return;
	...
	char szBuf[32] = {0};
	_snprintf_s(szBuf, sizeof(szBuf), _TRUNCATE, "%s", StringSplitNum((__int64)llMoney));
...
void CTradeMgr::LocalSaleItem(CGoodsGrid* pSaleGrid, CGoodsGrid* pSelfGrid, int nGridID, CCommandObj* pItem) {
	...
	if (CItemCommand* pBoat = dynamic_cast<CItemCommand*>(pItem)) {
		if (pBoat && pBoat->GetItemInfo()->sType == 43) {
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_782));
		}
	}
...
void CTradeMgr::CancelCharTrade(DWORD dwCharID) {
	if (dwCharID == m_dwMainID)
		...
		g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_787));
...
bool CTradeMgr::IsTrading() {
	if (frmPlayertrade)
		return frmPlayertrade->GetIsShow();
	return false;
}
