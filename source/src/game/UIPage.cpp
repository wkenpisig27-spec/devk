...
CPageItem::CPageItem(CForm& frmOwn)
	: CContainer(frmOwn), _nIndex(0), _pImage(nullptr), _pTitle(nullptr) {
...
CCompent* CPageItem::GetHintCompent(int x, int y) {
	...
	if (pHint = _pTitle->GetHintCompent(x, y))
		return pHint;
	if (pHint = _pImage->GetHintCompent(x, y))
		return pHint;
...
CPage::CPage(CForm& frmOwn)
	: CCompent(frmOwn), _nIndex(-1), _eButtonPos(bpLeftUp), _nButtonMargin(0), _nButtonWidth(100), _nButtonHeight(30), evtSelectPage(nullptr), _nMaxPage(0) {
