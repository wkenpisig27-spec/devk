...
CEdit::CEdit(CForm& frmOwn)
	: CCompent(frmOwn), _bIsPassWord(false), _bIsMulti(false), _bIsDigit(false), _bIsWrap(false), _nOffset(0), _nMaxNum(64), _nMaxNumVisible(64), _nFontHeight(24), _nMaxLineNum(0), _nCursorCol(0), _nCursorFirstCol(0), _nCursorSecondCol(0), _nCursorRow(0), _bParseText(false), _color(COLOR_BLACK), evtKeyDown(nullptr), evtKeyChar(nullptr), _nLeftMargin(0), _nTopMargin(2), evtEnter(nullptr), evtChange(nullptr), _pEnterButton(nullptr), _nCursorColor(COLOR_BLACK) {
	_IsFocus = true;
...
bool CEdit::OnChar(char c) {
	if (!IsNormal())
		return false;
	if (evtKeyChar && !evtKeyChar(this, c))
		return true;
	...
	case '\r': // �س�
		...
		if (!GetIsMulti()) {
			if (evtEnter && !_str.empty())
				evtEnter(this); // ����Edit,ִ��Ĭ�Ϸ���
		} else {
			//_nCursorCol = 0;
			//_nCursorRow++;
		}
	...
	case '@': {
		...
		unsigned int keyboard = (unsigned int)(reinterpret_cast<uintptr_t>(hCurKeyboard));
...
void CEdit::CorrectCursor() {
	if (_bIsWrap) {
		// Use actual pixel width for proportional font support
		int cursorPos = CGuiFont::s_Font.GetWidth(_str.substr(0, _nCursorCol).c_str());
		int textWidth = CGuiFont::s_Font.GetWidth(_str.c_str());
		
		if (_nOffset + GetWidth() - 5 < cursorPos) {
			_nOffset = cursorPos - GetWidth() + 5;
		} else if (_nOffset > cursorPos) {
			_nOffset = cursorPos;
		} else if (_nOffset > 0 && _nOffset + GetWidth() - 5 > textWidth) {
			_nOffset = textWidth - GetWidth() + 5;
			if (_nOffset < 0)
				_nOffset = 0;
		}
	} else {
...
void CEdit::Render() {
	...
	if (!GetIsPassWord()) {
		// Simplified rendering: always render full string, clipping handles visibility
		// CorrectCursor() calculates _nOffset for proper scrolling based on pixel width
		if (_bParseText) {
			...
			if (GetIsMulti()) {
				try {
					...
					LG("ui_error", "Exception in CEdit multiline render\n");
				}
			} else {
...
void CEdit::_GetCursorPos(int nCurPos) {
	...
	if (GetIsMulti()) {
		...
		// For password fields, measure asterisks instead of actual text
		if (GetIsPassWord()) {
			std::string asterisks(nCurPos, '*');
			cx = CGuiFont::s_Font.GetWidth(asterisks.c_str());
		} else {
			cx = CGuiFont::s_Font.GetWidth(_str.substr(0, nCurPos).c_str());
		}
		cx -= _nOffset;
	}
	// Bitmap fonts render at actual pixel coordinates, no scaling needed
	_nCursorX = cx + GetX();
