...
CWaitAttackState::CWaitAttackState(CActor* p)
	: CActionState(p), _pSkillInfo(nullptr), _pTarget(nullptr), _eUseSkill(enumInit), _nAngle(0), _IsHaveAngle(false), _nKeyFrameNum(1), _pSelf(nullptr), _nSkillActionIndex(0), _nAttackState(0), _nSkillSpeed(0), _nActionEnd(0), _fSkillRate(1.0f) {
...
void CWaitAttackState::SetSkill(CSkillRecord* p) {
	if (!p) {
		LG("state", RES_STRING(CL_LANGUAGE_MATCH_398));
	}
...
void CWaitAttackState::CalcSkillSpeed() {
	...
	if (!_pSkillInfo->IsPlayRand()) {
		...
		for (int i = 0; i < nCount; i++) {
			...
			if (_fSkillSpeed[i] < 0.1f)
				_fSkillSpeed[i] = 0.1f; // 0.1f
			if (_fSkillSpeed[i] > 10.0f)
				_fSkillSpeed[i] = 10.0f; // 10.0f
		}
	}
}
...
void CWaitAttackState::_UseSkill() {
	...
	if (_nKeyFrameNum <= 0 && _pSkillInfo->sActionKeyFrme != -1) {
		LG("error", RES_STRING(CL_LANGUAGE_MATCH_399), _pSelf->getLogName(), _pSkillInfo->szName, _nSkillPoseID);
	}
...
void CWaitAttackState::ActionFrame(DWORD pose_id, int key_frame) {
	...
	if (_pSkillInfo->sItemEffect1[0] > 0 && _pSkillInfo->sItemEffect1[1] == key_frame)
		_pSelf->ItemEffect(_pSkillInfo->sItemEffect1[0], _pSkillInfo->sItemDummyLink, _pSelf->getYaw());
	if (_pSkillInfo->sItemEffect2[0] > 0 && _pSkillInfo->sItemEffect2[1] == key_frame)
		_pSelf->ItemEffect(_pSkillInfo->sItemEffect2[0], _pSkillInfo->sItemDummyLink, _pSelf->getYaw());
...
void CWaitAttackState::ActionEnd(DWORD pose_id) {
	...
	if (_IsPoseValid(pose_id)) {
		...
		if (_pSkillInfo->IsPlayCyc() || _nActionEnd == 1) {
			if (_pSelf->IsMainCha())
				LG(_pSelf->getLogName(), "ActionEnd: %d\n", pose_id);
...
CAttackState::CAttackState(CActor* p)
	: CWaitAttackState(p), _pMove(nullptr), _nTotalDis(0), _IsMoveOver(false), _pCommand(nullptr), _dwEndTime(0), _IsForce(false), _IsLocalCancel(false), _nActionKeyCount(0), _nActionEndCount(0) {
	_IsSend = true;
...
void CAttackState::_UseSkill() {
	// NOTE: _pCommand->StartCommand() is now called in _Start() to prevent
	// use-after-free when the item is consumed before _UseSkill() runs.
...
bool CAttackState::_Start() {
	...
	if (_pSkillInfo->sID == 241) {
		if (_pSkillInfo->GetLevel() > _pSkillInfo->GetJobMax(_pSelf->getGameAttr()->get(ATTR_JOB))) {
			LG("error", RES_STRING(CL_LANGUAGE_MATCH_400), _pSkillInfo->GetLevel(), _pSkillInfo->GetJobMax(_pSelf->getGameAttr()->get(ATTR_JOB)));
			return false;
		}
	...
	// Execute command (e.g. cooldown) now BEFORE server can delete the item.
	// _UseSkill() may run later (after movement), by which time the server's
	// kitbag update could have already deleted the CItemCommand object.
	if (_pCommand) {
		_pCommand->StartCommand();
		_pCommand = nullptr; // Prevent use-after-free if item is consumed
	}
	...
	if (_nSkillSpeed == 0) {
		LG("CAttackState", RES_STRING(CL_LANGUAGE_MATCH_401));
	}
	...
	if (_pSkillInfo->IsAttackArea()) {
		...
		// Safety check: ensure target pointer is valid before accessing
		if (!_pTarget) {
			LG("CAttackState", "Error: _pTarget is null for non-area skill\n");
			return false;
		}
...
void CTraceAttackState::CheckTrace() {
	if (_pSelf->GetDefaultSkillInfo() == _pSkillInfo && _IsTrace && _pTarget && _pTarget->IsEnabled() && _pTarget->IsValid()) {
		...
		if (_pSelf->GetDefaultChaInfo()->chTerritory != _pTarget->GetDefaultChaInfo()->chTerritory) {
			// PKO FIX: Null-check GetScene() before GetGridRegion
			if (_pSelf->GetScene() && !g_IsMoveAble(_pSelf->getChaCtrlType(), _pSelf->GetDefaultChaInfo()->chTerritory, _pSelf->GetScene()->GetGridRegion(_pTarget->GetCurX() / 100, _pTarget->GetCurY() / 100)))
				return;
		}
...
void CTraceAttackState::MoveEnd(int x, int y, int nState) {
	...
	if ((nState & enumMSTATE_CANCEL) || (enumMSTATE_NOTARGET & nState) || (enumMSTATE_CANTMOVE & nState)) {
		_IsTrace = false;
	}
}
...
void CTraceAttackState::ServerEnd(int nState) {
	...
	if ((enumFSTATE_CANCEL & nState) || (enumFSTATE_TARGET_DIE & nState) || (enumFSTATE_TARGET_IMMUNE & nState) || (enumFSTATE_TARGET_NO & nState) || (enumFSTATE_OFF & nState)) {
		_IsTrace = false;
	}
}
...
void CHitAttackState::_End() {
	g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_402));
}
...
bool CHitAttackState::_Start() {
	...
	char szBuf[128] = {0};
	sprintf(szBuf, RES_STRING(CL_LANGUAGE_MATCH_403), _nPoseNum, _fSpeed);
	string str = szBuf;
