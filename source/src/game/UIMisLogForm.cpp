...
void CMisLogForm::_MouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	if (stricmp("frmMission", pSender->GetForm()->GetName()) == 0) {
		...
		if (strName == "btnNo" || strName == "btnClose") {
			...
			char szData[128];
			BYTE byType = 0;
			sprintf(szData, RES_STRING(CL_LANGUAGE_MATCH_722), g_stUIMisLog.m_wMisID);
			...
			// else
			//{
			sprintf(szBuf, RES_STRING(CL_LANGUAGE_MATCH_723), szData);
			//}
...
BOOL CMisLogForm::AddNode(WORD wMisID, BYTE byState, BYTE& byType) {
	...
	char szData[128];
	byType = 0;
	sprintf(szData, RES_STRING(CL_LANGUAGE_MATCH_722), wMisID);
	...
	strData += ">";
	strData += "<r";
	// Use pixel-based dot calculation for proportional fonts
	const int nTargetPixelWidth = 140; // Target width in pixels for mission name + dots
	int nTextWidth = CGuiFont::s_Font.GetWidth(szData);
	int nDotWidth = CGuiFont::s_Font.GetWidth(".");
	int nDotsNeeded = 0;
	if (nDotWidth > 0 && nTextWidth < nTargetPixelWidth) {
		nDotsNeeded = (nTargetPixelWidth - nTextWidth) / nDotWidth;
	}
	for (int i = 0; i < nDotsNeeded; i++) {
		strData += ".";
	}
	strData += ">";
