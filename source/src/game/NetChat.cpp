...
void NetPCTeam(stNetPCTeam& pcteam) {
	...
	case TEAM_MSG_GROUP_BREAK: // ???????
	{
		...
		int nArea = 0;
		if (pScene && pScene->_pTerrain) {
			MPTile* pTile = pScene->_pTerrain->GetTile(pCha->GetCurX() / 100, pCha->GetCurY() / 100);
			if (pTile) nArea = pTile->getIsland();
		}
	...
	default:
		LG("protocol", "msgNetPCTeam unknown kind:%d", pcteam.kind);
	}
}
...
void NetFrndAdd(unsigned long cha_id, const char* cha_name, const char* motto, unsigned short icon_id, const char* grp) { // grp-???????????
...
void NetSessCreate(unsigned long newsessid, stNetSessCreate cha[], short chanum) {
	...
	if (cha[0].lChaID == g_stUIChat._dwSelfID) {
		...
		int j = 0;
		for (; j < chanum; j++) {
			if (cha[j].lChaID == g_stUIChat._dwSelfID)
				continue;
			pArrName[i] = &cha[j];
			i++;
		}
...
extern void NetSessLeave(unsigned long sessid, unsigned long chaid) {
	...
	if (g_stUIChat._dwSelfID != chaid) {
		if (!pSession) {
			// char szBuf[2048] = { 0 };
			// sprintf( szBuf, "NetSessLeave(sessid:%d, chaid:%d, selfid:%d) error, pSession==nullptr SessionCount=%d\n", sessid, chaid, g_stUIChat._dwSelfID, CTalkSessionFormMgr::GetSessionCount() );
