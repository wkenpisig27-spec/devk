...
void CGameApp::MouseButtonDown(int nButton) {
	...
	try {
		...
		LG("input_error", "Exception in TestDemo MouseButtonDown\n");
...
void CGameApp::MouseMove(int nOffsetX, int nOffsetY) {
	...
	try {
		...
		LG("input_error", "Exception in TestDemo MouseMove\n");
...
void CGameApp::MouseButtonDB(int nButton) {
	...
	try {
		...
		if (_IsSceneOk()) {
			...
			// 锟斤拷锟斤拷戏模式锟叫硷拷锟斤拷双锟斤拷锟街革拷默锟斤拷锟接斤拷
...
void CGameApp::MouseScroll(int nScroll) {
	...
	try {
		...
		LG("input_error", "Exception in TestDemo MouseScroll\n");
...
void CGameApp::HandleSuperKey() {
	...
	try {
		...
		if (g_pGameApp->IsKeyDown(DIK_C)) {
			...
			if (pScene) {
				// 锟节放的斤拷色全锟斤拷写锟诫到一锟斤拷锟侥憋拷锟侥硷拷锟斤拷
		...
		LG("input_error", "Exception in TestDemo HandleSuperKey\n");
