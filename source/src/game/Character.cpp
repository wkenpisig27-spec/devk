...
CSkillRecord* CCharacter::_pDefaultSkillInfo = nullptr;
CSkillRecord* CCharacter::_pReadySkillInfo = nullptr;
...
inline void SetPreName(int nItem, char* szName, DWORD& dwColor) {
	...
	switch (nItem) {
	case 3936:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_7));
		dwColor = D3DCOLOR_ARGB(255, 192, 192, 192);
		return;
	case 3937:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_8));
		dwColor = D3DCOLOR_ARGB(255, 000, 255, 012);
		return;
	case 3938:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_9));
		dwColor = D3DCOLOR_ARGB(255, 217, 255, 119);
		return;
	case 3939:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_10));
		dwColor = D3DCOLOR_ARGB(255, 070, 182, 240);
		return;
	case 3940:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_11));
		dwColor = D3DCOLOR_ARGB(255, 255, 207, 000);
		return;
	case 3941:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_12));
		dwColor = D3DCOLOR_ARGB(255, 255, 000, 000);
		return;
	case 3942:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_13));
		dwColor = D3DCOLOR_ARGB(255, 241, 014, 240);
		return;

	case 5331:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_922));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,192,192,192); return;
	case 5332:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_923));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,135,135,135); return;
	case 5333:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_924));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,255,255,255); return;
	case 5334:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_925));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,070,182,240); return;
	case 5335:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_926));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,000,255,012); return;
	case 5336:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_927));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,217,255,119); return;
	case 5337:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_928));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,255,207,000); return;
	case 5338:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_929));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,255,127,000); return;
	case 5339:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_930));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,255,000,000); return;
	case 5340:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_931));
		dwColor = COLOR_SKYBLUE;
		return; // D3DCOLOR_ARGB(255,241,014,240); return;

	// TOM??
	case 822:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_14));
		dwColor = D3DCOLOR_ARGB(255, 255, 000, 000);
		return; // ???????
	case 823:
		strcpy(szName, RES_STRING(CL_LANGUAGE_MATCH_15));
		dwColor = D3DCOLOR_ARGB(255, 241, 014, 240);
		return; // ??????
	default:
		szName[0] = '\0';
...
static void __keyframe_proc(DWORD type, DWORD pose_id, DWORD key_id, DWORD key_frame, void* param) {
	...
	case KEYFRAME_TYPE_BEGIN: {
		...
		if (pCha->GetCurPoseType() > 6 && pCha->GetCurPoseType() < 15) {
			...
			if (item && item->GetItemDrapID() > 0 && pCha->GetScene()) {
...
CCharacter::CCharacter()
	: _bEnableAI(FALSE),
	  _isArrive(true),
	  _nTurnCnt(0),
	  _pszFootMusic(nullptr),
	  _pszWhoopMusic(nullptr),
	  _fMaxOpacity(1.0f),
	  _pNpcStateItem(nullptr),
	  _eChaModalType(enumMODAL_MAIN_CHA),
	  _eChaCtrlType(enumCHACTRL_PLAYER),
	  _IsForUI(false),
	  _pDefaultChaInfo(nullptr),
...
int CCharacter::LoadCha(const LoadChaInfo* info) {
	...
	// PKO FIX: Null-check GetScene() before AddSceneItem during ship loading
	for (DWORD i = 0; i < 4; i++) {
		if (strlen(info->prop[i]) == 0)
			continue;

		if (!this->GetScene()) {
			LG("error", "msgLoad Ship Power System error - no scene!\n");
			return FALSE;
		}
	...
	if (bValid == FALSE) {
		...
		for (int i = 0; i < ITEM_FACE_MAX; i++) {
			if (_pItemFaceEff[i]) {
				...
				_pItemFaceEff[i] = nullptr;
			}
		}
		memset(_ItemFace, 0, sizeof(_ItemFace));

		// PKO FIX: Null-check GetScene() before HandleSceneMsg during destroy
		if (GetScene())
...
void CCharacter::FrameMove(DWORD dwTimeParam) {
	...
	if (!_isArrive) {
		...
		// PKO FIX: Null-check _pSceneHeight to prevent crash during cleanup
		if (_pSceneHeight && _pSceneHeight->FrameMove(dwTimeParam)) {
			_UpdateHeight();
	...
	// Safety check: ensure actor exists before calling FrameMove
	if (_pActor) {
...
void CCharacter::Render() {
	...
	if (!_pScene || !_pScene->IsPointVisible((float)_nCurX / 100.0f, (float)_nCurY / 100.0f))
		return;

	// Skip rendering orphaned mounts (owner gone but mount still in array)
	if (GetIsMount() && !GetMountOwner())
		return;
...
int CCharacter::FaceTo(int nAngle) {
	if (!_pDefaultChaInfo || !_pDefaultChaInfo->IsFace())
		return getYaw();
	...
	if (IsBoat()) {
		_nAngleStep = (int)(600.0f / (float)g_Render.GetFPS());
	} else {
		_nAngleStep = (int)(1200.0f / (float)g_Render.GetFPS());
		if (_nAngleStep <= 0)
			_nAngleStep = 1;
	}
...
bool CCharacter::ItemEffect(int nEffectID, int nItemDummy, int nAngle) {
	...
	if (item->GetObjDummyRunTimeMatrix(&mat, nItemDummy) >= 0) {
		...
		if (nAngle != 999)
			pEffect->SetEffectDir(nAngle);
		pEffect->Emission(-1, &GetPos(), nullptr);
	}
...
CEffectObj* CCharacter::SkyEffect(int nEffectID, int nBeginDummy, int nItemDummy, int nSpeed, D3DXVECTOR3* pTarget, int nTargetChaID, CSkillRecord* pSkill) {
	...
	if (pSkill) {
		...
		case enumRANGE_TYPE_SQUARE: // ???S???????????
		case enumRANGE_TYPE_CIRCLE: // ??S?????
			break;
		case enumRANGE_TYPE_FAN: // ???S?????????
		{
			pEffect->SetFanAngle(pSkill->GetParam()[1]);
			pEffect->SetMagicDist((float)pSkill->GetParam()[0] / 100.0f);
		}
		case enumRANGE_TYPE_STICK: // ???S???????????
		{
			// ??????????????????????????????????????????????????????k?????
			int x, y;
...
bool CCharacter::ChangeReadySkill(int nSkillID) {
	...
	if (!p) {
		g_pGameApp->AddTipText(RES_STRING(CL_LANGUAGE_MATCH_17), "Unknown");
		return false;
	}

	if (_pReadySkillInfo && _pReadySkillInfo->sID == nSkillID)
		return true;
...
void CCharacter::_UpdateHeight() {
	...
	SetPos((float*)&_vPos); // ????????�??????, _vPos.x??_vPos.y??????
	if (GetDrunkState()) {
		UpdateChaMatrix();
	}
}
...
void CCharacter::UpdateTileColor() {
	...
	// PKO FIX: Null-check _pScene before accessing terrain
	if (!_pScene) return;
...
void CCharacter::setSideID(long v) {
	...
	if (_lSideID != 0 && IsPlayer() && _pScene) {
		...
		if (_pSideShade) {
			...
			_pSideShade->Emission(0, &GetPos(), nullptr);
...
bool CCharacter::PlayPose(DWORD pose, DWORD type, int time, int fps, bool isBlend, bool IsGlitched) {
	...
	if (GetIsFly()) {
		if (pose == POSE_WAITING || pose == POSE_WAITING2)
			pose = POSE_FLY_WAITING; // ???->????
		if (pose == POSE_RUN || pose == POSE_RUN2)
			pose = POSE_FLY_RUN; // ???->T??
		if (pose == POSE_SHOW)
			pose = POSE_FLY_SHOW; // ???->??????
		if (pose == POSE_SEAT)
		...
		// PKO FIX: Null-check GetScene() before HandleSceneMsg in PlayPose
		if (GetScene())
...
bool CCharacter::SpawnMount(int mountID) {
	...
	if (!chaMount && pInfo) {
		...
		if (chaMount) {
			...
			// Set mount initial position to owner's position to prevent flash at (0,0)
			chaMount->setPos(_nCurX, _nCurY);
			chaMount->setYaw(getYaw());
		} else {
			return false;
		}
	}
...
bool CCharacter::DespawnMount() {
	if (chaMount) {
		chaMount->SetMountOwner(nullptr);
...
bool CCharacter::UpdataItem(int nItem, DWORD nLink) {
	if (enumMODAL_MAIN_CHA != _eChaModalType)
		return false;
	switch (nLink) {
...
// PKO FIX: Null-check _pScene before GetFirstInvalidEffObj
					CEffectObj* pEffect = _pScene ? _pScene->GetFirstInvalidEffObj() : nullptr;
						if (pEffect) {
		...
		if (nItem > 0 && nItem != enumEQUIP_BOTH_HAND) {
			...
			if (!_pHandItem[nLink] || _pHandItem[nLink]->GetItemInfo()->lID != nItem) {
				if (_pHandItem[nLink]) {
					...
					_pHandItem[nLink] = nullptr;
				}

				// PKO FIX: Null-check _pScene
				CSceneItem* item = _pScene ? _pScene->AddSceneItem(nItem, getTypeID()) : nullptr;
				if (item) {
...
void CCharacter::setNpcState(DWORD dwState) {
	...
	// PKO FIX: Null-check _pScene before AddSceneItem calls in setNpcState
	if (!_pScene) return;
...
void CCharacter::DieTime() {
	...
	int id;
	CEffectObj* pEff;
	// PKO FIX: Null-check GetScene() during DieTime cleanup
	for (int n = 0; n < size && GetScene(); n++) {
		id = GetEffectID(n);
		if (id < 0)
			continue;
...
void CCharacter::SynchroSkillState(stSkillState* pState, int nCount) {
	...
	if (set.IsTrue(enumChaStateNoHide) != _ChaState.IsTrue(enumChaStateNoHide)) {
		if (set.IsTrue(enumChaStateNoHide))
			SetOpacity(_fMaxOpacity);
		else
			SetOpacity(_fMaxOpacity / 2.0f);
	}
...
void CCharacter::RefreshShopShop() {
	...
	if (g_IsShowModel && _pScene && _ChaState.IsFalse(enumChaStateNoShop)) {
		if (g_IsShowShop) {
			switch (CChaStateMgr::GetLastShopLevel()) {
			case 1:
				// PlayPose( POSE_SEAT2, PLAY_LOOP_SMOOTH );
...
void CCharacter::SetHide(BOOL bHide) {
	...
	if (_pNpcStateItem)
		_pNpcStateItem->SetHide(bHide);

	if (_pShopItem)
		_pShopItem->SetHide(bHide);

	if (_pSideShade)
		_pSideShade->SetHide(bHide);

	// PKO FIX: Null-check GetEffect/GetShadeObj returns to prevent crash
	for (i = 0; i < GetEffectNum(); i++) {
		CEffectObj* pEff = GetScene() ? GetScene()->GetEffect(GetEffectID(i)) : nullptr;
		if (pEff) pEff->SetHide(bHide);
	}
	for (i = 0; i < GetShadeNum(); i++) {
		CShadeEff* pShd = GetScene() ? GetScene()->GetShadeObj(GetShadeID(i)) : nullptr;
		if (pShd) pShd->SetHide(bHide);
	}
	if (_pBoatFog) {
...
void CCharacter::RefreshFog() {
	if (!_pDefaultChaInfo->HaveEffectFog())
		return;
...
void CCharacter::SetItemFace(unsigned int nIndex, int nItem) {
	...
	if (nIndex >= ITEM_FACE_MAX)
		return;

	if (nItem == _ItemFace[nIndex])
		return;
	...
	if (_pItemFaceEff[nIndex]) {
		...
		_pItemFaceEff[nIndex] = nullptr;
	}

	CItemRecord* pInfo = GetItemRecordInfo(nItem);
	if (!pInfo || pInfo->sItemEffect[0] == 0)
		return;
...
void CCharacter::_computeLinkedMatrix() {
	if (this->GetIsOnMount() && this->GetIsMountEquipped() && !this->GetIsForUI()) {
		...
		CItemRecord* pItemRec = GetItemRecordInfo(this->GetIsMountEquipped());
		if (!pItemRec) {
			return;
		}
		CMountInfo* pMount = GetMountInfo(pItemRec->szDataName);
...
MountData* CCharacter::GetMountData(){
	...
	data->offsetY = 0;
	data->boneID = 0;
	if(!GetIsOnMount() || !GetIsMountEquipped()) return nullptr;
...
// CLOAKGlow feature implementation
...
void CCharacter::RenderCloakGlow() {
	if (GetPart().SLink[enumEQUIP_CLOAK].sID != 0) {
		if (!CLOAKGlow[enumEQUIP_CLOAK] && CloakprevLevel == 0 && IsPlayer()) {
			UpdataItem(9604, enumEQUIP_CLOAK);
		}
	}
}
