...
bool CPickState::_Start() {
	if (!_pItem || !_pItem->IsValid() || !_pItem->IsPick())
		return false;
...
CEventState::CEventState(CActor* p)
	: CActionState(p), _pNode(nullptr), _pEvent(0) {
	_IsOver = true; // 动作直接结束了
}

bool CEventState::_Start() {
	if (!_pNode || !_pNode->IsValid())
		return false;
