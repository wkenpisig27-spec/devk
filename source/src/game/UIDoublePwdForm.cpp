...
void CDoublePwdMgr::_evtCreateFromMouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	if (strName == "btnYes") {
		...
		if (!IsPwdValid(g_stUIDoublePwd.edtDoublePwdCreate->GetCaption())) {
			// 密码只能是数字
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_797)); //"二次密码只能有 0 ~ 9 数字组成，请重新输入"
		...
		if (6 > nPwdLen || 12 < nPwdLen) {
			// 密码长度不在 6 ~ 12 之间
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_798)); //"二次密码长度不在 6 ~ 12，请重新输入"
		...
		if (0 != strcmp(g_stUIDoublePwd.edtDoublePwdCreate->GetCaption(),
			...
			// 两次密码不同
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_799)); //"两次输入的密码不同，请重新输入"
