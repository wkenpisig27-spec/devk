...
#include <fstream>
#include <iostream>
CSceneObjSet* CSceneObjSet::_Instance = nullptr;
...
void CSceneObj::Render() {
	if (_dwCullingFlag) {
		...
		for (DWORD i = 0; i < pri_num; i++) {
			...
			if (box == nullptr || box->GetObjNum() == 0) {
			...
			// PKO FIX: Null-check GetScene() before GetTerrain()
			MPTerrain* pTerrain = GetScene() ? GetScene()->GetTerrain() : nullptr;
			if (pTerrain) {
			...
			if (bInTerrainRange) {
				// PKO FIX: Null-check _pScene before IsSceneObjCulling
				if (_pScene && _pScene->IsSceneObjCulling() == FALSE) {
...
void CSceneObj::FrameMove(DWORD dwTimeParam) {
	// Update scene objects every frame for smooth 60 FPS animation
	// Time-based animation is handled by g_fFrameRate multiplier in lwPoseCtrl
