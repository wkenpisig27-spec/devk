...
#define ERR_CHK(p, s)                                       \
	{                                                       \
		if (p == 0) {                                       \
			LG("init", "msgShipFactory Load error: %s", s); \
		}                                                   \
	}
...
BOOL xShipFactory::CheckShipName() {
	...
	if (l == 0) {
		...
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_391), MIN_LENGTH, MAX_LENGTH);
		return 0;
	}
	...
	if (!CTextFilter::IsLegalText(CTextFilter::NAME_TABLE, sBoatName) ||
		!common::conformity::character::name::is_valid(sBoatName.c_str(), (unsigned short)sBoatName.length())) {
...
void xShipLaunchList::Update(DWORD num, const BOAT_BERTH_DATA* data,
	...
	memo->SetCaption(RES_STRING(CL_LANGUAGE_MATCH_393));
