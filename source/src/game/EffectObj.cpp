...
void (*OpertionList[])(CMagicEff* pEffCtrl) = {
    Part_bind, Part_follow, Part_foldir,
    // Part_drop,
    // Part_fly,
    // Part_trace,
    // Part_fshade,
...
namespace {
constexpr int kOpertionCount = (int)(sizeof(OpertionList) / sizeof(OpertionList[0]));
constexpr int kMagicCount = (int)(sizeof(MagicList) / sizeof(MagicList[0]));
constexpr int kGroupCount = (int)(sizeof(GroupList) / sizeof(GroupList[0]));

void TraceEff(const char* step) {
...
void BindOpertion(CMagicEff* eff, int idx) {
	if (!eff)
		return;
	if (idx >= 0 && idx < kOpertionCount)
		eff->RenderUpdate = OpertionList[idx];
	else
		eff->RenderUpdate = NULL;
}

void BindMagicUpdate(CMagicCtrl* ctrl, int idx) {
	if (!ctrl)
		return;
	if (idx >= 0 && idx < kMagicCount)
		ctrl->MagicUpdate = MagicList[idx];
	else
		ctrl->MagicUpdate = NULL;
}

void BindGroupEmission(CMagicEff* eff, int idx) {
	if (!eff)
		return;
	if (idx >= 0 && idx < kGroupCount)
		eff->GroupEmission = GroupList[idx];
	else
		eff->GroupEmission = NULL;
}
} // namespace
...
inline void Part_follow(CMagicEff* pEffCtrl) {
	...
	case 1: // ��ɫ����Ч
		...
		if (pCha->IsBoat()) {
			...
			tpos = pCha->GetPos();
			// If character is mounted, use ground-level Z from the mount
			// instead of the rider's elevated Z position
			if (pCha->GetIsOnMount()) {
				CCharacter* pMount = pCha->GetMount();
				if (pMount) {
					tpos.z = pMount->GetPos().z;
				}
			}
...
BOOL CMagicEff::Create(int iIdxID) {
	...
	if (!pInfo) {
		TraceEff("create: no magicinfo");
		return FALSE;
	}

	if (iIdxID == 132) {
		char buf[256];
		sprintf(buf, "create 132 name=%s dummy0=%d dummy2=%d eff=%d obj=%d",
			pInfo->szDataName, pInfo->nDummy[0], pInfo->nDummy2, pInfo->nEffType, pInfo->nObjType);
		TraceEff(buf);
	}


	if (getIdxID() == iIdxID && _pEffCtrl) {
		_bDail = false;
		_fsCurTime = 0;
		_fsDailTime = 0;
		_isID = -1;
	...
	if (iIdxID == 132)
		TraceEff("create 132 CopyPartCtrl");
	if (!Create(&Property, &ResMgr)) {
		if (iIdxID == 132)
			TraceEff("create 132 Create(property) failed");
		Clear();
		return FALSE;
	}
	if (iIdxID == 132)
		TraceEff("create 132 BindingRes done");
...
BOOL CMagicEff::Create(Eff_Property* pProperty, CMPResManger* pCResMagr) {
	...
	TraceEff("create prop GetPartCtrlID");
	int id = pCResMagr->GetPartCtrlID(pProperty->m_strName);
	if (id < 0) {
		LG("error", "msgCan't find eff file [%s]", pProperty->m_strName.c_str());
		TraceEff("create prop missing par");
		return FALSE;
	}

	_pEffCtrl->Clear();
	TraceEff("create prop GetPartCtrlByID");
	CMPPartCtrl* tctrl = pCResMagr->GetPartCtrlByID(id);
	if (!tctrl) {
		TraceEff("create prop load par failed");
		return FALSE;
	}
	TraceEff("create prop CopyPartCtrl");
	_pEffCtrl->CopyPartCtrl(tctrl);
	TraceEff("create prop BindingRes");
	_pEffCtrl->BindingRes(pCResMagr);
	TraceEff("create prop BindingRes done");
	_pDailTime = pCResMagr->GetDailTime();

	BindOpertion(this, pProperty->m_iIdxRender);
	return TRUE;
}
...
BOOL CMagicEff::CreateMagic(int iIdxID) {
	...
	if (getIdxID() == iIdxID) {
		if (_pMagicCtrl.empty() || !_pMagicCtrl[0])
			return FALSE;
		BindMagicUpdate(_pMagicCtrl[0], _pMagicCtrl[0]->GetRenderIdx());
	...
	BindMagicUpdate(_pMagicCtrl[0], id);
	_pMagicCtrl[0]->Reset();
	_bMagic = TRUE;
	return TRUE;
}
...
BOOL CMagicEff::CreateGroupMagic(int iIdxID) {
	...
	if (getIdxID() == iIdxID) {
		for (WORD n = 0; n < (WORD)_pMagicCtrl.size(); ++n) {
			BindMagicUpdate(_pMagicCtrl[n], _pMagicCtrl[n]->GetRenderIdx());
			_pMagicCtrl[n]->Reset();
		}
		_iOwnerID = -1;
		BindGroupEmission(this, _iGroupIdx);
		return TRUE;
	}
	Clear();
	...
	for (n = 0; n < sGroupParam.nClass; ++n) {
		for (m = 0; m < sGroupParam.vecNum[n]; ++m) {
			...
			BindMagicUpdate(_pMagicCtrl[idx], id);
...
void CMagicEff::setFollowObj(CSceneNode* pObj, NODE_TYPE eType, int iDummy, int iDummy2) {
	...
	if (_pEffCtrl && _pEffCtrl->GetPartCtrl() && _pEffCtrl->GetPartCtrl()->GetStripNum() > 0) {
		RenderUpdate = NULL;
	...
	} else if (eType == NODE_ITEM && _pEffCtrl) {
		CSceneItem* pItem = (CSceneItem*)pObj;
...
void CMagicEff::Render() {
	...
	if (_bMagic || _bGroupMagic) {
		...
		for (INT n = 0; n < (INT)_pMagicCtrl.size(); ++n) {
			if (_pMagicCtrl[n]->IsPlaying()) {
				if (_pTerrain)
					if (_pTerrain->IsPointVisible(_pMagicCtrl[n]->GetPos()->x, _pMagicCtrl[n]->GetPos()->y)) {
						bov = false;
					}
			}
		}
	...
	if (_pTerrain)
		if (!_pTerrain->IsPointVisible(_vPos.x, _vPos.y)) {
			if (!_pEffCtrl || !_pEffCtrl->GetPartCtrl() || !_pEffCtrl->GetPartCtrl()->IsPlaying()) {
				SetValid(FALSE);
				return;
			}
			if (_bMagicEm && !_pObj)
...
BOOL CMagicEff::HitTestMap(D3DXVECTOR3* vPos) {
	...
	float fz = pScene->GetGridHeight(vPos->x, vPos->y);
	if (vPos->z < fz) {
		return TRUE;
	}
	return FALSE;
...
BOOL CMagicEff::HitTestMap() {
	...
	if (!pScene) return FALSE;
	_vMapTarget = _vPos;
	_vMapTarget.z = pScene->GetGridHeight(_vMapTarget.x, _vMapTarget.y);
...
void CMagicEff::_UpdatePos() {
	...
	// PKO FIX: Null-check _pScene to prevent crash during map transitions
	_fHei = _pScene ? _pScene->GetGridHeight(fX, fY) : 0.0f;
...
void CMagicEff::Emission(int iID, D3DXVECTOR3* vBegin, D3DXVECTOR3* vEnd, int iTime) {
	try {
		...
		if (!_pEffCtrl || !_pEffCtrl->GetPartCtrl()) {
			_bPlay = FALSE;
			return;
		}
		...
		if (_bMagicEm) {
			...
			if (_iIdxID == 132)
				TraceEff("emission 132 Play done");
		} else {
			_pEffCtrl->GetPartCtrl()->Play(0);
			_pEffCtrl->GetPartCtrl()->SetPlayType(0);
		}
		...
		LG("error", "CMagicEff::Emission exception id=%d\n", _iIdxID);
		_bPlay = FALSE;
	}
}
...
bool CShadeEff::Create(CShadeInfo* pInfo) {
	...
	if (!CMPShadeCtrl::Create(str, &ResMgr, pInfo->fsize, pInfo->nAni != 0, pInfo->nRow, pInfo->nCol))
		return false;
...
bool CShadeEff::Create(s_string strTexName, float fSize, bool bAni, int iRow, int iColnum) {
	_bUpSea = false;
	SetHide(FALSE);
...
bool CShadeEff::CreateAttachLight(int iIdxID, float fRange, D3DXCOLOR dwcolor) {
	...
	if (!pInfo)
		return false;
	std::string str = pInfo->szDataName;
	if (!CMPShadeCtrl::Create(str, &ResMgr, fRange, pInfo->nAni != 0, pInfo->nRow, pInfo->nCol))
		return false;
	if (pInfo->nUseAlphaTest) {
		// ʹ��AlphaTest
		SetRenderIndex(4);
