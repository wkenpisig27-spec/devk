...
void CGuildChallengeMgr::ChargeMoney(int iIndex) {
	...
	if (m_lChargeMoney[iIndex] == -1) {
		if (GetChallengeMasterIndex(pMainCha->getGuildName()) != -1) {
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_585));
			return;
		}
	} else {
		...
		if (iMasterIndex != -1) {
			...
			if (iIndex > iMasterIndex) {
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_587));
				return;
			}
		}
	}
	...
	char szBuf[64];
	stNumBox* numBox = CBoxMgr::ShowNumberBox(_enterChargeMoney, -1, RES_STRING(CL_LANGUAGE_MATCH_589), false);
	sprintf(szBuf, "%lld", iNextCharge);
...
bool CGuildChallengeMgr::IsValid(int iIndex, long lMoney) {
	if (m_lChargeMoney[iIndex] == -1) {
		...
		long long lMinMoney = m_lChargeMoney[iIndex] + CGuildChallengeMgr::CHARGE_MONEY;
		if (lMoney < lMinMoney) {
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_591));
			return false;
		}
	}

	if (lMoney >= 100000000000LL) {
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_592));
		return false;
	}
	return true;
}
