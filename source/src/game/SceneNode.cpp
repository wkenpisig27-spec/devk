...
CSceneNode::CSceneNode()
	: _dwID(0), _dwTypeID(0), _dwAttachID(0), _dwFrameCnt(0), _dwRenderCnt(0), _bValid(0), _nCurX(0), _nCurY(0), _nYaw(0), _nPitch(0), _nRoll(0), _bHide(0), _pScene(nullptr), _nHeightOff(0), _nPoseHeightOff(0), _nSize(0), lTag(0) {
	_iEffNum = 0;
...
void CSceneNode::SetShadeShow(int iIdx, bool bShow) {
	...
	// PKO FIX: Null-check _pScene before accessing shade objects
	if (!_pScene) return;
...
void CSceneNode::SetEffectShow(int iIdx, bool bShow) {
	...
	// PKO FIX: Null-check _pScene before accessing effect objects
	if (!_pScene) return;
