...
static CActionState* g_state = nullptr;
// CLargerMap* CGameScene::_pLargerMap			= nullptr;

inline static CCharacter* GetCharacter(unsigned int nID, const char* error = nullptr) {
	...
	if (!pCha && error) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_247), nID, error);
	}
	return pCha;
}
...
void NetLoginSuccess(char byPassword, char iCharNum, NetChaBehave chabehave[]) {
	...
	if (!pScene) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_249));
		return;
	}
...
void NetLoginFailure(unsigned short Errno) {
	...
	CLoginScene* pScene = dynamic_cast<CLoginScene*>(CGameApp::GetCurScene());
	if (!pScene) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_250));
		return;
	}
	...
	case ERR_AP_INVALIDPWD: {
		...
		if (dwCount >= 3) {
			...
			if (dwLast - dwFirst <= 60 * 1000) {
				g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_251));
	...
	case ERR_AP_BANUSER: {
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_252));
		return;
	}
	case ERR_AP_INVALIDUSER: {
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_253));
		return;
	}
...
void NetBeginPlay(unsigned short Errno) // ??NetRetCode.h?l?????errno????
{
...
void NetNewCha(unsigned short Errno) // ??NetRetCode.h?l?????errno????
{
	//  LG( "select", "NetNewCha - Errno:%d, Info:%s\n", Errno, g_GetServerError(Errno) );

	CCreateChaScene& rkScene = CCreateChaScene::GetCurrScene();
	// CWorldScene& rkScene = CWorldScene::GetCurrScene();
	if (Errno == 0) { // Success create new character.
...
void NetDelCha(unsigned short Errno) // ??NetRetCode.h?l?????errno????
{
	//  LG( "select", "NetDelCha - Errno:%d, Info:%s\n", Errno, g_GetServerError(Errno) );

	CSelectChaScene* pScene = dynamic_cast<CSelectChaScene*>(CGameApp::GetCurScene());
	if (!pScene) {
		LG("protocol", "NetDelCha fail.");
		return;
	}
...
void NetCreatePassword2(unsigned short Errno) {
	...
	if (Errno == ERR_SUCCESS) {
		...
		// ??�
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_172), "");
	} else if (Errno == ERR_PT_INVALID_PW2) {
		// ?????????????????h?????????
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_801));
	} else if (Errno == ERR_PT_KICKUSER) {
		// Player session invalid - need to reconnect
		g_pGameApp->MsgBox("Session expired. Please reconnect.");
	} else if (Errno == ERR_MC_NETEXCP) {
		// Network/server communication error
		g_pGameApp->MsgBox("Server communication error. Please try again.");
	} else {
		// d????? - show error code for debugging
		char errMsg[128];
		_snprintf_s(errMsg, sizeof(errMsg), _TRUNCATE, "Error code: %d", Errno);
		g_pGameApp->MsgBox(errMsg);
	}
}
...
void NetUpdatePassword2(unsigned short Errno) {
	...
	if (Errno == ERR_SUCCESS) {
		...
		// d?????
		g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_375));
	}
}
...
void NetActorMove(unsigned int id, stNetNotiMove& list) {
	...
	if (false && cha && cha->getAttachID() == id) {
		...
		if (!g_state) {
			LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_258));
			return;
		}
	} else {
		cha = CGameApp::GetCurScene()->SearchByID(id);
		if (!cha) {
			LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_259), id, list.nPointNum, list.SPos[0].x, list.SPos[0].y);
			return;
		}
		...
		if (!g_state) {
			...
			if (!cha->GetActor()->SwitchState(g_state)) {
				LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_260), cha->GetActor()->GetState(), cha->getLogName(), list.nPointNum, list.sState, list.SPos[1].x, list.SPos[1].y, GetTickCount());
				return;
			}
		}
	}
	...
	if (!g_state) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_261), cha->getLogName(), list.nPointNum, list.sState, list.SPos[1].x, list.SPos[1].y, GetTickCount());
		return;
	}
	...
	if (list.sState) {
		if (list.nPointNum > 1) {
			...
			LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_262), cha->getLogName(), list.nPointNum, list.sState, list.SPos[1].x, list.SPos[1].y, GetTickCount());
		}
	}
}
...
CCharacter* stNetActorCreate::CreateCha() {
	...
	if (chSeeType == enumENTITY_SEEN_SWITCH) {
		...
		if (pCha) {
			...
			LG("NetActorCreate", RES_STRING(CL_LANGUAGE_MATCH_263));
		}
		return pCha;
	}
	...
	if (p) {
		LG("NetActorCreate", RES_STRING(CL_LANGUAGE_MATCH_264), szName, ulWorldID, p->getLogName());
		p->SetValid(FALSE);
	}
	...
	if (chMainCha) {
		if (chMainCha == enumMainPlayer) {
			...
			if (!g_stUIBoat.AddBoat(p)) {
				LG("error", "g_stUIBoat.AddBoat(p) Failed! WorldID:%u, Name:%s\n", ulWorldID, szName);
			} else {
				LG("SC_Cha", "g_stUIBoat.AddBoat(p) Success. WorldID:%u, Name:%s\n", ulWorldID, szName);
			}
			if (!g_stUIBoat.GetHuman()) {
				LG("error", "g_stUIBoat.GetHuman() is NULL after adding boat! WorldID:%u\n", ulWorldID);
			}
		}
		p->SetHide(TRUE);
	...
	if (chMainCha != 2) {
		...
		case enumEXISTS_NATALITY: {
			...
			if (pEffect && pEffect->Create(p->GetDefaultChaInfo()->sBornEff)) {
				pEffect->Emission(-1, &p->GetPos(), nullptr);
...
void NetActorDestroy(unsigned int nID, char chSeeType) {
	...
	if (chSeeType == enumENTITY_SEEN_SWITCH) {
		...
		if (pCha) {
			...
			LG("NetActorCreate", RES_STRING(CL_LANGUAGE_MATCH_265));
		}
		return;
	}
...
void NetSynSkillState(DWORD dwCharID, stNetSkillState* pSSkillState) {
	...
	if (pCha) {
		...
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_266), dwCharID);
	}
}
...
void NetActorSkillRep(unsigned int nID, stNetNotiSkillRepresent& SSkillRep) {
	...
	CCharacter* pCha = CGameApp::GetCurScene()->SearchByID(nID);
	if (!pCha) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_267), SSkillRep.lSkillID, nID);
		return;
	}

	static CWaitAttackState* state = nullptr;
	static CActionState* g_state = nullptr;

	if (pCha->IsMainCha()) {
		state = dynamic_cast<CAttackState*>(pCha->GetActor()->GetCurState());
		if (!state) {
			LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_268));
			return;
		}
...
void NetActorLean(unsigned int nID, stNetLeanInfo& lean) {
	...
	if (lean.chState == 0) {
		...
		if (cha && cha->getAttachID() == nID) {
			...
			if (!cha) {
				LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_270), nID);
				return;
			}
...
void NetSwitchMap(stNetSwitchMap& switchmap) {
	...
	// PKO FIX: Removed dangerous memory stress test that caused heap corruption during map transitions
	// Previously: static char* pMemory = new char[1]; delete[] pMemory; pMemory = new char[rand() % 777 + 1];
	...
	if (switchmap.sEnterRet != ERR_SUCCESS) {
		...
		{
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_271));
			CGameApp::Waiting(false);
		} else {
...
void NetSay(stNetSay& netsay, DWORD dwColour) // �??
{
	...
	if (!cha->IsPlayer()) {
		LG("roadsay", RES_STRING(CL_LANGUAGE_MATCH_272), netsay.m_srcid, cha->getName(), cha->getLogName(), netsay.m_content);
	}
...
CSceneItem* NetCreateItem(stNetItemCreate& info) {
	...
	if (pItem) {
		LG("NetCreateItem", RES_STRING(CL_LANGUAGE_MATCH_273), info.lID, pItem->GetItemInfo()->szName);
		pItem->SetValid(FALSE);
	}

	pItem = pScene->AddSceneItem(info.lID, 0);
	if (pItem == nullptr) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_274), info.lID);
		return nullptr;
	}
	...
	pItem->PlayObjImpPose(ANIM_CTRL_TYPE_MAT, 0, PLAY_LOOP, 0.0f,
		2.0f / CSteadyFrame::GetAnimMultiplier());
...
void NetChangeChaLookEnergy(unsigned int nID, stLookEnergy& SLookEnergy) {
	...
	LG(pszLogName, RES_STRING(CL_LANGUAGE_MATCH_275));
...
void NetQueryRelive(unsigned int nID, stNetQueryRelive& SQueryRelive) {
	LG(g_LogName.GetMainLogName(), RES_STRING(CL_LANGUAGE_MATCH_276), nID, GetTickCount());
...
void NetMapMask(unsigned int nID, BYTE* pMask, long lLen) {
	...
	if (pMask) {
		...
		g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_277));
	}
...
void NetActorChangeCha(unsigned int nID, stNetChangeCha& SChangeCha) {
	...
	// log
	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_278), GetTickCount());
...
void NetShowBoatList(DWORD dwNpcID, BYTE byNumBoat, const BOAT_BERTH_DATA& Data, BYTE byType) {
	((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->SetType(byType);
	((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->SetNpcID(dwNpcID);
	...
	if (byType == mission::BERTH_LUANCH_LIST) {
		...
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eTrade);
	} else if (byType == mission::BERTH_BAG_LIST) {
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eBag);
	} else if (byType == mission::BERTH_REPAIR_LIST) {
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eRepair);
	} else if (byType == mission::BERTH_SALVAGE_LIST) {
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eSalvage);
	} else if (byType == mission::BERTH_SUPPLY_LIST) {
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eSupply);
	} else if (byType == mission::BERTH_BOATLEVEL_LIST) {
		((CWorldScene*)g_pGameApp->GetCurScene())->GetShipMgr()->_launch_list->Update(byNumBoat, &Data, xShipLaunchList::eUpgrade);
	}
}
...
void NetTradeAddBoat(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex,
	...
	if (byOpType == mission::TRADE_DRAGTO_ITEM) {
		...
		// ??????????
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, byCount, byItemIndex, nullptr, &Data);
	}
}
...
void NetTradeAddItem(DWORD dwCharID, BYTE byOpType, USHORT sItemID, BYTE byIndex,
	...
	if (byOpType == mission::TRADE_DRAGTO_ITEM) {
		...
		g_stUITrade.DragItemToTrade(dwCharID, sItemID, byIndex, byCount, byItemIndex, &sGrid, nullptr);
	}
}

void NetTradeShowMoney(DWORD dwCharID, long long llMoney) {
	g_stUITrade.ShowCharTradeMoney(dwCharID, llMoney);
}
...
void NetStallAddBoat(BYTE byGrid, USHORT sItemID, BYTE byCount, __int64 llMoney, NET_CHARTRADE_BOATDATA& Data) {
	g_stUIBooth.AddTradeBoothBoat(byGrid, sItemID, byCount, llMoney, Data);
}

void NetStallAddItem(BYTE byGrid, USHORT sItemID, BYTE byCount, __int64 llMoney, NET_CHARTRADE_ITEMDATA& Data) {
	SItemGrid sGrid;
	memset(&sGrid, 0, sizeof(sGrid));
	sGrid.sID = sItemID;
	sGrid.sNum = byCount;
	...
	g_stUIBooth.AddTradeBoothGood(byGrid, sItemID, byCount, llMoney, sGrid);
}
...
void NetSynAttr(DWORD dwWorldID, char chType, short sNum, stEffect* pEffect) {
	...
	if (!pCha) {
		if (enumATTRSYN_INIT == chType)
			LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_279), dwWorldID);
		return;
	}
...
void NetFace(DWORD dwCharID, stNetFace& netface, char chType) {
	if (enumACTION_SKILL_POSE == chType) {
		...
		if (pCha == CGameApp::GetCurScene()->GetMainCha())
			return;
...
void NetChangeKitbag(DWORD dwChaID, stNetKitbag& SKitbag) {
	...
	if (count > grd->GetMaxNum()) {
		count = grd->GetMaxNum();
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_280), SKitbag.nGridNum, grd->GetMaxNum());
	}

	CItemRecord* item = nullptr;
	int nMarginNum = 0;
	...
	for (int i = 0; i < count; i++) {
		...
		if (pGrid[i].SGridContent.sID > 0) {
			if (chType == enumSYN_KITBAG_EQUIP) {
				...
				// Handle same-slot replacement during equip swap.
				// When an item is equipped from the fast bar, the previously equipped
				// item may land in the same inventory slot (in-place replacement).
				// In that case there's no sID==0 removal entry, so we must capture
				// the old item's fast command indexes here before SetItem() destroys it.
				// Note: We do NOT call DelFastCommand() here — the old object's destructor
				// (~CCommandObj) will auto-clear all fast slot pointers via
				// CFastCommand::DelCommand(this) when SetItem() deletes it below.
				// Calling DelFastCommand would send an unnecessary shortcut-clear packet.
				if (fastCommandSlot[0] == -1 && grd == g_stUIEquip.GetGoodsGrid()) {
					CCommandObj* pOldObj = grd->GetItem(pGrid[i].sGridID);
					if (pOldObj) {
						CItemCommand* pOldItem = dynamic_cast<CItemCommand*>(pOldObj);
						CItemRecord* pNewItemRec = GetItemRecordInfo(pGrid[i].SGridContent.sID);
						if (pOldItem && pNewItemRec && pOldItem->GetItemInfo() != pNewItemRec) {
							CFastCommand::FindFastCommandIndexes(pOldObj, fastCommandSlot);
						}
					}
				}
			}
			...
			if (!item) {
				LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_281), pGrid[i].SGridContent.sID);
				continue;
			}
			...
			if (pObj && pObj->GetItemInfo() == item) {
				...
				if (!grd->SetItem(pGrid[i].sGridID, pObj)) {
					LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_282), item->szName, pGrid[i].sGridID);
					continue;
				}
			}
			...
			// Sync color state with server's bValid to fix reused objects
			// retaining stale _dwColor (e.g. red from booth SetIsValid(false))
			pObj->SetIsValid(pGrid[i].SGridContent.IsValid());
			// pObj->SetPrice(pGrid[i].SGridContent)
			nMarginNum = pObj->GetTotalNum() - nMarginNum;

			if (SKitbag.chBagType != 1 && nMarginNum > 0) // modify by Philip.Wu  2006-06-21  ???0????BUG????
			{
				switch (chType) {
				case enumSYN_KITBAG_PICK:
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_283), item->szName, nMarginNum);
					break;
				// case enumSYN_KITBAG_FROM_NPC:	g_pGameApp->SysInfo("NPC??[%s ?? %d]!", item->szName, nMarginNum );	break;
				case enumSYN_KITBAG_SYSTEM:
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_284), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_TRADE:
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_285), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_FORGES:
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_286), item->szName, nMarginNum);
					break;
				case enumSYN_KITBAG_FORGEF:
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_287), item->szName, nMarginNum);
					break;
				}
			}
		} else {
			...
			if (!grd->DelItem(pGrid[i].sGridID)) {
				...
				case enumSYN_KITBAG_SWITCH:
					break;
				default:
					LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_288), pGrid[i].sGridID);
				}
				continue;
			}
		}
	...
	if (grd == g_stUIGuildBank.GetBankGoodsGrid())
...
void NetKitbagLockedSpaces(short slots, CGoodsGrid* grd) {
	...
	// Remove any existing locked placeholder items
	for (int index = 0; index < grd->GetMaxNum(); index++) {
	...
	// Grid is always at least 8 rows (48 slots visible). Grows when capacity exceeds 48.
	const int cols = 6;
	int minRows = 8;
	int neededRows = (slots + cols - 1) / cols;
	int rows = neededRows > minRows ? neededRows : minRows;
	int totalSlots = rows * cols;

	grd->SetContent(rows, cols);
	grd->Refresh();

	// Place lock icons from capacity to end of visible grid
	for (int index = slots; index < totalSlots; index++) {
...
void NetTriggerAction(stNetNpcMission& info) {
	...
	case mission::TE_GET_ITEM: {
		...
		g_pGameApp->ShowMidText(RES_STRING(CL_LANGUAGE_MATCH_289), szData, info.sCount, info.sNum);
	} break;
	...
	default: {
		g_pGameApp->ShowMidText(RES_STRING(CL_LANGUAGE_MATCH_290), info.sID, info.sCount, info.sNum);
	} break;
	}
}
...
void NetFailedAction(char chState) {
	...
	if (pCha) {
		...
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_291));
	}
}
...
void stNetDefaultSkill::Exec(void) {
	...
	// log
	LG(szLogName, RES_STRING(CL_LANGUAGE_MATCH_292), GetTickCount());
	...
	if (!pSkill) {
		LG("protocol", RES_STRING(CL_LANGUAGE_MATCH_293), sSkillID);
		return;
	}
...
void NetValidateSlotItemResponse(char chFormType, char chSlotIndex, short sGridID, char chResult) {
	if (chResult == 1) {
		// Validation successful - place the item in the slot
		switch (chFormType) {
		case 1: // Forge
		case 3: // Milling
			g_stUIForge.OnValidateSlotSuccess(chSlotIndex, sGridID);
			break;
		case 2: // Gem Combining
		case 4: // Fusion
		case 5: // Upgrade
		case 6: // Elf Shift
			g_stUIMakeEquip.OnValidateSlotSuccess(chFormType, chSlotIndex, sGridID);
			break;
		}
	}
	// If validation failed (chResult == 0), do nothing - server may have sent error message
}
...
void stNetAppendLook::Exec(CCharacter* pCha) {
	...
	if (bValid[1])
		pCha->SetItemFace(1, sLookID[1]);
	else
		pCha->SetItemFace(1, 0);
}
...
void NetItemUseSuccess(unsigned int nID, short sItemID) {
	...
	// Set cooldown now that server confirmed successful use
	// For type 71 (skill items): group by skill ID, get duration from skill
	// For regular items: group by item ID, get duration from item
	if (pInfo->sType == 71) {
		int nSkillID = atoi(pInfo->szAttrEffect);
		CSkillRecord* pSkill = GetSkillRecordInfo(nSkillID);
		if (pSkill) {
			int duration = atoi(pSkill->szFireSpeed);
			if (duration > 0) {
				GUI::CItemCommand::SetCooldown(nSkillID, CGameApp::GetCurTick(), duration);
			}
		}
	} else if (pInfo->nCooldown > 0) {
		// Regular consumables with cooldown defined in ItemInfo.txt
		int duration = pInfo->nCooldown * 1000; // Convert seconds to ms
		GUI::CItemCommand::SetCooldown(pInfo->lID, CGameApp::GetCurTick(), duration);
	}

	if (pInfo->sUseItemEffect[0] <= 0)
		return;
	...
	if (g_stUIMap.IsPKSilver() && (200 <= nEffectID && nEffectID <= 205))
		return;																														 // added by Philip.Wu  2008-01-15 ?????????????????????????
	if ((nEffectID >= 361 && nEffectID <= 369) || (nEffectID >= 3354 && nEffectID <= 3359) || (nEffectID >= 564 && nEffectID < 600)) // added by Philip.Wu  2007-12-07  ??????
	{
		CEffectObj* pEffect = CGameApp::GetCurScene()->GetFirstInvalidEffObj();
		if (!pEffect)
			return;
