...
CImeInput::CImeInput(void)
	: _pImage(nullptr), _ImmNameColor(0xf00000ff), _pList(nullptr), _nShowX(0), _nShowY(0), _bIsFull(false), _hImc(nullptr) {
...
void CImeInput::Clear() {
	...
	if (_pList) {
		delete[] reinterpret_cast<char*>(_pList);
		_pList = nullptr;
	}
	if (_pImage) {
		delete _pImage;
...
bool CImeInput::_GetCompositionString(char* str, DWORD ImeValue) {
	DWORD dwSize = ImmGetCompositionString(_hImc, ImeValue, nullptr, 0);
...
bool CImeInput::_GetCandidateList() {
	...
	if (_pList) { delete[] reinterpret_cast<char*>(_pList); _pList = nullptr; }

	memset(_strCandidate, 0, sizeof(_strCandidate));
	{
		if (dwSize = ImmGetCandidateList(_hImc, 0x0, nullptr, 0)) {
...
void CImeInput::SetScreen(bool isFull, int w, int h) {
	...
	if (_bIsFull) {
		...
		::ImmAssociateContextEx(g_pGameApp->GetHWND(), nullptr, IACE_DEFAULT);
	}
}
