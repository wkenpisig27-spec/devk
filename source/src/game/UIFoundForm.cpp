...
bool CFoundMgr::Init() {
	...
	for (int i = 0; i < FOUND_COUNT; i++) {
		cmdFound[i]->evtUseCommand = _evtSlotUseEvent;
	}
...
void CFoundMgr::PopItem(int iIndex, bool bRetry /* = false*/) {
	...
	cmdFound[iIndex]->DelCommand(); // ??????delete Item
...
//-----------------------------------------------------------------------------
void CFoundMgr::_evtInventoryUseEvent(CCommandObj* pSender, bool& isUse) {
	if (!g_stUIFound.frmFound || !g_stUIFound.frmFound->GetIsShow()) {
		return;
	}
	...
	for (int i = 0; i < CFoundMgr::FOUND_COUNT; i++) {
		if (!g_stUIFound.cmdFound[i]->GetCommand()) {
			g_stUIFound.PushItem(i, *pItemCommand, false);
			isUse = false;
			return;
		}
	}
...
//-----------------------------------------------------------------------------
void CFoundMgr::_evtSlotUseEvent(CCommandObj* pSender, bool& isUse) {
	isUse = false;

	for (int i = 0; i < CFoundMgr::FOUND_COUNT; i++) {
		if (g_stUIFound.cmdFound[i]->GetCommand() == pSender) {
			g_stUIFound.PopItem(i, false);
			return;
		}
	}
}
