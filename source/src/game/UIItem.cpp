...
void CColorItem::Render(int x, int y) {
	// Use pixel-based positioning for proportional fonts
	int xPos = x;
	
	ITEM_TEXT_ARRAY::iterator pos;
	for (pos = m_TextArray.begin(); pos != m_TextArray.end(); pos++) {
		CGuiFont::s_Font.Render((*pos).strData.c_str(), xPos, y, (*pos).dwColor);
		// Advance by actual pixel width of this segment
		int segmentWidth, segmentHeight;
		CGuiFont::s_Font.GetSize((*pos).strData.c_str(), segmentWidth, segmentHeight);
		xPos += segmentWidth;
	}
}
