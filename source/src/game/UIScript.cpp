//----------------------------------------------------------------------
//	2005/4/13	Arcol	?????i?????????UI_SetFormStyleEx
//----------------------------------------------------------------------
//
//  IMPORTANT: Color Parameter Handling
//  ====================================
//  When passing color values (DWORD) from Lua to C:
//  
//  CaLua converts Lua numbers (doubles) to signed int by default.
//  For color values like 0xffffffff (COLOR_WHITE), this causes data loss:
//    - Lua: 4294967295.0 (double)
//    - CaLua with "int": -1 (signed int truncation)
//    - C++ DWORD parameter: reinterprets -1 bits as 0xffffffff (works!)
//
//  Solution: Register as "int" in CLU_RegisterFunction, but use DWORD
//  in the C++ function signature. The signed int bits are reinterpreted
//  as unsigned DWORD, preserving all color data.
//
//  Example:
//    CLU_RegisterFunction("UI_SetTextColor", "int", "int, int", ...);
//    int UI_SetTextColor(int id, DWORD color) { ... }
//                                   ^^^^^
//                                   Must be DWORD, not int!
//
//  This approach avoids modifying CaLua library code.
//----------------------------------------------------------------------
#include "stdafx.h"
#include "caLua.h"
#include "script.h"
...
int UI_SetListFontColor(int list, DWORD nBackColor, DWORD nSelectColor) {
	CList* l = GetList(list);
	if (l) {
...
int UI_ComboSetTextColor(int id, DWORD color) {
	CCombo* combo = dynamic_cast<CCombo*>(CGuiData::GetGui(id));
	if (!combo)
		return R_FAIL;
...
int UI_SetLabelExFont(int id, int nFontIndex, int IsShadow, DWORD dwShadowColor) {  // NOTE: DWORD required for color
	...
	g->SetShadowColor(dwShadowColor);
	return R_OK;
}
...
int UI_SetEditCursorColor(int nEditID, DWORD color) {
	CEdit* t = dynamic_cast<CEdit*>(CGuiData::GetGui(nEditID));
	if (!t)
		return R_FAIL;
...
int UI_SetTextColor(int id, DWORD color) {  // NOTE: DWORD required for color values (0xffffffff)
	CGuiData* t = CGuiData::GetGui(id);
	if (!t)
		return R_FAIL;
...
int UI_TreeLoadImage(int nTreeID, int nType, char* imagefile, int w, int h, int sx, int sy, int itemw, int itemh) {
	...
	auto pic = [&]() -> CGuiPic* {
		...
		case enumTreeAddImage:
			return f->GetAddImage();
		case enumTreeSubImage:
			return f->GetSubImage();
		case enumTreeNodeImage:
			return f->GetNodeImage();
		default:
			return nullptr;
		}
	}();
...
int UI_GridNodeAddItem(int nodeid, int itemid) {
	...
	if (pHint) {
		...
		} else if (stricmp(obj->GetItem()->GetString(), RES_STRING(CL_LANGUAGE_MATCH_530)) == 0) {
			pHint->SetHint(g_GetAreaName(pHint->nTag));
		} else if (stricmp(obj->GetItem()->GetString(), RES_STRING(CL_LANGUAGE_MATCH_529)) == 0) {
...
// NOTE: All color parameters must be DWORD (not int) to preserve values like 0xffffffff
int UI_SetChatColor(DWORD world, DWORD road, DWORD team, DWORD guild, DWORD gm, DWORD system, DWORD trade, DWORD person) {
	// ????,�??,????,????,GM,??,????,???
...
int UI_SetLabelExShadowColor(int label_id, DWORD color) {
...
// Dialog bubble image (speech bubble background) - stub implementation
// The dialog bubble feature is not fully implemented in this codebase
int UI_LoadHeadSaySayImage(int uw, int uh, char* file, int w, int h, int tx, int ty, int isHorizontal) {
	// Not implemented - dialog bubble rendering not supported
	return R_OK;
}

// Dialog bubble mouse pointer image - stub implementation
int UI_LoadHeadSayMouseImage(int uw, int uh, char* file, int w, int h, int tx, int ty, int isHorizontal) {
	// Not implemented - dialog bubble rendering not supported
	return R_OK;
}
...
int UI_SetTextParse(int nIndex, char* file, int w, int h, int sx, int sy, int frame) // ?????????????????
{
	CGraph* p = new CGraph(file, w, h, sx, sy, frame);
...
int UI_SetHeadSayBkgColor(DWORD color) {
...
void MPInitLua_Gui() {
	...
	CLU_RegisterFunction("UI_ComboSetTextColor", "int", "int, uint", CLU_CDECL, CLU_CAST(UI_ComboSetTextColor));
	...
	CLU_RegisterFunction("UI_SetListFontColor", "int", "int, uint, uint", CLU_CDECL, CLU_CAST(UI_SetListFontColor));
	...
	CLU_RegisterFunction("UI_SetChatColor", "int", "uint, uint, uint, uint, uint, uint, uint, uint", CLU_CDECL, CLU_CAST(UI_SetChatColor));
	...
	CLU_RegisterFunction("UI_SetEditCursorColor", "int", "int, uint", CLU_CDECL, CLU_CAST(UI_SetEditCursorColor));
	...
	CLU_RegisterFunction("UI_SetTextColor", "int", "int, uint", CLU_CDECL, CLU_CAST(UI_SetTextColor));
	...
	CLU_RegisterFunction("UI_SetLabelExShadowColor", "int", "int, uint", CLU_CDECL, CLU_CAST(UI_SetLabelExShadowColor));
	...
	CLU_RegisterFunction("UI_SetLabelExFont", "int", "int,int,int,uint", CLU_CDECL, CLU_CAST(UI_SetLabelExFont));
