...
bool Connection::Connect(dbc::cChar* hostname, dbc::uShort port, dbc::uLong timeout) {
	...
	m_timeout = std::max<unsigned short>(timeout, 1000);
	if (m_timeout) {
		m_tick = GetTickCount();
	} else {
		m_tick = 0;
...
void Connection::HandshakeDone() {
	LONG l_stat;
	MutexArmor l_lock(m_mtx);
	if ((l_stat = m_status.Assign(CNST_HANDSHAKE)) == CNST_TIMEOUT) {
		m_status = CNST_TIMEOUT;
		if (m_datasock) {
			m_datasock->Free();
			m_datasock = 0;
...
/*
	...
	pk.WriteCmd(CMD_CM_SELROLE);			//锟斤拷锟斤拷
	pk.WriteLong(m_cat);
