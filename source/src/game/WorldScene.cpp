...
void CWorldScene::_FrameMove(DWORD dwTimeParam) {
	...
	if ((dwLastTime < dwTimeParam) && GetMainCha()) {
		...
		if (_pTerrain) {
			...
			// PKO FIX: Null-check _pMapInfo to prevent crash after map transitions
			if (_pMapInfo && _pMapInfo->IsShowSwitch) {
				...
				if (nArea != _nOldMainChaInArea) {
					...
					if (pArea) {
						...
						std::string name = std::string(_pMapInfo->szName) + RES_STRING(CL_LANGUAGE_MATCH_790);
		...
		if (GetUseLevel().IsTrue(LEVEL_CHA_RUN)) {
			...
			if (IsMouseWalk) {
				...
				if (!IsShop) {
					...
					int types[5]{enumCHACTRL_MONS, enumCHACTRL_MONS_TREE, enumCHACTRL_MONS_MINE, enumCHACTRL_MONS_FISH, enumCHACTRL_MONS_DBOAT};
					for (auto type : types) {
						CCharacter* Monster = HitSelectCharacter(nMouseX, nMouseY, type);
						// Validate the character is valid before setting target info
						// SetTargetInfo also has internal checks, but check here to avoid unnecessary calls
						if (Monster && Monster->IsValid()) {
							g_stUIStart.SetTargetInfo(Monster);
							break;
						}
					}
...
void CWorldScene::_SceneCursor() {
	static CSceneItem* pItem = nullptr;
	static CCharacter* pAttackCha = nullptr;
	...
	if (pSkill && pSkill->IsAttackArea()) {
		...
		if (pSkill) {
			...
			if (IsDefault) {
				...
				if (_pTerrain &&
					(_pTerrain->IsGridBlock(nScrX / 50, nScrY / 50) || !g_IsMoveAble(pMain->getChaCtrlType(), pMain->GetDefaultChaInfo()->chTerritory, (EAreaMask)_pTerrain->GetTile(nScrX / 100, nScrY / 100)->sRegion))) {
		...
		static CAttackState* attack = nullptr;
...
void CWorldScene::_KeyDownEvent(int key) {
	...
	__end:
			...
			// ?????�??????u??? 99 ????       modify by Philip.Wu   2007-09-11
			//
			// ????? ????? ????????
			// ????   ?????g
			//
			...
			if (group_id <= 99) {
				...
				int thousand = (group_id / 1000) % 10; // g?
...
bool CWorldScene::_MouseButtonDown(int nButton) {
	...
	if (nButton == 0) {
		if (_e3DMouseState == enumClick)
			g_pGameApp->GetCursor()->MoveTo(_vMousePos);

		// ??????L???????????
		CUIInterface::MainChaMove();

		// ????z?????????????  add by Philip.Wu  2006-06-02
		_pShipMgr->CloseForm();

		return true;
	} else if (nButton == 1) {
		/*
		IDirect3DTextureX*		pCurSuf = nullptr;
		HRESULT hr;
...
CCharacter* CWorldScene::HitSelectCharacter(int nScrX, int nScrY, int nSelect) {
	...
	case enumSC_TEAM:
		if (CCharacter* pMain = CGameScene::GetMainCha()) {
			if (pMain->GetTeamLeaderID() > 0) {
				...
				for (; i < _nChaCnt; i++) {
					pObj = &_pChaArray[i];
					if (pObj->IsValid() && pObj->GetTeamLeaderID() == nMainTeamID && pObj->getChaCtrlType() == enumCHACTRL_PLAYER && pObj->IsEnabled() && !pObj->IsHide()) {
	...
	case enumSC_PLAYER: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_PLAYER && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_PLAYER_ASHES: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_PLAYER && !pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_ENEMY: // ???????PK,??PK?????+????????,??????????
	{
		...
		if (pMain && pMain->GetIsPK()) {
			...
			if (nMainTeamID != 0) {
				for (; i < _nChaCnt; i++) {
					pObj = &_pChaArray[i];
					if (pObj->IsValid() && (pObj->getChaCtrlType() == enumCHACTRL_MONS || (pObj->getChaCtrlType() == enumCHACTRL_PLAYER && pObj->GetTeamLeaderID() != nMainTeamID)) && pObj->IsEnabled() && !pObj->IsHide()) {
						if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
							if (!pObjXXX) {
								pObjXXX = pObj;
								dis = info.dis;
				...
				for (; i < _nChaCnt; i++) {
					pObj = &_pChaArray[i];
					if (pObj->IsValid() && (pObj->getChaCtrlType() == enumCHACTRL_MONS || (pObj->getChaCtrlType() == enumCHACTRL_PLAYER && pMain != pObj)) && pObj->IsEnabled() && !pObj->IsHide()) {
						if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
							if (!pObjXXX) {
								pObjXXX = pObj;
								dis = info.dis;
			...
			for (; i < _nChaCnt; i++) {
				pObj = &_pChaArray[i];
				if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS && pObj->IsEnabled() && !pObj->IsHide()) {
					if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
						if (!pObjXXX) {
							pObjXXX = pObj;
							dis = info.dis;
	...
	case enumSC_MONS_TREE: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS_TREE && pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_MONS_REPAIRABLE: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS_REPAIRABLE && pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_MONS_MINE: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS_MINE && pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_MONS_FISH: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS_FISH && pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
	...
	case enumSC_MONS_DBOAT: {
		for (; i < _nChaCnt; i++) {
			pObj = &_pChaArray[i];
			if (pObj->IsValid() && pObj->getChaCtrlType() == enumCHACTRL_MONS_DBOAT && pObj->IsEnabled() && !pObj->IsHide()) {
				if (SUCCEEDED(pObj->HitTestPrimitive(&info, &org, &ray))) {
					if (!pObjXXX) {
						pObjXXX = pObj;
						dis = info.dis;
...
void CWorldScene::LoadingCall() {
	...
	if (g_Config.m_bEditor) // ????g? ??????
	{
		static bool firstTime = true;
		if (firstTime) {
			char szMap[32] = {0};
			char szAppName[] = {(char)0xB1, (char)0xE0, (char)0xBC, (char)0xAD, (char)0xC6, (char)0xF7, (char)0}; // ?????????????K???????????
