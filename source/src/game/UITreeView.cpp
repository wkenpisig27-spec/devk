...
CTreeGridNode::CTreeGridNode(CTreeView* own, CItemObj* item)
	: CTreeNodeObj(own), _pItem(item), _nUnitWidth(32), _nUnitHeight(32), _nColMaxNum(3), _nRowNum(0) {
	_nFocusX1 = 0;
	_nFocusX2 = 0;
	_nFocusY1 = 0;
	_nFocusY2 = 0;
...
CTreeView::CTreeView(CForm& frmOwn)
	: CCompent(frmOwn), _nColSpace(4), _nRowSpace(5), _pRoot(nullptr), _IsAutoExpand(true), evtSelectChange(nullptr), evtMouseDown(nullptr), evtSelectLost(nullptr), evtItemLost(nullptr), evtItemChange(nullptr), evtMouseDragEnd(nullptr), _pDragItem(nullptr), _pDragNode(nullptr), evtMouseDB(nullptr), _dwSelectColor(COLOR_BLACK) {
	// _IsFocus = true;
...
void CTreeView::Render() {
	try {
		...
		LG("ui_error", "Exception in CTreeView::Render()\n");
	}
}
...
void CTreeView::SetSelectNode(CTreeNodeObj* p) {
	_pSelectItem->SetItem(p, nullptr);
}
