...
#define OPEN_AMBIENT_ENV
	...
	if (_bShowSceneObj) {
		...
		for (; it != it_end; ++it) {
			...
			if (pObj->IsValid() && pObj->IsHide() == FALSE) {
				...
				if (_pTerrain) {
					MPTile* pRegionTile = _pTerrain->GetTile((int)(pObj->getPos().x), (int)(pObj->getPos().y));
					if (pRegionTile) {
						int a = pRegionTile->getIsland();
						if (a != nArea) {
							nArea = a;
...

	{
		...
		if (g_IsShowModel) {
			for (i = 0; i < _nChaCnt; i++) {
				...
				if (cha_lgt) {
					v = pCha->GetPos();
					tile = _pTerrain ? _pTerrain->GetTile((int)v.x, (int)v.y) : nullptr;
					if (!tile) {
			...
			if (pMain)
				nMainTeamID = pMain->GetTeamLeaderID();
			for (i = 0; i < _nChaCnt; i++) {
				pCha = &_pChaArray[i];
				if (!pCha->IsValid())
					continue;
...

	{
		...
		if (_IsShowPath && _pTerrain) {
			g_cFindPath.RenderPath(_pTerrain.get());
		}
	...
	if (_bShowSceneObj) {
		...
		for (; it != it_end; ++it) {
			...
			if (pObj->IsValid() && pObj->IsHide() == FALSE) {
				...
				int a = 0;
				if (_pTerrain) {
					MPTile* pRegionTile2 = _pTerrain->GetTile((int)(pObj->getPos().x), (int)(pObj->getPos().y));
					if (pRegionTile2) a = pRegionTile2->getIsland();
				}
				if (a != nArea) {
					nArea = a;
...
void CGameScene::_RenderUI() {
	...
	for (i = 0; i < _nChaCnt; i++) {
		// Disable rendering of mount name
		if (pCha->IsValid() && !pCha->IsHide() && !pCha->GetIsForUI() && !pCha->GetIsMount()) {
