...
BOOL MC_GUILD_LISTTRYPLAYER(LPRPACKET pk) {
	...
	cChar* l_motto = pk.ReadString();
	uChar l_stat = CGuildData::eState::normal; /*pk.ReadChar();*/
...
BOOL PC_GUILD(LPRPACKET pk) {
	...
	case MSG_GUILD_START: {
		...
		for (uChar i = 0; i < l_num; ++i) {
			...
			if (l_chaname) strncpy(info.szChaName, l_chaname, sizeof(info.szChaName) - 1);
			if (l_job) strncpy(info.szJob, l_job, sizeof(info.szJob) - 1);
			if (l_motto) strncpy(info.szMotto, l_motto, sizeof(info.szMotto) - 1);
...
BOOL MC_GUILD_LISTCHALL(LPRPACKET pk) {
	...
	for (int i = 0; i < MAX_GUILD_CHALLLEVEL; i++) {
		...
		if (Info.byLevel[i]) {
			...
			cChar* guildStr = pk.ReadString();
			cChar* challStr = pk.ReadString();
			if (guildStr) strncpy(Info.szGuild[i], guildStr, 64 - 1);
			if (challStr) strncpy(Info.szChall[i], challStr, 64 - 1);
			Info.dwMoney[i] = pk.ReadLongLong();
		}
	}
