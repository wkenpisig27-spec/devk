...
CCheckBox::CCheckBox(CForm& frmOwn)
	: CCompent(frmOwn), evtCheckChange(nullptr), _pImage(nullptr), _TextColor(0xffff0000), _isDown(false), _isChecked(false) {
...
void CCheckBox::Render() {
	...
	if (!_strCaption.empty())
		CGuiFont::s_Font.Render(_strCaption.c_str(), GetX() + _nTextOffsetX, GetY() + _nTextOffsetY, _TextColor);
}
...
CCheckGroup::CCheckGroup(CForm& frmOwn)
	: CContainer(frmOwn), _nSize(0), _pActive(nullptr), _nActive(-1), evtSelectChange(nullptr) {
}
...
void CCheckGroup::Init() {
	...
	if (!_items.empty())
		_UpdataActive((CCheckBox*)_items[0]);
}

void CCheckGroup::SetActiveIndex(int n) {
	_nActive = n;
	if (!_items.empty())
		_UpdataActive((CCheckBox*)_items[_nActive]);
}
