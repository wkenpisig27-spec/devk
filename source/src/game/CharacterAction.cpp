...
void InitPoseData() {
	g_PoseData.Init("scripts\\txt\\CharacterAction.tx");
}
...
bool CGameCharacterAction::Init(const char* ptcsFileName) {
	bool bRet = true;
	char tcsLine[1024], tcsTemp[1024];
	...
	while (!feof(fFile)) {
		...
		if (iCurType < 0)
			continue;
		// ???????
		m_SCharacterAction[iCurType].m_SActionInfo = nullptr;
		...
		while (!feof(fFile)) {
			...
			if (tcsLine[lIndex] == _TEXT('\t')) // tab?
			{
				...
				// ????, ???????????
				if (_ttoi(tcsTemp) < 1) {
					MessageBox(nullptr, _TEXT(RES_STRING(CL_LANGUAGE_MATCH_24)), _TEXT(RES_STRING(CL_LANGUAGE_MATCH_25)), 0);
					goto end;
				}
				//
		...
		if (m_SCharacterAction[iCurType].m_SActionInfo == nullptr) {
			m_SCharacterAction[iCurType].m_iMaxActionNum = 0;
			bRet = false;
			goto end;
		}
		...
		while (!feof(fFile)) {
			...
			if (tcsLine[lIndex] == _TEXT('\t')) // tab?
			{
				...
				// if (m_SCharacterAction[iCurType].m_SActionInfo[iCurActNO - 1].m_sKeyFrame == nullptr)
				//{
...
void CGameCharacterAction::Free(void) {
	...
	if (m_SCharacterAction) {
		for (i = 0; i < m_iMaxCharacterType; i++) {
			...
			//		m_SCharacterAction[i].m_SActionInfo[j].m_sKeyFrame = nullptr;
			//	}
			// }

			// delete [] m_SCharacterAction[i].m_SActionInfo;
			// m_SCharacterAction[i].m_SActionInfo = nullptr;
...
bool CGameCharacterAction::GetCharAction(int iCharType, SCharacterAction* SCharAct) {
	if (m_SCharacterAction == nullptr || SCharAct == nullptr)
		return false;
	if (iCharType < 1 || iCharType > m_iMaxCharacterType)
		return false;
	if (m_SCharacterAction[iCharType - 1].m_iCharacterType < 1)
