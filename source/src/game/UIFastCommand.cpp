...
CFastCommand::CFastCommand(CForm& frmOwn)
	: CCommandCompent(frmOwn), _pCommand(nullptr), _pCommand2(nullptr), evtChange(nullptr), topBar(false) {
...
CFastCommand::CFastCommand(const CFastCommand& rhs)
	: CCommandCompent(rhs), _pCommand(rhs._pCommand), _pCommand2(rhs._pCommand2), evtChange(rhs.evtChange) {
	_fast.push_back(this);
...
bool CFastCommand::MouseRun(int x, int y, DWORD key) {
	...
	if (!_pCommand) {
		InRect(x, y);
		// Empty slot - don't consume event so parent form can handle drag
		return false;
		...
		// Slot's own drag is active (rearranging skills) - consume event
		return _IsMouseIn;
	}

	// For other events (e.g. Mouse_LDown), don't consume so form drag can work
	return false;
}
...
COneCommand::COneCommand(CForm& frmOwn)
	: CCommandCompent(frmOwn), _pCommand(nullptr), evtBeforeAccept(nullptr), evtThrowItem(nullptr), evtUseCommand(nullptr), _pActive(nullptr), _IsShowActive(false), _eShowStyle(enumSmall) {
	_SetSelf();
}
