...
bool CComposeMgr::Init() {
	...
	for (int i = 0; i < COMPOSE_COUNT; i++) {
		cmdCompose[i]->evtUseCommand = _evtSlotUseEvent;
	}
...
void CComposeMgr::SetComposeUI() {
	...
	// if ((nullptr == pItem1) || (nullptr == pItem2) || (nullptr == pItem3) || (nullptr == pItem4) || (nullptr == pItem5) || (nullptr == pItem6))
	//{
	//	btnForgeYes->SetIsEnabled(false);
	//	return;
	// }
...
//-----------------------------------------------------------------------------
void CComposeMgr::_evtInventoryUseEvent(CCommandObj* pSender, bool& isUse) {
	if (!g_stUICompose.frmCompose || !g_stUICompose.frmCompose->GetIsShow()) {
		return;
	}
	...
	for (int i = 0; i < CComposeMgr::COMPOSE_COUNT; i++) {
		if (!g_stUICompose.cmdCompose[i]->GetCommand()) {
			g_stUICompose.PushItem(i, *pItemCommand, false);
			isUse = false;
			return;
		}
	}
...
//-----------------------------------------------------------------------------
void CComposeMgr::_evtSlotUseEvent(CCommandObj* pSender, bool& isUse) {
	isUse = false;

	for (int i = 0; i < CComposeMgr::COMPOSE_COUNT; i++) {
		if (g_stUICompose.cmdCompose[i]->GetCommand() == pSender) {
			g_stUICompose.PopItem(i, false);
			return;
		}
	}
}
