...
static CStateSynchro* pSynchro = nullptr;
...
CServerHarm::~CServerHarm() {
	// Nullify back-pointers to avoid use-after-free when CSynchroManage
	// later deletes CAttackEffect/CAttackRepSynchro objects
	for (synchros::iterator it = _harm.begin(); it != _harm.end(); ++it) {
		(*it)->SetServerHarm(nullptr);
	}
	_harm.clear();
	for (synchros::iterator it = _rep.begin(); it != _rep.end(); ++it) {
		(*it)->SetServerHarm(nullptr);
	}
	_rep.clear();
}
...
void CServerHarm::HitRepresent(CSkillRecord* pSkill, int nAngle) {
	...
	for (synchros::iterator it = _harm.begin(); it != _harm.end(); it++) {
		...
		if (pCha) {
			...
			if (pSkill->sTargetEffectID > 0)
				pCha->SelfEffect(pSkill->sTargetEffectID, pSkill->sTargetDummyLink, false, -1, nAngle);
		}
	}
}

bool CServerHarm::AddRep(CAttackRepSynchro* s) {
	if (_pSkill->IsPlayCyc())
		return false;
