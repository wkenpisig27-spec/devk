...
bool CIsSkillUse::IsAttack(CSkillRecord* pSkill, CCharacter* pSelf, CCharacter* pTarget) {
	...
	case enumSKILL_TYPE_TEAM:
		if (pSelf == pTarget || (pSelf->GetTeamLeaderID() > 0 && pSelf->GetTeamLeaderID() == pTarget->GetTeamLeaderID())) {
			return true;
		}
...
const char* CIsSkillUse::GetError() {
	...
	switch (_eError) {
	case enumInValid:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_149), _pSkill->szName);
		break;
	case enumNotEnergy:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_150), _pSkill->szName);
		break;
	case enumNotAttack:
		return RES_STRING(CL_LANGUAGE_MATCH_151);
	case enumNotUse:
		return RES_STRING(CL_LANGUAGE_MATCH_152);
	case enumNotMP:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_153), _pSkill->szName);
		break;

	case enumSelf:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_154), _pSkill->szName);
		break;
	case enumFish:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_155), _pSkill->szName);
		break;
	case enumDieBoat:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_156), _pSkill->szName);
		break;
	case enumTree:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_157), _pSkill->szName);
		break;
	case enumMine:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_158), _pSkill->szName);
		break;
	case enumOnlyTeam:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_159), _pSkill->szName);
		break;
	case enumDie:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_160), _pSkill->szName);
		break;
	case enumTargetError:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_161), _pSkill->szName);
		break;

	case enumHelpMons:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_162), _pSkill->szName);
		break;
	case enumAttackMain:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_163), _pSkill->szName);
		break;
	case enumAttackTeam:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_164), _pSkill->szName);
		break;
	case enumAttackDie:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_165), _pSkill->szName);
		break;
	case enumAttackPlayer:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_166), _pSkill->szName);
		break;
	case enumRepair:
		sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_167), _pSkill->szName);
		break;
	}
	return buf;
}
