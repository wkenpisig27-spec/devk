...
CTextButton* CTextButton::m_pCurButton = nullptr; // 当前激活的Button

CTextButton::CTextButton(CForm& frmOwn)
	: CCompent(frmOwn), _isDown(false), _textColor(0xffff0000), _eFormModal(CForm::mrNone), evtMouseClick(nullptr), evtMouseDownContinue(nullptr), _dwFlashCycle(0), _dwLastClick(0), evtMouseRClick(nullptr), evtMouseDBClick(nullptr) {
	_pImage = new CGuiPic(this, csEnd);
}

CTextButton::CTextButton(const CTextButton& rhs)
	: CCompent(rhs), _pImage(new CGuiPic(*rhs._pImage)), _strCaption(rhs._strCaption), _textColor(rhs._textColor), _isDown(rhs._isDown), _eFormModal(rhs._eFormModal), evtMouseClick(rhs.evtMouseClick), evtMouseDownContinue(rhs.evtMouseDownContinue), _dwFlashCycle(0), _dwLastClick(0), evtMouseRClick(nullptr), evtMouseDBClick(nullptr) {
		...
		if (key & Mouse_LUp) {
			if (_isDown) {
				...
				const auto isDoubleClick = [&] {
					// Two clicks with less than 500 milliseconds are considered as double clicks
					const auto sinceLastClick = GetTickCount() - _dwLastClick;
					const auto timeout = 500;
					return sinceLastClick < timeout;
				};

				if (evtMouseDBClick && isDoubleClick()) // 两次点击时间少于 500 毫秒，则认定为双击
				{
...
void CTextButton::DoClick(eMouseState state) {
	...
	case Mouse_LUp: {
		if (evtMouseClick)
			...
			_frmOwn->evtEntrustMouseEvent(this, _eFormModal, GetX(), GetY(), Mouse_LUp);
	} break;
