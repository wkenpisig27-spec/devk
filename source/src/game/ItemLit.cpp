...
LW_RESULT lwItemLit::Load(const char* file) {
	...
	LW_ZERO_RET(fp);
	LW_FAILED_RET(fp->CreateFile(file, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING));
...
LW_RESULT lwItemLit::Save(const char* file) {
	...
	LW_FAILED_RET(lwGUIDCreateObject((lxvoid**)&fp, LW_GUID_FILE));
	LW_ZERO_RET(fp);
	LW_FAILED_RET(fp->CreateFile(file, GENERIC_WRITE, FILE_SHARE_READ, nullptr, CREATE_ALWAYS));
