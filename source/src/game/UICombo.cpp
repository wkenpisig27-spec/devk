...
CCombo::CCombo(CForm& frmOwn)
	: CCompent(frmOwn), _pEdit(new CEdit(frmOwn)), _pList(new CList(frmOwn)), _pButton(new CTextButton(frmOwn)), _eListStyle(lsDown), evtSelectChange(nullptr), evtMouseClick(nullptr) {
	_IsFocus = false;
	_SetSelf();
}

CCombo::CCombo(const CCombo& rhs)
	: CCompent(rhs), _pEdit(new CEdit(*rhs._pEdit)), _pList(new CList(*rhs._pList)), _pButton(new CTextButton(*rhs._pButton)), _eListStyle(rhs._eListStyle) {
	_SetSelf();
	...
	if (IsNoDrag(x, y, key)) {
		if (key & Mouse_LDown) {
			if (!_isChild && GetActive() != this) {
				...
				if (evtMouseClick)
					evtMouseClick(this, x, y, key);
				return true;
			}
		}

		if (_pButton->MouseRun(x, y, key))
			return true;
		if (key & Mouse_LDown) {
			_pList->SetIsShow(!_pList->GetIsShow());
			return true;
