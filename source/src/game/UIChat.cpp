...
CForm* CChat::_frmDetails = nullptr;
CForm* CChat::_frmBlockPlayer = nullptr;
...
CEdit* CChat::_pEditBlockName = nullptr;
...
CTreeGridNode* CChat::_pBlockedNode = nullptr;
...
CMenu* CChat::_BlockedMouseRight = nullptr;
...
CChat::CChat()
	: m_pQQTreeView(nullptr), _pGroupNode(nullptr), _pFrndNode(nullptr) {
	frmChatManage = nullptr;
}

bool CChat::Init() {
	OutputDebugStringA("PKO: CChat::Init - start\n");
	...
	//	filterTxt.close();
	// }
	OutputDebugStringA("PKO: CChat::Init - AddHotKeyEvent...\n");
	CFormMgr::s_Mgr.AddHotKeyEvent(CTalkSessionFormMgr::OnHotKeyShow);

	OutputDebugStringA("PKO: CChat::Init - new CTeamMgr...\n");
	_pTeamMgr = new CTeamMgr;

	CFormMgr& mgr = CFormMgr::s_Mgr;
	OutputDebugStringA("PKO: CChat::Init - initializing team controls...\n");

	// ???????
	char szBuf[80] = {0};
	char dbgBuf[256] = {0};
	for (int i = 0; i < MAX_MEMBER; i++) {
		sprintf(dbgBuf, "PKO: CChat::Init - team loop i=%d - mgr.Find(frmTeamMenber%d)...\n", i, i + 1);
		OutputDebugStringA(dbgBuf);
		sprintf(szBuf, "frmTeamMenber%d", i + 1);
		frmTeamMenber[i] = mgr.Find(szBuf);
		if (!frmTeamMenber[i]) {
			sprintf(dbgBuf, "PKO: CChat::Init - frmTeamMenber%d NOT FOUND!\n", i + 1);
			OutputDebugStringA(dbgBuf);
			return false;
		}
		OutputDebugStringA("PKO: CChat::Init - SetIsShow(false)...\n");
		frmTeamMenber[i]->SetIsShow(false);
		OutputDebugStringA("PKO: CChat::Init - Refresh()...\n");
		...
		OutputDebugStringA("PKO: CChat::Init - finding d3dDown...\n");
		...
		OutputDebugStringA("PKO: CChat::Init - finding d3dHead...\n");
		...
		OutputDebugStringA("PKO: CChat::Init - creating CCharacter2D...\n");
		_pCharacter[i] = new CCharacter2D;
		OutputDebugStringA("PKO: CChat::Init - CCharacter2D->Create()...\n");
		_pCharacter[i]->Create(rt);
		sprintf(dbgBuf, "PKO: CChat::Init - team loop i=%d complete\n", i);
		OutputDebugStringA(dbgBuf);
	}
	...
	if (!pbtnAdd)
		return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmQQ->GetName(), "btnBlock");
	...
	if (!_BlockedMouseRight)
		return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmQQ->GetName(), "BlockedRight");
	...
	if (!pbtnAdd)
		return _Error(RES_STRING(CL_LANGUAGE_MATCH_473), _frmDetails->GetName(), "btnRefurbish");
	...
	if (!pbtnAdd)
		return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmBlockPlayer->GetName(), "btnYes");
	pbtnAdd->evtMouseClick = _evtBlockPlayer;
	_pEditBlockName = dynamic_cast<CEdit*>(_frmBlockPlayer->Find("edtTradeGold"));
	if (!_pEditBlockName)
		return _Error(RES_STRING(CL_LANGUAGE_MATCH_411), _frmBlockPlayer->GetName(), "edtTradeGold");
...
void CChat::End() {
	...
	// delete _pTeamMgr;
	SAFE_DELETE(_pTeamMgr); // UI????

	for (int i = 0; i < MAX_MEMBER; i++) {
		// delete _pCharacter[i];
		//_pCharacter[i] = 0;
		SAFE_DELETE(_pCharacter[i]); // UI????
	}
}
...
int CChat::TeamSend(DWORD dwMsg, void* pData, DWORD dwParam) {
	...
	case enumSTM_NODE_CHANGE: {
		...
		CChatIconInfo* pIconInfo = GetChatIconInfo(pMember->GetIconID());
		if (pIconInfo && pItem) // modify by Philip.Wu  2006-08-13  ??????????
		{
			CGuiPic* pPic = pItem->GetImage();
			string strPath = "texture/ui/HEAD/";
			if (pMember->IsOnline()) {
...
void CChat::_MainMouseClick(CGuiData* pSender, int x, int y, DWORD key) {
	...
	} else if (pSelectNode == _pTeamMgr->Find(enumTeamMaster)->GetPointer()) // ??
	{
	...
	} else if (pSelectNode == _pTeamMgr->Find(enumTeamPrentice)->GetPointer()) // ??
	{
...
void CChat::_OnMouseSessMenu(CGuiData* pSender, int x, int y, DWORD key) {
	...
	if (!pItem)
		return;
	string str = pItem->GetString();
	if (str == RES_STRING(CL_LANGUAGE_MATCH_485)) // ????
	{
...
void CChat::_OnMouseMasterMenu(CGuiData* pSender, int x, int y, DWORD key) {
	...
	if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_482)) // ????
	{
	...
	} else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_484)) // ????
	{
		CCharacter* pMainCha = CGameScene::GetMainCha();
	...
	} else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_481)) // ????
	{
		CCozeForm::GetInstance()->OnPrivateNameSet(_curSelectMember->GetName());
	} else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_478)) // ????
	{
		CTalkSessionFormMgr::ApplySession(&_curSelectMember);
	} else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_854)) // ????(??????)
	{
		CCharacter* pMain = CGameScene::GetMainCha();
		if (pMain && pMain->getGameAttr()) {
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_889), _curSelectMember->GetName(), nLevel * 500); // "???? %s ????\n??:%d\n????"
...
void CChat::_OnMouseStudentMenu(CGuiData* pSender, int x, int y, DWORD key) {
	...
	} else if (strCommand == RES_STRING(CL_LANGUAGE_MATCH_854)) // ????(??????)
	{
		...
		if (pMain && pMain->getGameAttr()) {
			...
			char szBuffer[256] = {0};
			sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_856), _curSelectMember->GetName(), nLevel * 5); // ???? %s ????\n??:%d  ??:%d\n????
