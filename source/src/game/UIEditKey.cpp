...
//---------------------------------------------------------------------------
CEditKey::CEditKey()
	: _IsReadyOnly(false), _dwFontIndex(0), _nCursorX(0), _nCursorY(0), _dwCursorColor(0xffffffff) {
...
bool CEditKey::OnChar(char c) {
	...
	default: {
		...
		if (_nEnterPos == 1) {
			IsError = false;
			if (_ismbslead((unsigned char*)_szEnter, (unsigned char*)&_szEnter[0]) == 0 && _ismbstrail((unsigned char*)_szEnter, (unsigned char*)&_szEnter[0]) == 0) {
				// 为英文或控制字符
				_nEnterPos = 0;
