...
void CStoreMgr::OpenStoreAsk() {
	...
	if (pCha->GetIsFight()) {
		g_pGameApp->SysInfo("Please enter the safe zone!");
		return;
	} else {
		CS_StoreOpenAsk("");
	}
...
void CStoreMgr::ShowStoreForm(bool bShow) {
	...
	CFormMgr::s_Mgr.SetEnableHotKey(HOTKEY_STORE, !bShow); // ?????j????
...
void CStoreMgr::ShowStoreWebPage() {
	...
	if (g_cooperate.code == COP_CGA) // ?????????
	{
		...
		ShellExecute(0, "open", strURL.c_str(), nullptr, nullptr, SW_SHOW);
	} else {
		/*ShellExecute(0, "open",
				RES_STRING(CL_LANGUAGE_MATCH_938), // "http://cache.moliyo.com/shop_77/", nullptr, nullptr, SW_SHOW);*/
...
void CStoreMgr::ShowTryonForm(bool bShow) {
	if (bShow) {
		...
		if (false == frmTryon->GetIsShow()) {
			...
			m_pCurrMainCha->FightSwitch(true); // ??????j????????
...
void CStoreMgr::AddStoreItemInfo(long nSeq, long nID, const char* szName, long nPrice, const char* szRemark, bool isHot, long nTime, long nRemainNum, long nRemainTime) {
	...
	char szTemp[128] = {0};
	// sprintf(szTemp, "%s:%s", RES_STRING(CL_LANGUAGE_MATCH_845), szName);
	m_stStoreGui[nSeq].labName->SetCaption(szName); // ?????
	m_stStoreGui[nSeq].labName->SetIsShow(true);

	// sprintf(szTemp, "%s:%s", RES_STRING(CL_LANGUAGE_MATCH_846), StringSplitNum(nPrice, 3, ','));
...
void CStoreMgr::SetStoreVip(long nVip) {
	...
	if (nVip) {
		...
		labMemberStyle->SetCaption(RES_STRING(CL_LANGUAGE_MATCH_902)); // ?????
	} else {
		// if(btnToVip) btnToVip->SetIsEnabled(true);
		labMemberStyle->SetCaption(RES_STRING(CL_LANGUAGE_MATCH_903)); // ??????
	}
...
bool CStoreMgr::ResetLastOperate(bool bSilent) {
	...
	if (dwCurr - dwLast < 10000) {
		if (!bSilent) {
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_895)); // ?????????????????????????
		}
...
void CStoreMgr::_SetIsShowUserInfo(bool bShow) {
	if (bShow) {
		labListPage->SetIsShow(!bShow);	 // ??????k
		btnLeftPage->SetIsShow(!bShow);	 // ??????T
		btnRightPage->SetIsShow(!bShow); // ???????T
	...
	btnTrade->SetIsShow(!bShow); // ?????T
	btnTryon->SetIsShow(!bShow); // ?????t
	btnViewAll->SetIsShow(!bShow);
	labTrade->SetIsShow(!bShow); // ?????t??
	labTryon->SetIsShow(!bShow); // ?????t??
	...
	imgBackGround10->SetIsShow(!bShow); // ????I??????
}
...
void CStoreMgr::_ShowTradeSelectBox() {
	if (0 <= m_nCurSel && m_nCurSel < STORE_PAGE_SIZE &&
		...
		if (0 == m_nVip && _IsCurSelVipNode()) {
			g_pGameApp->MsgBox(RES_STRING(CL_LANGUAGE_MATCH_913)); // VIP???????VIP????�?????
			return;
		}
...
bool CStoreMgr::_IsCurSelVipNode(void) {
	...
	if (it != m_mapNode.end()) {
		...
		if (strName.size() > 3) {
			...
			if (0 == _stricmp(strName.c_str(), RES_STRING(CL_LANGUAGE_MATCH_902))) // ?????
			{
				return true;
			}
		}
	...
	if (pSelItem) {
		...
		for (MapNodeIter it = g_stUIStore.m_mapNode.begin(); it != g_stUIStore.m_mapNode.end(); ++it) {
			if (it->second == strItem) {
				...
				} else if (strItem == RES_STRING(CL_LANGUAGE_MATCH_921)) // ??j???
				{
...
void CStoreMgr::_evtStoreFormMouseEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	} else if (strName == "btnRightPage") // ??h?
	{
	...
	} else if (strName == "btnReceiveMoney") // ?h???t
	{
		if (g_stUIStore.imgBackGround10->GetIsShow())
			return;
		g_stUIStore.m_pkExchangeNum =
			g_stUIBox.ShowNumberBox(_evtExchangeEvent, -1, RES_STRING(CL_LANGUAGE_MATCH_904), false); // ??????h?????
	}
	// else if(strName == "btnToVIP")	// ???VIP
	//{
	//	if(g_stUIStore.imgBackGround10->GetIsShow()) return;
	//	CBoxMgr::ShowSelectBox(_evtStoreToVipEvent, RES_STRING(CL_LANGUAGE_MATCH_915), true); // ???????VIP????????
	// }
	else if (strName == "btnReceiveMoDou") // ?h?H??
	{
		if (g_stUIStore.imgBackGround10->GetIsShow())
			return;
		...
		default:
			strURL = RES_STRING(CL_LANGUAGE_MATCH_939); // "http://pay.moliyo.com/";
			break;
		}

		ShellExecute(0, "open",
					 strURL.c_str(), nullptr, nullptr, SW_SHOW);
...
void CStoreMgr::_evtExchangeEvent(CCompent* pSender, int nMsgType, int x, int y, DWORD dwKey) {
	...
	if (num > 0) {
		...
		sprintf(szBuffer, RES_STRING(CL_LANGUAGE_MATCH_905), g_stUIStore.m_nExchangeNum); // ???h?????: %d
...
void CStoreMgr::_evtStoreFormClose(CForm* pForm, bool& IsClose) {
	// ?????????
	CFormMgr::s_Mgr.SetEnableHotKey(HOTKEY_STORE, true); // ?????j????
