...
bool CFormMgr::_IsDebugMode = false;				// ????????g? -added by Arcol
bool CFormMgr::_IsDrawFrameInDebugMode = true;		// ?????????g? -added by Arcol
bool CFormMgr::_IsDrawBackgroundInDebugMode = true; // ?????????g? -added by Arcol

CFormMgr::CFormMgr()
	: _bEnabled(false), _bInit(false), _nEnableHotKey(0xFFFFFFFF), _nTempleteNo(-1), /*_nMouseHover(0),*/ _pHintGui(nullptr) // ?????j????
{
...
bool CFormMgr::HandleWindowMsg(DWORD dwMsg, WPARAM wParam, LPARAM lParam) {
	if (!_bEnabled)
		return false;
	...
	case WM_HOTKEY: {
		if (LOWORD(lParam) == MOD_ALT) {
			...
			for (vfrm::const_iterator it = _forms->begin(); it != end; ++it) {
				if ((*it)->GetHotKey() > 0 && (*it)->GetHotKey() == (UINT)HIWORD(lParam)) {
					if ((*it)->GetIsShow())
						(*it)->Hide();
					else
						(*it)->Show();
	...
	CImeInput::s_Ime.HandleWindowMsg(dwMsg, wParam, lParam);
	return false;
}
...
void CFormMgr::ShowDebugInfo() {
	...
	for (i = 0; i < (int)_forms->size(); i++) {
		...
		//	pForm->GetParent()?pForm->GetParent()->GetName():"�???????",pForm->_allCompents.size());
	...
	data0 = GetFormTempleteMax(); // g??????
	for (i = 0; i < (int)GetFormTempleteMax(); i++) {
		if (_showforms[i]->size() > 0) {
			data1++; // ???g????
		}
	}
	data2 = GetFormTempletetNum();					// ??jg????
	data3 = (int)_allForms.size();					// ????????
	data4 = (int)_forms->size();					// ??jg????????
	data5 = (int)_show.size() + (int)_modal.size(); // ??jg?????????
	data6 = (int)_modal.size();						// ??jg??g???????
	data7 = (int)CGuiTime::_times.size();
	sprintf(buf, RES_STRING(CL_LANGUAGE_MATCH_576), data0, data1, data2, data3, data4, data5, data6, data7, totalTick);
