...
CSelectItem::CSelectItem()
	: _pItem(nullptr), _isSelect(false), _nIndex(-1), _isEnabled(true), _pList(nullptr) {
	_pImage = new CGuiPic;
}
...
CListItems::CListItems()
	: _pSelect(new CSelectItem()), _dwCount(0), _nItemLeft(0), _nItemTop(0), _dwFirstShowRow(0), _pList(nullptr), _dwLastShowRow(0), _nShowCount(1000), _nMaxRow(0), _IsMouseFollow(false), _pItemPic(nullptr), _nPicLeft(0), _nPicTop(0), _nPicX(0) {
}

CListItems::CListItems(const CListItems& rhs)
	: _pSelect(new CSelectItem(*rhs._pSelect)), _dwCount(rhs._dwCount), _dwFirstShowRow(rhs._dwFirstShowRow), _pList(rhs._pList), _dwLastShowRow(rhs._dwLastShowRow), _nShowCount(rhs._nShowCount), _nX1(rhs._nX1), _nX2(rhs._nX2), _nY1(rhs._nY1), _nY2(rhs._nY2), _nHeight(20), _nMaxRow(rhs._nMaxRow), _IsMouseFollow(rhs._IsMouseFollow) {
...
bool CListItems::MouseRun(int x, int y, DWORD key) {
	if (x >= _nX1 && x <= _nX2 && y >= _nY1 && y <= _nY2) {
		if (_IsMouseFollow || (key & Mouse_LDown) || (key & Mouse_LDB) || (key & Mouse_RDown)) {
			...
			if (key & Mouse_LDown) {
				if (_pList->evtListMouseDown)
					_pList->evtListMouseDown(_pList, x, y, key);
				if (_pList->evtSelectChange)
					_pList->evtSelectChange(_pList);
			}
			if (key & Mouse_LDB) {
				if (_pList->evtListMouseDB)
					_pList->evtListMouseDB(_pList, x, y, key);
			}
			if (key & Mouse_RDown) {
				if (_pList->evtListMouseDB)
					_pList->evtListMouseDB(_pList, x, y, key);
...
void CListItems::Refresh() {
	if (_nHeight > 0)
		_nShowCount = (_nY2 - _nY1) / _nHeight;
...
CList::CList(CForm& frmOwn)
	: CCompent(frmOwn), _nRowHeight(22), _nSpaceBottom(0), _dwFontColor(COLOR_BLACK), _dwSelectColor(0xFF00FF71), _nLeftMargin(5), _nTopMargin(5), _nRightMargin(5), _nBottomMargin(5), _IsChangeColor(true), evtSelectChange(nullptr), evtListMouseDown(nullptr), evtListMouseDB(nullptr) {
	_IsFocus = false;
...
CList::CList(const CList& rhs)
	: CCompent(rhs), _pImage(new CGuiPic(*rhs._pImage)), _pScroll(new CScroll(*rhs._pScroll)), _pItems(new CListItems(*rhs._pItems)), _pItemImage(new CGuiPic(*rhs._pItemImage)) {
	_Copy(rhs);
