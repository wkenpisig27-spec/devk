...
void CGameScene::HandleSceneMsg(int nMsgType, int nParam1, int nParam2, int nParam3) {
	...
	case SCENEMSG_CHA_CREATE: {
		...
		CShadeEff* pShade = nullptr;
		/*           pShade = GetFirstInvalidShadeObj();
				   if( pShade )
				   {
					   pShade->Create(2);
					   pShade->setChaID(pCha->getID());
					   pShade->Emission( 0, &pCha->GetPos(), nullptr);
		...
		if (pCha->IsBoat()) {
			...
			if (pEff) {
				if (pEff->getIdxID() == EFFECT_RIPPLE2) {
					pEff->SetHide(FALSE);
					pEff->Emission(-1, &pCha->GetPos(), nullptr);
				}
			}
		} else {
			int size = (int)pCha->GetEffectNum();
			...
			for (int n = 0; n < size; n++) {
				...
				if (pEffect && ((pEffect->getIdxID() == CHA_EFFECT1) || (pEffect->getIdxID() == CHA_EFFECT2) || (pEffect->getIdxID() == CHA_EFFECT3) || (pEffect->getIdxID() == CHA_EFFECT4))) {
	...
	case SCENEMSG_SCENEOBJ_CREATE: {
		...
		//	//pShade->Emission( -1, &pObj->getPos(), nullptr);
		//	//pShade->SetValid(TRUE);
		//	//pShade->SetHide(TRUE);
	...
	case SCENEMSG_SCENEITEM_CREATE: {
		...
		if (nParam2 != 0) {
			...
			if (pShade) {
				...
				pShade->Emission(0, &pItem->getPos(), nullptr);
				pShade->SetValid(TRUE);
				pShade->SetHide(TRUE);
