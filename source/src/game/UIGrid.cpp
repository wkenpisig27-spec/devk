...
CGrid::CGrid(CForm& frmOwn)
	: CCompent(frmOwn), _pImage(nullptr), _nUnitHeight(32), _nUnitWidth(32), _nFirst(0), _nLast(0), _nColNum(0), _nMaxPage(0), _nPage(0), _nMargin(5), _nPageNum(0), _nSize(0), _pSelect(nullptr), _nSpaceX(2), _nSpaceY(2), _pSelectImage(nullptr), evtSelectChange(nullptr), _clPageTextColor(0xff0000ff), _nSelectIndex(-1) {
	// _IsFocus = true;
...
CGrid::CGrid(const CGrid& rhs)
	: CCompent(rhs), _pImage(new CGuiPic(*rhs._pImage)), _pSelectImage(new CImage(*rhs._pSelectImage)), _pSizeImg(new CDragTitle(*rhs._pSizeImg)), _pNextPage(new CTextButton(*rhs._pNextPage)), _pPriorPage(new CTextButton(*rhs._pPriorPage)) {
...
void CGrid::Render() {
	...
	if (_pPriorPage->GetIsShow())
		_pPriorPage->Render();
	CGuiFont::s_Font.Render(_strPage, _nStrX, _nStrY, _clPageTextColor);
	if (_pNextPage->GetIsShow())
		_pNextPage->Render();
	if (_pSizeImg->GetIsShow())
		_pSizeImg->Render();
}
...
bool CGrid::MouseRun(int x, int y, DWORD key) {
	...
	if (IsNoDrag(x, y, key)) {
		...
		if (_pNextPage->MouseRun(x, y, key))
			return true;
		if (_pPriorPage->MouseRun(x, y, key))
			return true;

		if (_pSizeImg->MouseRun(x, y, key))
			return true;
		...
		if (_IsSelect) {
			if (_pSelect && _pSelect != _memory[x])
				_pSelect->Reset();
			...
			if ((key & Mouse_LDown) && evtSelectChange)
