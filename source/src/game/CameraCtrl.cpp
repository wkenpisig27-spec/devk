...
void CCamDither::_A() {
	...
	if (_InitFlag) {
		...
		if (_FrameNum >= N[_Step] * 4) {
			if (!lwFloatEqual(_RunningMat._41, _InitMat._41, 1e-2f) || !lwFloatEqual(_RunningMat._42, _InitMat._42, 1e-2f) || !lwFloatEqual(_RunningMat._43, _InitMat._43, 1e-2f)) {
				int x = 0;
			}
			_FrameNum = 0;
			if (++_Step >= T) {
...
void CCameraCtrl::InitModel(const int iType, D3DXVECTOR3* pInitPos) {
	...
	CGameApp::GetCurScene()->GetTerrain()->SetShowSize(
...
void CCameraCtrl::SetViewTransform() {
	...
	if (m_CamDither.GetState()) {
		...
		lwMatrix44Inverse(&mat, nullptr, &mat);
