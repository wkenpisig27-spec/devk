...
PATH_LINK* SearchPath(BYTE* block_buf, short width, short height, short sx, short sy, short tx, short ty, long STEP_LIMIT) {
	...
	BYTE const* target_ptr = &block_buf[ty * width + tx];

	node_list[0].buf_ptr = &block_buf[sy * width + sx];

	node_list[0].last_node_ptr = nullptr;

	BYTE *last_buf_ptr, *new_buf_ptr;

	PATH_LINK* path_link_ptr = nullptr;
	BOOL found_flag = FALSE;

	UINT32 lMaxStep = (height - 1) * (height - 1);
	if ((long)lMaxStep > STEP_LIMIT)
		lMaxStep = STEP_LIMIT; // 限制最大步数
...
skip:
	...
	if (found_flag) {
		...
		while (path_link_ptr->next_path_ptr != nullptr) {
...
CFindPath::CFindPath(long step, int range) {
	...
	_byTempBlock = std::make_unique<uint8_t[]>(MAX_PATH_STEP * MAX_PATH_STEP);
	_byTempBlock[0] = 1;
...
BOOL CFindPath::IsCross(int nCurX, int nCurY, int nTargetX, int nTargetY) {
	...
	for (int n = 1; n < inum; ++n) {
		...
		if (int(vPos.x) != nx || int(vPos.y) != ny) {
			...
			if (nx < 1 || nx >= _nWidth - 1 || ny < 1 || ny >= _nWidth - 1)
				return FALSE;
...
BOOL CFindPath::FindPath(CGameScene* pScene, CCharacter* pCha, int nSelfX, int nSelfY, int nTargetX, int nTargetY, bool& IsWalkLine) {
	...
	BYTE* byBuf = _byTempBlock.get();

	_nTargetX = _nWidth / 2 + ((nTargetX - nCurX));
	_nTargetY = _nWidth / 2 + ((nTargetY - nCurY));

	if (_nTargetX < 0 || _nTargetX >= _nWidth || _nTargetY < 0 || _nTargetY >= _nWidth) {
		LG("path_find", "msgTarget out of pathfinding grid bounds: _nTargetX=%d, _nTargetY=%d, _nWidth=%d\n", _nTargetX, _nTargetY, _nWidth);
		return FALSE;
	}
...
void CFindPath::RenderPath(MPMap* pMap) {
	...
	for (int m = 0; m < _vecPathPoint.size(); m++) {
		...
		if (_vecPathPoint[m]->z < SEA_LEVEL)
			_vecPathPoint[m]->z = SEA_LEVEL;
...
void CFindPathEx::Move() {
	...
	// Safety check: ensure main character and actor exist
	CCharacter* pMainCha = pScene->GetMainCha();
	if (!pMainCha || !pMainCha->GetActor())
		return;

	COneMoveState* pMoveState = dynamic_cast<COneMoveState*>(pMainCha->GetActor()->GetCurState());

	m_iCurX = (int)pMainCha->GetPos().x;
	m_iCurY = (int)pMainCha->GetPos().y;

	// dead gay can't use path finding
	if (!pMainCha->IsEnabled()) {
		Reset();
		return;
	}
