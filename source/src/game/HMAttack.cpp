...
CAttackEffect::CAttackEffect()
	: CStateSynchro(), _isTargetDied(false), _isDoubleAttack(false), _pTarget(nullptr), _pAttack(nullptr), _IsMiss(false), _pSkill(nullptr), _isBeatBack(false), _pRepSynchro(nullptr) {
}
...
CAttackEffect::~CAttackEffect() {
	...
	if (_pRepSynchro) {
		_pRepSynchro->SetAttackEffect(nullptr);
	}
}
...
void CAttackEffect::_Exec() {
	if (_pRepSynchro) {
		_pRepSynchro->SetAttackEffect(nullptr);
		_pRepSynchro->Exec();
		_pRepSynchro = nullptr;
	}

	// Safety check: ensure target is valid before any operations
...
void CAttackEffect::ExecHarm(CSizeArray<stEffect>& Value, CCharacter* pTarget, CCharacter* pAttack) {
	...
	for (int i = 0; i < count; i++) {
		...
		switch (p[i].lAttrID) {
		case ATTR_HP:
			type = val < 0 ? enumSubLife : enumAddLife;
			if (pTarget->IsEnabled()) {
		...
		case ATTR_CEXP:
			if (isMain) {
				if (pTarget->IsBoat()) {
					g_pGameApp->SysInfo(RES_STRING(CL_LANGUAGE_MATCH_144), val);
				} else if (val > 0) {
					// g_pGameApp->SysInfo( RES_STRING(CL_LANGUAGE_MATCH_145), val );

					// ??????80??�??????????BUG????
					LONG64 dwLastExp = g_stUIBoat.GetHuman()->getGameAttr()->get(ATTR_CEXP);
					LONG64 dwLv80Exp = 2425349810; // ??????????????
					if (dwLastExp < dwLv80Exp && dwLv80Exp <= p[i].lVal) {
						LONG64 dwObtainExp;
						dwObtainExp = (p[i].lVal - dwLv80Exp) * LEVEL80_EXP; // 80 ?????l???
						dwObtainExp += (dwLv80Exp - dwLastExp);						// 79 ???l???
	...
	if (Change.GetChangeBitFlag(ATTR_LV)) {
		if (isMain)
			CGameApp::GetCurScene()->RefreshLevel();
...
CAttackRepSynchro::CAttackRepSynchro()
	: CStateSynchro(), _pTarget(nullptr), _pSkill(nullptr), _pAttack(nullptr), _pAttackEffect(nullptr), _IsAttackDied(false) {
}

CAttackRepSynchro::~CAttackRepSynchro() {
	if (_pAttackEffect) {
		_pAttackEffect->SetAttackRep(nullptr);
	}
}
...
CAttribSynchro::CAttribSynchro()
	: CStateSynchro(), _nType(0), _IsAlreadyExec(false), _pCha(nullptr) {
}
...
void CAttribSynchro::_Exec() {
	...
	if (enumATTRSYN_SKILL_STATE == _nType) {
		// ??????????????????????????
		if (!_Value.IsEmpty())
			CAttackEffect::ExecHarm(_Value, pCha);
			...
			if (p[i].lAttrID == ATTR_CEXP) {
				...
				if (pCha->IsBoat()) {
					...
					if (dwLastExp < dwLv80Exp && dwLv80Exp <= (DWORD)p[i].lVal) {
						LONG64 dwObtainExp;
						dwObtainExp = ((DWORD)p[i].lVal - dwLv80Exp) * LEVEL80_EXP; // 80 ?????l???
						dwObtainExp += (dwLv80Exp - dwLastExp);						// 79 ???l???
...
CSkillStateSynchro::CSkillStateSynchro()
	: CStateSynchro(), _chType(0), _pCha(nullptr) {
}
