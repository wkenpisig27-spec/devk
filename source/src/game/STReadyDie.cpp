...
CReadyDieState::CReadyDieState(CActor* p)
	: CActionState(p), _state(enumDied), _IsActionEnd(false), _eDieState(enumInit), _nFallDownTime(0), _IsAlreadyEffect(false), _pAttack(nullptr), _pArcTrack(nullptr), _isFlyOff(false) {
...
bool CReadyDieState::_Start() {
	...
	// PKO FIX: Null-check GetScene() and GetSign() during death state
	if (_pCha->GetScene() && _pCha->GetScene()->GetSign())
	...
	if (_pCha->IsMainCha()) {
		...
		if ((rand() % 9) == 0) {
			...
			// PKO FIX: Null-check GetScene() before GetGridHeight during death fly-off
			end.z = _pCha->GetScene() ? (float)_pCha->GetScene()->GetGridHeight(end.x, end.y) : _pCha->GetPos().z;
			if (dis < 10)
