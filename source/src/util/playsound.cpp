...
struct mus_dat {
	...
	volatile int status;
	enum { PMT_PLAYING,
		   PMT_FREE }; // current status of child thread
...
#define env_snd_init(id, used, cmd, group, channel) \
	mus_init(id, used, cmd, group, channel, nullptr, true, false, true)
#define cmn_snd_init(id, used, cmd, group, channel) \
	mus_init(id, used, cmd, group, channel, nullptr, false, true, true)
#define bkg_snd_init(id, used, cmd) \
	mus_init(id, used, cmd, -1, -1, nullptr, true, false, false)

inline bool is_env_snd(int id) {
	mus_dat* pdat = &mus_dat_array[id];
	if ((pdat->loop == true) && (pdat->mem_cache == false) && (pdat->mix == true))
		return true;
	else
		return false;
}
...
inline bool is_cmn_snd(int id) {
	mus_dat* pdat = &mus_dat_array[id];
	if ((pdat->loop == false) && (pdat->mem_cache == true) && (pdat->mix == true))
		return true;
	else
		return false;
}
...
inline bool is_bkg_snd(int id) {
	mus_dat* pdat = &mus_dat_array[id];
	if ((pdat->mem_cache == false) && (pdat->mix == false))
		return true;
	else
		return false;
}
...
LG("mus", "thread#%d allocated=%d, abuf=%p, alen=%d, volume=%d\n",
	...
	if (channel == -1) { // some error occured
...
void bkg_snd_finished_callback() {
	...
	if ((mus_hwnd != nullptr) && (mus_msg_id != 0))
...
inline void wave_cache_set_used(wave_cache_dat* p, bool used = true) { p->used = used; }

inline bool wave_cache_is_used(wave_cache_dat* p) { return p->used; }

inline void wave_cache_record(wave_cache_dat* p) { p->time_stamp = GetTickCount(); }
...
bool wave_cache_load(wave_cache_dat* p) {
	...
	FILE* fp = nullptr;
	if (fp = fopen(p->fname, "rb"), fp == nullptr) {
...
//         and if cache is not full, return the a free index
{
	...
	if (found)
		...
		return (i > WAV_CACHE_ID_END) ? WAV_CACHE_IS_FULL : i;
}
...
int wave_cache_add(char const* fname) { // add the file to cache, or replace the last-called (discard)
	int id;
	wave_cache_dat* p = nullptr;
...
void wave_cache_exit() { // release all memory allocated
	wave_cache_dat* p;
	for (int i = WAV_CACHE_ID_START; i <= WAV_CACHE_ID_END; ++i) {
		p = &wave_cache_dat_array[i];
		if (!p->used)
...
bool mus_mgr_init(bool enable, DWORD_PTR dwValue) {
	...
	// create exit signal
	sig_exit = CreateEvent(nullptr, TRUE, FALSE, nullptr);
	if (sig_exit == nullptr) {
		LG("mus_err", "create exit signal error\n");
		return false;
	}
	if ((sig_exit != nullptr) && (GetLastError() == ERROR_ALREADY_EXISTS)) {
		LG("mus_err", "exit signal already exist\n");
		return false;
	}

	sig_task = CreateEvent(nullptr, FALSE, FALSE, nullptr);
	if (sig_task == nullptr) {
		LG("mus_err", "create task signal error\n");
		return false;
	}
	if ((sig_task != nullptr) && (GetLastError() == ERROR_ALREADY_EXISTS)) {
	...
	for (i = MUS_ID_START; i <= MUS_ID_END; ++i) {
		sig_cmd_array[i] = CreateEvent(nullptr, FALSE, FALSE, nullptr);
		thrdpool[i] = CreateThread(nullptr, 0, play_mus_thread, (LPVOID)i, 0, &tid);
...
bool bkg_snd_play(char const* fname, bool loop /* = false */) {
	...
	int i = 0;
	while (true) {
		if ((pdat->status == mus_dat::PMT_PLAYING) && (pdat->used == true))
			break;
		else if ((pdat->errcode == mus_dat::OP_FAILED) || (pdat->errcode == mus_dat::OP_FILE_NOT_FOUND))
			break;
		else
			Sleep(1);
...
void bkg_snd_stop() {
	...
	int i = 0;
	while (true) {
		if ((pdat->status == mus_dat::PMT_FREE) && (pdat->used == false))
			break;
		else
			Sleep(1);
