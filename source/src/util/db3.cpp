...
TDt& TDt::operator+=(int i) {
	int j = GetJulian(m_month, m_day, m_year);
	j += i;
...
std::string TDt::Format(int a_nDisplayFormat) {
	...
	return s.c_str(); // alive as int as the object is
}
...
//
int TDt::IsLeap() {
	return ((m_year >= 1582) ? (m_year % 4 == 0 && m_year % 100 != 0 || m_year % 400 == 0) : (m_year % 4 == 0));
}
...
int TDt::YYMM() { // integer yymm ( 9201 )
	int nY = Year();
	if (nY > 1999) {
		nY -= 2000;
	} else if (nY > 1900) {
...
// d1 - d2 = result
int TDt::DifferenceInYears(int d1, int d2) {
	TDt dt1 = d1;
	TDt dt2 = d2;
...
// d1 - d2 = result
int TDt::DifferenceInMonths(int d1, int d2) {
	TDt dt1 = d1;
	TDt dt2 = d2;
...
void TDt::GetMDY(int& nm, int& nd, int& ny, int jul) {
	if (jul == 0) {
		nm = 0;
		nd = 0;
		ny = 0;
...
int TDt::GetJulian(int am, int ad, int ay) {
	double m, y;
	int jul;
	...
	jul = (int)(floor(365.25 * (y)) + floor(30.6001 * m) + ad);
	jul -= 443;
...
void CSQLBaseWhere::SetWhere(const char* columnName, int value, char a_DataType) {
	...
	if (a_DataType == CSQLBaseWhere::LONG) {
		...
		ss << dec << value;
		_where += ss.str(); // unfinished: test this
	} else if (a_DataType == CSQLBaseWhere::ODBC_DATE || a_DataType == CSQLBaseWhere::JULIAN_DATE) {
		TDt dt(value);
...
void CSQLInsert::SetColumn(const char* columnName, const char* a_chValue, char a_DataType) {
	...
	if (a_chValue[0] == '\0') {
		...
		_values += "NULL";
	} else if (strlen(a_chValue) >= 3 && a_chValue[0] == '{' && a_chValue[1] == 'd' && a_chValue[2] == '\'') // {d'1972-01-15'} etc.
	{
		_values += a_chValue;
	...
	} else // otherwise single quote all strings
	{
		if (a_DataType == STRING) {
			...
			_values += cs;
			_values += "'";
		} else if (a_DataType == ODBC_DATE || a_DataType == DATE_MMDDYYYY) {
			TDt dt(a_chValue);
...
void CSQLInsert::SetColumn(const char* columnName, int value, char a_DataType) {
	if (_columns != "")
		_columns += ", ";
	_columns += columnName;
	...
	if (a_DataType == LONG) {
		...
		if (_values != "")
			_values += ", ";
		_values += ach;
	} else if (a_DataType == ODBC_DATE || a_DataType == JULIAN_DATE) {
		TDt dt(value);
		std::string cs = dt.Format(TDt::ODBC);
		if (_values != "")
			_values += ", ";
...
int CSQLRecordset::lSQLGetData(int column, int a_eDataType) {
