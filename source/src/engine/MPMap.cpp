  ...
  m_pSkyDoomVB(NULL)
{
	...
	_pDefaultTile->TexLayer[1].btTexNo   = 255; // Indicates DefaultTile
	_pDefaultTile->fHeight				 = UNDERWATER_HEIGHT;
	_pDefaultTile->dwColor				 = 0xffffffff;
    // _pDefaultTile->TexLayer

	// _ActiveSectionArray will be dynamically sized in Load()
...
MPMap::~MPMap()
{
    ...
    delete[] _pVertBuf;
#endif
    // PKO FIX: Free sky dome vertex buffer to prevent memory leak
    SAFE_DELETE_ARRAY(m_pSkyDoomVB);
...
void MPMap::Render()
{
	...
	g_Render.SetSamplerState(0, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC);
	g_Render.SetSamplerState(0, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC);
	g_Render.SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	g_Render.SetSamplerState(0, D3DSAMP_MAXANISOTROPY, 8);
	...
	g_Render.SetSamplerState(1, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC);
	g_Render.SetSamplerState(1, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC);
	g_Render.SetSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR);
	g_Render.SetSamplerState(1, D3DSAMP_MAXANISOTROPY, 8);
...
void MPMap::_GenerateTerrainGroup(int nTileStartX, int nTileStartY)
{
    ...
    for(short y = 0; y < _nShowHeight; y++)
	{
		for(short x = 0; x < _nShowWidth; x++)
		{
			...
			{
				_bRenderSea = TRUE; // Need to render water
			}

			// Bounds check for tile buffers
			if (x < _nTileBufferWidth && y < _nTileBufferHeight)
			{
				_fTileHeightBuffer[y * _nTileBufferWidth + x] = pTile->fHeight;
				_sTileRegionAttr[y * _nTileBufferWidth + x] = pTile->sRegion;
			}
            
			int gx = x * 2;
            int gy = y * 2;
			
			// Bounds check for height/block buffers (2x tile dimensions)
			if (gx + 1 < _nHeightBufferWidth && gy + 1 < _nHeightBufferHeight)
			{
				_fHeightBuffer[gy * _nHeightBufferWidth + gx] = pTile->getObjHeight(0);
				_fHeightBuffer[gy * _nHeightBufferWidth + gx + 1] = pTile->getObjHeight(1);
				_fHeightBuffer[(gy + 1) * _nHeightBufferWidth + gx] = pTile->getObjHeight(2);
				_fHeightBuffer[(gy + 1) * _nHeightBufferWidth + gx + 1] = pTile->getObjHeight(3);
				
				_btBlockBuffer[gy * _nHeightBufferWidth + gx] = pTile->IsBlock(0);
				_btBlockBuffer[gy * _nHeightBufferWidth + gx + 1] = pTile->IsBlock(1);
				_btBlockBuffer[(gy + 1) * _nHeightBufferWidth + gx] = pTile->IsBlock(2);
				_btBlockBuffer[(gy + 1) * _nHeightBufferWidth + gx + 1] = pTile->IsBlock(3);
			}
...

	{
		...
		const int bufferWidth = m_pBlock->m_nBlockBufferWidth;
		const int bufferHeight = m_pBlock->m_nBlockBufferHeight;
		...
		for (short y = 0; y < m_iRange; y++)
		{
			for (short x = 0; x < m_iRange; x++)
			{
				...
				// Bounds check for tile region attr
				if (x < bufferWidth && y < bufferHeight)
				{
					m_pBlock->m_sTileRegionAttr[y * bufferWidth + x] = pBlockData->sRegion;
				}

				int gx = x * 2;
				int gy = y * 2;

				// Bounds check for block buffer (which is 2x larger)
				if (gx + 1 < bufferWidth && gy + 1 < bufferHeight)
				{
					m_pBlock->m_btBlockBuffer[gy * bufferWidth + gx] = pBlockData->IsBlock(0);
					m_pBlock->m_btBlockBuffer[gy * bufferWidth + gx + 1] = pBlockData->IsBlock(1);
					m_pBlock->m_btBlockBuffer[(gy + 1) * bufferWidth + gx] = pBlockData->IsBlock(2);
					m_pBlock->m_btBlockBuffer[(gy + 1) * bufferWidth + gx + 1] = pBlockData->IsBlock(3);
				}
			}
		}
	}
