...
LW_RESULT lwRenderCtrlAgent::SetRenderCtrl(DWORD ctrl_type)
{
    ...
    // Validate ctrl_type before attempting to create render controller
    if(ctrl_type == LW_INVALID_INDEX)
    {
        LG("error", "SetRenderCtrl: invalid ctrl_type (LW_INVALID_INDEX), model file may be corrupted");
        goto __ret;
    }
