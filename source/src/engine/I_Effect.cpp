...
#else
void	I_Effect::Init(IDirect3DDeviceX*	 pDev,EFFECT_TYPE  eType, WPARAM wParam, LPARAM lParam)	
#endif
{ 
	m_pDev = pDev; 
	_eEffectType		= eType;
	_fLength			= 5.0f;//!2��
	_wFrameCount		= (WORD)wParam;	//!֡
...
#else
CEffectModel::CEffectModel(IDirect3DDeviceX*  pDev,lwIResourceMgr*	pRes)
#endif
{
	m_pDev = pDev;
	m_strName			= "";
...
#else
void	CEffectModel::InitDevice(IDirect3DDeviceX*  pDev,lwIResourceMgr*	pRes)
#endif
{
	m_pDev = pDev;
	m_pRes = pRes;
...
#else
IDirect3DDeviceX*	CEffectModel::GetDev()
#endif
{
	return m_pDev;
}
...
bool	CEffectModel::CreateTriangle()
{
	...
	//lsvb->Unlock();
	//�����һ�����þ�Ҫlock�Ļ�����Ҫ�ȵ���g_mesh0->LoadVideoMemory();
	//��Ϊ���ǵ�vb��������render��ʱ��
	_dwVerCount = 3;
	_dwFaceCount= 1;
...
bool	CEffectModel::CreateShadeModel(WORD wVerNum , WORD wFaceNum,int iGridCrossNum,bool usesoft)
{
	...
	{
		mi.vercol_seq[n] = 0xffffffff;
		mi.vertex_seq[n] = lwVector3(0,0,0);//!����ȫ��Ϊ��
...
#endif
{
	...
	if(id < 0)
	{
		LG("error", "msgCEffectFont����%sû���ҵ�",m_vecTexName.c_str());
		m_lpCurTex = NULL;
		m_pTex = NULL;
	}else
	{
	...
	if(id < 0)
	{
		LG("error", "msgCEffectFont����%sû���ҵ�",_strBackBmp.c_str());
		_lpBackTex = NULL;
	}else
	{
#ifdef USE_RENDER
