...
{
    OutputDebugStringA("PKO: lwItem::Load() - start\n");
    if( _obj )
    {
...
__a:
        ...
        char debugBuf[512];
        sprintf(debugBuf, "PKO: lwItem::Load() - loading path: %s\n", path);
        ...
        OutputDebugStringA("PKO: lwItem::Load() - creating lwPrimitive\n");
        // create new item object
        lwPrimitive* imp = LW_NEW( lwPrimitive( _res_mgr ) );

        OutputDebugStringA("PKO: lwItem::Load() - loading lwGeomObjInfo\n");
        // begin load mesh
        lwGeomObjInfo info;

        if( LW_FAILED( info.Load( path ) ) )
        {
            OutputDebugStringA("PKO: lwItem::Load() - info.Load FAILED\n");
            return LW_RET_FAILED;
        }
        OutputDebugStringA("PKO: lwItem::Load() - info.Load succeeded\n");

        OutputDebugStringA("PKO: lwItem::Load() - imp->Load starting\n");
        if( LW_FAILED( imp->Load( &info, path_info->GetPath( PATH_TYPE_TEXTURE_ITEM ), &res ) ) )
        {
            OutputDebugStringA("PKO: lwItem::Load() - imp->Load FAILED\n");
            return LW_RET_FAILED;
        }
        OutputDebugStringA("PKO: lwItem::Load() - imp->Load succeeded\n");
...
{    
    ...
    if(_obj)
    {
        // Outline is character-physique only (lwPhysique). Items — including
        // move/quest markers and equipped weapons — never run inverted-hull.
