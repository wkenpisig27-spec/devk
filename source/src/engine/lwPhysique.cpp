...
LW_RESULT lwPhysique::LoadBone( const char* file )
{
    ...
    else
    {
        ...
        {
            // PKO FIX: Clean up allocated resources on RegisterTask failure
            delete bi;
            LW_SAFE_RELEASE(i_data);
            LW_SAFE_RELEASE(ctrl_bone);
            decCount();
            char szData[128];
...
#else
        lwIAnimCtrlBone* ctrl_bone = NULL;  // PKO FIX: init to NULL for safe cleanup
        bool bOwnIData = false;
        ...
        goto __addr_2;
__addr_1:
        // PKO FIX: Release resources on error to prevent memory leak
        if (bOwnIData) { LW_SAFE_RELEASE(i_data); }
        LW_SAFE_RELEASE(ctrl_bone);
        goto __ret;
__addr_2:
        ;
...
LW_RESULT lwPhysique::LoadPrimitive( DWORD part_id, const char* file )
{
    ...
    // Check if mesh was loaded successfully before using it
    lwIMesh* loadedMesh = mesh_agent->GetMesh();
    if(loadedMesh == NULL)
    {
        LG("error", "lwPhysique::LoadPrimitive - Failed to load mesh for part %d, file: %s", part_id, path);
        LW_SAFE_RELEASE(mesh_agent);
        LW_SAFE_RELEASE(imp);
        return LW_RET_FAILED;
    }
    loadedMesh->SetResFile( &rfm );
    ...
    // =======================
    // create render ctrl agent
    if(LW_FAILED(imp->LoadRenderCtrl(&pInfo->rcci)))
    {
        LG("error", "lwPhysique::LoadPrimitive - LoadRenderCtrl failed for part %d (ctrl_id=0x%08X)", part_id, pInfo->rcci.ctrl_id);
        // Continue without render ctrl - the model will render with default settings
    }
    // state ctrl
...
LW_RESULT lwPhysique::Update()
{
    ...
    if( _anim_agent )
    {
        ...
        for( DWORD i = 0; i < LW_MAX_SUBSKIN_NUM; i++ )
        {
            ...
            // PKO FIX: Null-check GetMeshAgent() and GetMesh() to prevent crash
            // during map transitions when meshes may not be loaded yet
            lwIMeshAgent* meshAgent = pri->GetMeshAgent();
            if (!meshAgent || !meshAgent->GetMesh())
                continue;

            if(LW_FAILED(ctrl_obj->UpdateObject(pri_ctrl, meshAgent->GetMesh())))
                goto __ret;
        }
    }    
