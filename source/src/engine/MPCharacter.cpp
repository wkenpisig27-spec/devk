...
lwIPoseCtrl* MPCharacter::GetPoseCtrl()
{
...

	{
			...
			goto __ret;
		{
			...
			// PKO FIX: Null-check GetAnimCtrl() before chaining GetPoseCtrl()
			{
				lwIAnimCtrl* anim_ctrl = ctrl_obj->GetAnimCtrl();
				if (anim_ctrl == NULL)
					goto __ret;
				ret = anim_ctrl->GetPoseCtrl();
			}
		}
	}
__ret:
...
lwIPoseCtrl* MPCharacter::GetObjImpPoseCtrl( DWORD skin_id, DWORD ctrl_type )
{
    ...
    // PKO FIX: Null-check anim_agent to prevent crash
    if(anim_agent == NULL)
        return NULL;
    ...
    case ANIM_CTRL_TYPE_MAT:
        {
            ...
            // PKO FIX: Null-check GetAnimCtrl() before GetPoseCtrl()
            lwIAnimCtrl* ac = ctrl_obj->GetAnimCtrl();
            return ac ? ac->GetPoseCtrl() : NULL;
        }
        break;
    case ANIM_CTRL_TYPE_BONE:
        {
...
{
    // PKO FIX: Null-check all chained pointers to prevent crash
    lwIAnimCtrlAgent* anim_agent = _physique->GetAnimCtrlAgent();
    if(anim_agent == NULL)
        return NULL;
...
DWORD MPCharacter::CheckPosePlayingStatus()
{
    ...
    // PKO FIX: Null-check GetPlayPoseInfo() to prevent crash
    lwPlayPoseInfo* ppi = GetPlayPoseInfo();
    if(ppi == NULL)
        return 0;

    return ctrl->IsPosePlaying( ppi );
}
