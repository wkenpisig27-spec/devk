...
static void lwTexInfo_LoadFromFile(lwTexInfo* info, FILE* fp)
{
    ...
    fread(&info->level, sizeof(DWORD), 1, fp);
    fread(&info->usage, sizeof(DWORD), 1, fp);
    fread(&info->format, sizeof(D3DFORMAT), 1, fp);
    fread(&info->pool, sizeof(D3DPOOL), 1, fp);
    fread(&info->byte_alignment_flag, sizeof(DWORD), 1, fp);
    fread(&info->type, sizeof(DWORD), 1, fp);
    fread(&info->width, sizeof(DWORD), 1, fp);
    fread(&info->height, sizeof(DWORD), 1, fp);
    fread(&info->colorkey_type, sizeof(DWORD), 1, fp);
    fread(&info->colorkey, sizeof(lwColorValue4b), 1, fp);
    fread(&info->file_name, LW_MAX_NAME, 1, fp);
    
    // Read the 4-byte data field from file into a temp, then assign to void*
    DWORD data_placeholder;
    fread(&data_placeholder, sizeof(DWORD), 1, fp);
    info->data = reinterpret_cast<void*>(static_cast<uintptr_t>(data_placeholder));
    
    fread(&info->tss_set[0], sizeof(lwRenderStateAtom) * LW_TEX_TSS_NUM, 1, fp);
}
...
{
    char buf[512];
    sprintf(buf, "PKO: lwMtlTexInfo_Load - sizeof(lwTexInfo)=%zu, sizeof(tex_seq)=%zu, file pos=%ld\n", 
    ...
    if(version >= EXP_OBJ_VERSION_1_0_0_0)
    {
        ...
        // Read each lwTexInfo individually to handle x86/x64 pointer size difference
    ...
    else if(version == MTLTEX_VERSION0000)
    {
        ...
        for(DWORD i = 0; i < LW_MAX_TEXTURESTAGE_NUM; i++)
        {
            ...
            t->usage = 0;
            // PKO FIX: Use D3DPOOL_MANAGED for all texture stages, not just stage 0
            // D3DPOOL_DEFAULT textures are lost on device reset (fullscreen toggle)
            // and cause invisible textures on characters/NPCs
...
LW_RESULT lwAnimDataBone::GetValue(lwMatrix44* mat, DWORD bone_id, float frame, DWORD start_frame, DWORD end_frame)
{
    ...
    // by lsh
    // ����frame > _frame_num û����">="����Ϊ�����ڿ���PLAY_LOOP�����ʱ
    // ��Ҫ��ֵ��0֡�͵�_data._frame_num - 1֡������
    if(frame < 0 || frame > _frame_num)
        goto __ret;

    {
        ...
        else // use linear interpolate
        {
            ...
            if (max_f == _frame_num)
            {
                // �������ǲ�ֵ��0֡�͵�_data._frame_num - 1֡������
                max_f = 0;
                //max_f = _frame_num - 1; �����߽�ض�
            }
...
{
    OutputDebugStringA("PKO: lwMeshInfo_Load - start\n");
    char buf[256];
    sprintf(buf, "PKO: lwMeshInfo_Load - file pos = %ld, version = %u\n", ftell(fp), version);
    OutputDebugStringA(buf);
    ...
    // header
    OutputDebugStringA("PKO: lwMeshInfo_Load - reading header\n");
    if(version >= EXP_OBJ_VERSION_1_0_0_4)
    {
        sprintf(buf, "PKO: lwMeshInfo_Load - version >= 1_0_0_4, header size = %zu\n", sizeof(info->header));
        OutputDebugStringA(buf);
    ...
    // Validate FVF after loading header - catch struct alignment issues early
    // Valid FVF values should be < 0x10000 (typical combinations of D3DFVF flags)
    // Values like 0x3F801544 indicate reading float data as FVF (struct misalignment)
    if(info->fvf > 0x10000)
    {
        sprintf(buf, "PKO: lwMeshInfo_Load - ERROR: Invalid FVF 0x%08X detected (possible struct alignment issue or corrupted file)", info->fvf);
        OutputDebugStringA(buf);
        LG("error", "%s", buf);
        // Don't try to recover - return failure so the mesh isn't used with invalid data
        return LW_RET_FAILED;
    }
...
{
    OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - reading header\n");
    fread((lwGeomObjInfoHeader*)&id, sizeof(lwGeomObjInfoHeader), 1, fp);

    char buf[256];
    sprintf(buf, "PKO: lwGeomObjInfo::Load(fp) - header read: mtl_size=%u, mesh_size=%u, helper_size=%u, anim_size=%u\n", 
    ...
    if(mtl_size> 100000)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - mtl_size too large, FAILED\n");
        return LW_RET_FAILED;
    }
    // end
    ...
    if(mtl_size > 0)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - loading mtl data\n");
        lwLoadMtlTexInfo(&mtl_seq, &mtl_num, fp, version);
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - mtl data loaded\n");
    }

    // read mesh data
    if(mesh_size > 0)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - loading mesh data\n");
        lwMeshInfo_Load(&mesh, fp, version);
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - mesh data loaded\n");
    }

    // read helper datas
    if(helper_size > 0)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - loading helper data\n");
        helper_data.Load(fp, version);
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - helper data loaded\n");
    }

    // read animation data
    if(anim_size > 0)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - loading anim data\n");
        anim_data.Load(fp, version);
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - anim data loaded\n");
    }

    OutputDebugStringA("PKO: lwGeomObjInfo::Load(fp) - all data loaded\n");
...
{
    OutputDebugStringA("PKO: lwGeomObjInfo::Load(file) - opening file\n");
    FILE* fp = fopen(file, "rb");
    if(fp == NULL)
    {
        OutputDebugStringA("PKO: lwGeomObjInfo::Load(file) - fopen FAILED\n");
        return LW_RET_FAILED;
    }
    ...
    OutputDebugStringA("PKO: lwGeomObjInfo::Load(file) - Load(fp) returned\n");
