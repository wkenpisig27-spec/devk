...
unsigned int __stdcall __thread_proc_load_tex(void* param)
{
    ...
    if(LW_FAILED(tex->LoadSystemMemory()))
    {
        ...
        // PKO: Log texture file that failed to load system memory
        char buf[512];
        sprintf(buf, "PKO ERROR: LoadSystemMemory FAILED for texture: %s\n", tex->GetFileName());
    ...
    if(LW_FAILED(tex->LoadVideoMemory()))
    {
        ...
        // PKO: Log texture file that failed to load video memory
        char buf[512];
        sprintf(buf, "PKO ERROR: LoadVideoMemory FAILED for texture: %s\n", tex->GetFileName());
        OutputDebugStringA(buf);
    }
...
LW_RESULT lwTex::BeginPass()
{
    ...
    else if(_load_type == LOADINGRES_TYPE_RUNTIME_MT)
    {
        if(!(_load_mask & LOADINGRES_MASK_RTMT0))
        {
            ...
            {
                // PKO FIX: If thread pool registration fails, fall back to synchronous loading
                OutputDebugStringA("PKO WARNING: ThreadPool task registration FAILED - falling back to sync load\n");
                SetLoadResMask(LOADINGRES_MASK_RTMTREG_FAILED, 0);
                
                // Load synchronously as fallback
                if(LW_FAILED(LoadSystemMemory()))
                {
                    SetLoadResMask(LOADINGRES_MASK_LOADSM_FAILED, 0);
                }
                if(LW_FAILED(LoadVideoMemory()))
                {
                    SetLoadResMask(LOADINGRES_MASK_LOADVM_FAILED, 0);
                }
                SetLoadResMask(LOADINGRES_MASK_RTMT1, 0);
            }
    ...
    // PKO FIX: Skip rendering if texture handle is NULL (failed to load)
    if(_tex == NULL && !(_state & RES_STATE_VIDEOMEMORY))
    {
        ret = RES_PASS_SKIPTHISDRAW;
        goto __ret;
    }
...
{
    // PKO FIX: Safety check - never set a NULL texture
    // This can happen if texture loading failed but BeginPass still returned RES_PASS_DEFAULT
    if(_tex == NULL)
    {
...
LW_RESULT lwTex::LoseDevice()
{
    ...
    // PKO FIX: Clear load mask so runtime textures can reload after device reset
    _load_mask = 0;
    
    // Only unload D3DPOOL_DEFAULT textures (they MUST be recreated after device reset)
    // D3DPOOL_MANAGED textures are automatically restored by DirectX
    if(_pool == D3DPOOL_DEFAULT)
    {
        if(LW_FAILED(UnloadVideoMemory()))
            goto __ret;
...
LW_RESULT lwTex::ResetDevice()
{
    ...
    // Only reload D3DPOOL_DEFAULT textures (they were unloaded in LoseDevice)
    // D3DPOOL_MANAGED textures are automatically restored by DirectX
    if(_pool == D3DPOOL_DEFAULT)
    {
        if(LW_FAILED(LoadVideoMemory()))
...
{
    OutputDebugStringA("PKO: lwMtlTexAgent::LoadMtlTex - start\n");
    LW_RESULT ret = LW_RET_FAILED;
    ...
    for(DWORD j = 0; j < LW_MAX_MTL_TEX_NUM; j++)
    {
        ...
        char buf[512];
        sprintf(buf, "PKO: lwMtlTexAgent::LoadMtlTex - loading texture stage %u, file: %s\n", j, i->tex_seq[j].file_name);
        OutputDebugStringA(buf);

        if(LW_FAILED(LoadTextureStage(&i->tex_seq[j], tex_path)))
        {
            OutputDebugStringA("PKO: lwMtlTexAgent::LoadMtlTex - LoadTextureStage FAILED\n");
            goto __ret;
        }
    }

    OutputDebugStringA("PKO: lwMtlTexAgent::LoadMtlTex - SUCCESS\n");
    ret = LW_RET_OK;
__ret:
    return ret;
}
...
{
    OutputDebugStringA("PKO: LoadTextureStage - start\n");
    LW_RESULT ret = LW_RET_FAILED;
    ...
    char buf[256];
    sprintf(buf, "PKO: LoadTextureStage - stage=%u, LW_MAX_MTL_TEX_NUM=%d\n", stage, LW_MAX_MTL_TEX_NUM);
    OutputDebugStringA(buf);

    if(stage < 0 || stage >= LW_MAX_MTL_TEX_NUM)
    {
        OutputDebugStringA("PKO: LoadTextureStage - stage out of range\n");
        goto __ret;
    }

    if(_tex_seq[stage] != NULL)
    {
        OutputDebugStringA("PKO: LoadTextureStage - tex_seq[stage] already set\n");
        goto __ret;
    }

    OutputDebugStringA("PKO: LoadTextureStage - calling CreateTex\n");
    if(LW_FAILED(_res_mgr->CreateTex(&obj)))
    {
        OutputDebugStringA("PKO: LoadTextureStage - CreateTex FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: LoadTextureStage - CreateTex OK\n");
    
    OutputDebugStringA("PKO: LoadTextureStage - calling LoadTexInfo\n");
    if(LW_FAILED(obj->LoadTexInfo(info, tex_path)))
    {
        OutputDebugStringA("PKO: LoadTextureStage - LoadTexInfo FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: LoadTextureStage - LoadTexInfo OK\n");
    ...
    OutputDebugStringA("PKO: LoadTextureStage - SUCCESS\n");
    ret = LW_RET_OK;
__ret:
    return ret;
...
LW_RESULT lwThreadPoolMgr::Create()
{
    ...
    // PKO: Increased thread pool capacity for smoother texture loading
    // 4 threads for parallel loading, 8192 task queue for handling busy areas
...
LW_RESULT lwResourceMgr::CreateRenderCtrlVS(lwIRenderCtrlVS** ret_obj, DWORD type)
{
    ...
    // Bounds check to prevent crash from invalid ctrl_id (e.g., LW_INVALID_INDEX = 0xFFFFFFFF)
    if(type >= LW_RENDER_CTRL_PROC_NUM)
    {
        LG("error", "CreateRenderCtrlVS: invalid render ctrl type %u (0x%08X), max is %d", 
           type, type, LW_RENDER_CTRL_PROC_NUM);
        goto __ret;
    }
...
LW_RESULT lwResourceMgr::RegisterTex(lwITex* obj)
{
    ...
    {
        // PKO: Log texture pool full - this causes missing textures!
        OutputDebugStringA("PKO WARNING: Texture pool FULL! Cannot register texture.\n");
        goto __ret;
    }
