...
BOOL MPGameApp::Init(HINSTANCE hInst,const char *pszClassName, int nScrWidth, int nScrHeight, int nColorBit, BOOL bFullScreen)
{
	...
	OutputDebugStringA("PKO: MPGameApp::Init - calling g_Render.Init()...\n");
   	if(!g_Render.Init(_hWnd, dev_width, dev_height, nColorBit, bFullScreen))
    {
        return FALSE;
    }
	OutputDebugStringA("PKO: MPGameApp::Init - g_Render.Init() done\n");

	OutputDebugStringA("PKO: MPGameApp::Init - creating MPTexSet(0, 8192)...\n");
	MPTexSet *pTextureSet = new MPTexSet(0, 8192);
	OutputDebugStringA("PKO: MPGameApp::Init - MPTexSet created\n");
	
	_nLogoTexID = GetTextureID("texture/logo.BMP");

    pTextureSet->EnableRequest(FALSE);

	OutputDebugStringA("PKO: MPGameApp::Init - calling _InitInput()...\n");
	_InitInput();
	OutputDebugStringA("PKO: MPGameApp::Init - _InitInput() done\n");
...
{
	OutputDebugStringA("PKO: MPGameApp::LoadResource() - calling g_Render.InitResource()...\n");
	if (!g_Render.InitResource())
	{
		return FALSE;
	}
	OutputDebugStringA("PKO: MPGameApp::LoadResource() - done\n");
	return TRUE;
}

BOOL MPGameApp::LoadRes2()
{
	OutputDebugStringA("PKO: MPGameApp::LoadRes2() - calling g_Render.InitRes2()...\n");
	if (!g_Render.InitRes2())
	{
		return FALSE;
	}
	OutputDebugStringA("PKO: MPGameApp::LoadRes2() - done\n");
	return TRUE;
}
...
*/
	...
	if(_dwMouseKey & M_Down)
	{
		...
		if(_dwMouseKey & M_RDown)
			MouseButtonDown(1);
		// Middle mouse button (2) intentionally ignored - no functionality
	}
	...
	if(_dwMouseKey & M_RDB)
		MouseButtonDB(1);
	// Middle mouse double-click intentionally ignored

	if(_dwMouseKey & M_LUp)
		MouseButtonUp(0);
	if(_dwMouseKey & M_RUp)
		MouseButtonUp(1);
	// Middle mouse up intentionally ignored
}
