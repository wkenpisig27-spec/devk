...
LW_RESULT lwPrimitive::LoadMtlTex( DWORD mtl_id, lwMtlTexInfo* info, const char* tex_path )
{
    ...
    char buf[256];
    sprintf(buf, "PKO: LoadMtlTex - mtl_id=%u, LW_MAX_SUBSET_NUM=%d\n", mtl_id, LW_MAX_SUBSET_NUM);
    OutputDebugStringA(buf);

    if( mtl_id < 0 || mtl_id >= LW_MAX_SUBSET_NUM )
    {
        OutputDebugStringA("PKO: LoadMtlTex - mtl_id out of range\n");
        goto __ret;
    }

    OutputDebugStringA("PKO: LoadMtlTex - calling CreateMtlTexAgent\n");
    if( LW_FAILED( _res_mgr->CreateMtlTexAgent( &_mtltex_agent_seq[ mtl_id ] ) ) )
    {
        OutputDebugStringA("PKO: LoadMtlTex - CreateMtlTexAgent FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: LoadMtlTex - CreateMtlTexAgent OK\n");

    OutputDebugStringA("PKO: LoadMtlTex - calling agent->LoadMtlTex\n");
    if( LW_FAILED( _mtltex_agent_seq[ mtl_id ]->LoadMtlTex( info, tex_path ) ) )
    {
        OutputDebugStringA("PKO: LoadMtlTex - agent->LoadMtlTex FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: LoadMtlTex - agent->LoadMtlTex OK\n");
...
{
    OutputDebugStringA("PKO: lwPrimitive::Load - start\n");
    LW_RESULT ret = LW_RET_FAILED;
    ...
    OutputDebugStringA("PKO: lwPrimitive::Load - calling LoadRenderCtrl\n");
    if(LW_FAILED(LoadRenderCtrl(&info->rcci)))
    {
        OutputDebugStringA("PKO: lwPrimitive::Load - LoadRenderCtrl FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: lwPrimitive::Load - LoadRenderCtrl OK\n");
    ...
    // mtltex info
    OutputDebugStringA("PKO: lwPrimitive::Load - loading mtltex\n");
    for( i = 0; i < info->mtl_num; i++ )
    {
        if( LW_FAILED( LoadMtlTex( i, &info->mtl_seq[ i ], tex_path ) ) )
        {
            OutputDebugStringA("PKO: lwPrimitive::Load - LoadMtlTex FAILED\n");
            goto __ret;
        }
    }
    OutputDebugStringA("PKO: lwPrimitive::Load - mtltex OK\n");

    // mesh info
    OutputDebugStringA("PKO: lwPrimitive::Load - loading mesh\n");
    if( LW_FAILED( LoadMesh( &info->mesh ) ) )
    {
        OutputDebugStringA("PKO: lwPrimitive::Load - LoadMesh FAILED\n");
        goto __ret;
    }
    OutputDebugStringA("PKO: lwPrimitive::Load - mesh OK\n");
    ...
    if( res )
    {
        ...
        if(mesh)
            mesh->SetResFile( &info );
    }

    // helper info
    if( info->helper_data.type != HELPER_TYPE_INVALID )
    {
        OutputDebugStringA("PKO: lwPrimitive::Load - loading helper\n");
    ...
    if( info->anim_size > 0 )
    {
        OutputDebugStringA("PKO: lwPrimitive::Load - loading anim\n");
        if( LW_FAILED( LoadAnimData( &info->anim_data, tex_path, res ) ) )
        {
            OutputDebugStringA("PKO: lwPrimitive::Load - LoadAnimData FAILED\n");
            goto __ret;
        }
        OutputDebugStringA("PKO: lwPrimitive::Load - anim OK\n");
    }


    ret = LW_RET_OK;
    OutputDebugStringA("PKO: lwPrimitive::Load - SUCCESS\n");
__ret:
    return ret;
}
...
LW_RESULT lwPrimitive::LoadRenderCtrl(const lwRenderCtrlCreateInfo* rcci)
{
    ...
    {
        // PKO FIX: Release render_agent on failure to prevent memory leak
        LW_SAFE_RELEASE(render_agent);
        goto __ret;
    }
...
LW_RESULT lwPrimitive::LoadAnimData( lwIAnimDataInfo* data_info, const char* tex_path, const lwResFile* res )
{
    ...
    if(info->anim_mat)
    {
        ...
        { LW_SAFE_RELEASE(ctrl_obj); goto __ret; } // PKO FIX: prevent leak

        if(LW_FAILED(ctrl->LoadData(data)))
        { LW_SAFE_RELEASE(ctrl); LW_SAFE_RELEASE(ctrl_obj); goto __ret; } // PKO FIX: prevent leak
...
LW_RESULT lwPrimitive::Clone( lwIPrimitive** ret_obj )
{
    ...
    *ret_obj = o;
    ...
    ret = LW_RET_OK;
__ret:
    // PKO FIX: Release partially-cloned object on failure to prevent memory leak
    ...
    if(_anim_agent)
    {
        ...
        for(DWORD i = 0; i < anim_ctrl_num; i++)
        {
            ...
            case ANIM_CTRL_TYPE_BONE:
                {
                    // PKO FIX: Null-check _mesh_agent and GetMesh() to prevent crash during async loading
                    if(!_mesh_agent || !_mesh_agent->GetMesh())
                        goto __ret;
...
//#if(defined MULTITHREAD_LOADING_MESH)
    // PKO FIX: Null-check _mesh_agent, GetMesh(), and _render_agent to prevent crash
    if(!_mesh_agent || !_mesh_agent->GetMesh() || _mesh_agent->GetMesh()->IsLoadingOK() == 0)
    {
        goto __addr_ret_ok;
    }
    if(!_render_agent)
...
LW_RESULT lwPrimitive::RenderSubset( DWORD subset )
{
    ...
    // PKO FIX: Null-check _render_agent and _mesh_agent to prevent crash
