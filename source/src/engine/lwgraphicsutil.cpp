...
LW_RESULT _ConvertMeshDataVB(void** data, DWORD* size, DWORD* stride, const lwMeshInfo* info)
{
    // Check for invalid/uninitialized mesh (FVF = 0 means no vertex format)
    if(info->fvf == 0)
    {
        LG("error", "_ConvertMeshDataVB: mesh has no FVF format (0), skipping - likely corrupted model file");
        *data = NULL;
        *size = 0;
        *stride = 0;
        return LW_RET_FAILED;
    ...
    case FVF_XYZB4UBYTE4NORMALTEX2: // bin: 1001000011100,  Dec: 4636
        ...
        break;
    default:
        // Log invalid FVF but don't block with MessageBox - this can happen with corrupted/misaligned model files
        // FVF values like 0x3F801544 indicate struct alignment issues (looks like float data read as FVF)
        LG("error", "_ConvertMeshDataVB: Unknown FVF format 0x%08X (%u) - possible struct alignment issue or corrupted model file", 
           info->fvf, info->fvf);
        return LW_RET_FAILED;
    }
