...
__ret:
    // PKO FIX: Release model_node on failure to prevent memory leak
    if(ret == TREENODE_PROC_RET_ABORT && model_node)
    {
        model_node->Release();
    }
    return ret;
...
LW_RESULT lwNodePrimitive::Load(lwIGeomObjInfo* geom_info, const char* tex_path, const lwResFile* res)
{
    ...
    // base info
    // warning:ע�⣬�����_type������ʾMODELNODE_������
    // �����õ�������Ϣtype�����ǣ�����
    //_type = info->type;

    // ������Ϣ_id, _link_id, _link_parent_id���Ѿ���lwLoadModelInfo������
    //_id = info->id;
    ...
    */
    if(_parent)
    {
        if(_ref_ctrl_obj_bone)
        {
            ...
            // PKO FIX: Null-check _mesh_agent and GetMesh() to prevent crash
            if(!_mesh_agent || !_mesh_agent->GetMesh())
                goto __ret;
...
// SetParent����parent��VertexBlendCtrlʱ������
// 1.����lwAnimCtrlAgent��ANIM_CTRL_TYPE_BONE�Ķ���
// 2.����_ref_ctrl_obj_bone
// 3.����lwRenderCtrlAgent��vs_type��vs_ctrl����Ϊvs_type�����Զ�������
//   �����޷�ͨ��fvf����������ʽ�����ڲ�ƥ�䣬���ｨ��vs_type��vs_ctrl
//   ���ⲿ��ʾ����
//   ���Բ���Ҫ����vs_type������Ч���Ըߣ�,��Ҫ����vs_ctrl
...
LW_RESULT lwNodeObject_PlayDefaultPose(lwINodeObject* obj)
{
    ...
    ppi.frame = 0.0f;
    ppi.type = PLAY_LOOP;
    // FPS-aware default pose velocity. See lwPhysique.cpp for g_lwAnimVelocity.
...
LW_RESULT lwNodePrimitive_PlayPoseAll(lwINodePrimitive* obj, const lwPlayPoseInfo* info)
{
...

    {
        ...
        for (DWORD i = 0; i < n; i++)
        {
            ...
            // lwAnimCtrlObjBone��lwPrimitive����Ϊ�������Ƕ���������
            ctrl_obj->GetTypeInfo(&type_info);
