...
CMPPartCtrl::CMPPartCtrl(void)
{
	...
	m_iPartNum = 0;
#ifndef USE_GAME
	//m_vecPartSys.resize(MAX_PART_SYS);//�������ϵͳ����Ŀ���ܴ���6
#endif
	m_fLength = 0;//�������Ϊ0����ѭ������
	m_fCurTime = 0;
...
bool	CMPPartCtrl::LoadFromFile(char* pszName)
{
	...
	{
		LG("error", "msg[%s]�İ汾[%d]����ϵͳ�İ汾[%d] (CMPPartCtrl::LoadFromFile)\n",pszName,t_dwVersion,CMPPartCtrl::ParVersion);
		return false;
	}
	if(t_dwVersion < 2)
	{
		LG("error", "msg[%s]�İ汾[%d]����ϵͳ�ܴ����İ汾[%d] (CMPPartCtrl::LoadFromFile)\n",pszName,t_dwVersion,2);
		return false;
	}
	...
	for (int n = 0; n < m_iPartNum; ++n)
	{
		...
		{
			LG("error", "msg[%s]�ĵ�[%d]����ϵͳ����ʧ��.(CMPPartCtrl::LoadFromFile)\n", pszName, n);
			return false;
		}
		if(t_dwVersion < 6)
		{
	...
	if(t_dwVersion >= 7)
	{
		...
		for (int n = 0; n < m_iStripNum; ++n)
		{
			...
			{
				LG("error", "msg[%s]�ĵ�[%d]��Strip����ʧ��.(CMPPartCtrl::LoadFromFile)\n", pszName, n);
				delete[] m_pcStrip;
				m_pcStrip = nullptr;
				return false;
			}
...
bool	CMPPartCtrl::LoadFromMemory(CMemoryBuf*	pbuf)
{
	...
	if ( t_dwVersion > CMPPartCtrl::ParVersion)
	{
		LG("error", "msg�汾[%d]����ϵͳ�İ汾[%d] (CMPPartCtrl::LoadFromFile)\n",t_dwVersion,CMPPartCtrl::ParVersion);
		return false;
	}
	if(t_dwVersion < 2)
	{
		LG("error", "msg�汾[%d]����ϵͳ�ܴ����İ汾[%d] (CMPPartCtrl::LoadFromFile)\n",t_dwVersion,2);
		return false;
	}
...
void	CChaModel::Begin()
{
	...
	m_pDev->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW); // ������Ⱦ
