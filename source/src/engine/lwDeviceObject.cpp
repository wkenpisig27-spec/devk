...
: _sys_graphics(sys_graphics), _d3d(0), _dev(0), _bShadowPass(false),
#if (defined LW_USE_DX9)
...
LW_RESULT lwDeviceObject::SetStreamSource(UINT stream_num, IDirect3DVertexBufferX* stream_data, UINT offset_byte, UINT stride)
{
    ...
    //���ﲻ��ֻ�ж�stream_data������Ҫ�ж�offset_byte,stride
...
LW_RESULT lwDeviceObject::SetIndices(IDirect3DIndexBufferX* index_data, UINT base_vert_index)
{
    ...
    //���ﲻ��ֻ�ж�index_data������Ҫ�ж�base_vert_index
...
LW_RESULT lwDeviceObject::SetRenderState(D3DRENDERSTATETYPE state, DWORD value)
{
    ...
    // During shadow pass, block materials from changing alpha/blend/cull states
    // so character silhouettes render as solid (no hollow spots from alpha discard)
    if (_bShadowPass) {
...
LW_RESULT lwDeviceObject::SetTextureStageState(DWORD stage, D3DTEXTURESTAGESTATETYPE type, DWORD value)
{
    ...
    // During shadow pass, block materials from changing texture stage color/alpha
    // operations on stages 0-1, so our force-black TFACTOR override stays active
