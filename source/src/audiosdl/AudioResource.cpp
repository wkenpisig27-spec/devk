...
ulong AudioRes::get_resID(const char* resource, int type) {
	...
	if (!audio) {
		...
		if (!audio) {
			// Auto-fallback: try alternate extension if file missing
			if (type == TYPE_MP3 || type == TYPE_OGG) {
				std::string path = resource;
				size_t dot = path.find_last_of('.');
				if (dot != std::string::npos) {
					std::string ext = path.substr(dot);
					if (_stricmp(ext.c_str(), ".mp3") == 0) {
						path.replace(dot, 4, ".ogg");
						audio = _addRes(path.c_str(), TYPE_OGG);
					} else if (_stricmp(ext.c_str(), ".ogg") == 0) {
						path.replace(dot, 4, ".mp3");
						audio = _addRes(path.c_str(), TYPE_MP3);
					}
				}
			}
