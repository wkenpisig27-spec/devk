...
extern void lua_callalert(lua_State* L, int status);
...
inline int lua_LG(lua_State* L) {
	...
	// Guard against null pszFile - lua_tostring returns NULL for non-string values
	if (!pszFile) {
		PARAM_ERROR;
		return 0;
	}
