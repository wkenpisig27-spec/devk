...
int TeamMember::GetInvitedMax() {
	return g_gpsvr->const_team.InvitedMax;
}

int TeamMember::JoinTeam(const char* szName) {
	int l_lret = 0;
	if (GetTeam())
		return l_lret;
	Player* ply;
...
int TeamMember::JoinTeam(uLong cha_id) {
	int l_lret = 0;
	if (GetTeam())
		return l_lret;
...
int TeamMember::_BeginRun(Team* team) {
	return RunBiDirectItem<TeamMember>::_BeginRun(team);
}
int TeamMember::_EndRun() {
