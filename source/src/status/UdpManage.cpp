...
bool CUdpManage::Init(int nPort, ReceiveManageEvent evtReceive) {
	...
	DWORD dwThread = 0;
	_hThread = CreateThread(nullptr, 0, ListenThreadProc, this, 0, &dwThread);
	if (_hThread == nullptr)
		return false;
...
void CUdpManage::ReceiveEvent(CUdpServer* pUdpServer, const char* szClientIP, unsigned int nClientPort, const char* pData, int len, void* pParam) {
	...
	if (pThis->_evtReceive)
		pThis->_evtReceive(pThis, pUdpServer, szClientIP, nClientPort, pData, len);
}
