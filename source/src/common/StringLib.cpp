...
// Core implementation - all numeric overloads delegate to this
const char* StringSplitNum(const char* szNumStr, int nCount, char cSplit) {
	static char szBuf[256] = {0};
	static char szTmp[256] = {0};

	sprintf(szBuf, "%s", szNumStr);
	strcpy(szTmp, szBuf);
...
// Overloads convert to string and delegate to core implementation
const char* StringSplitNum(int nNumber, int nCount, char cSplit) {
	char szNum[32];
	sprintf(szNum, "%d", nNumber);
	return StringSplitNum(szNum, nCount, cSplit);
}

const char* StringSplitNum(unsigned int nNumber, int nCount, char cSplit) {
	char szNum[32];
	sprintf(szNum, "%u", nNumber);
	return StringSplitNum(szNum, nCount, cSplit);
}

const char* StringSplitNum(__int64 nNumber, int nCount, char cSplit) {
	char szNum[32];
	sprintf(szNum, "%lld", nNumber);
	return StringSplitNum(szNum, nCount, cSplit);
}
