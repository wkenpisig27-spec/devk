...
void CTerrainAttrib::setTileAttrib(int nX, int nY, unsigned char btAttrib, bool bAdd /* = true */) {
	...
	if (_validateFilePointer() && _validateTile(nX, nY)) {
		...
		_seekTile(nX, nY);
		int offset = sizeof _tile_attrib.attrib;
...
public:
	...
	int SEEK(int offset, int origin) { return fseek(_fp, offset, origin); }
	size_t READ(void* buffer, size_t size, size_t count) { return fread(buffer, size, count, _fp); }
	int TELL() { return ftell(_fp); }
...
inline disk_stuff::disk_stuff(char const* fname, char const* mode) {
	...
	if (_fp == nullptr)
		throw std::runtime_error("disk_stuff constructor");
}
...
inline bool disk_stuff::chfile(char const* fname, char const* mode) {
	...
	_fp = fopen(fname, mode);
	if (_fp == nullptr)
		throw std::runtime_error("disk_stuff chfile");
	return true;
}
...
inline cache_stuff::cache_stuff(char const* fname, char const* mode) try : disk_stuff(fname, mode) {
	...
	throw std::runtime_error("other exception in cache_stuff");
};
...
inline mem_stuff::mem_stuff(char const* fname, char const* mode) try : disk_stuff(fname, mode) {
	...
	throw std::runtime_error("other exception in mem_stuff");
};
...
// read all data into heap
{
	...
	try {
		...
		(ba);
		throw std::runtime_error("no heap to new");
	} catch (...) {
		throw std::runtime_error("other exception to new in mem_stuff::load");
	}
	...
	if (READ(p, 1, len) < size_t(len)) {
		delete p;
		throw std::runtime_error("fread fault in mem_stuff::load");
	}
...
inline terrain_attr_mem::terrain_attr_mem(char const* fname) try : terrain_attr(), mem_stuff(fname, "rb") {
	...
	throw std::runtime_error("exception in terrain_attr_mem constructor");
};
...
inline bool terrain_attr_mem::get_attr(unsigned int x, unsigned int y, unsigned short& attrib) {
	if (_dat == nullptr || x >= _hdr->width || y >= _hdr->height)
		return false;
...
inline bool terrain_attr_mem::has_attr(unsigned int x, unsigned int y, unsigned char attrib_mask) {
	if (_dat == nullptr || x >= _hdr->width || y >= _hdr->height || attrib_mask < 1 || attrib_mask > 16)
		return false;
...
public:
	...
	bool get_trrnattr(char const* fname, unsigned int x, unsigned int y, unsigned short& attr) { return true; }
...
int tamem_mgr::load_map(char const* fname) {
	...
	if (id != INVALID_INDEX)
		...
		try {
			tmp = new terrain_attr_mem(fname);
		} catch (...) {
...
bool s_getTileAttrib(int id, unsigned int x, unsigned int y, unsigned short& attrib) { return TAM_MGR::instance()->get_trrnattr(id, x, y, attrib); }

bool s_hasTileAttrib(int id, unsigned int x, unsigned int y, unsigned char attrib_mask) { return TAM_MGR::instance()->has_trrnattr(id, x, y, attrib_mask); }

bool s_getTileIsland(int id, unsigned int x, unsigned int y, unsigned char& island_index) { return TAM_MGR::instance()->get_isldidx(id, x, y, island_index); }

bool s_getAttribFileInfo(int id, unsigned int& width, unsigned int& height) { return TAM_MGR::instance()->get_info(id, width, height); }
