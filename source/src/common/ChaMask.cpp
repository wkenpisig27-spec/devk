...
void CMaskData::AddMap(const char* pszName, int w, int h) {
...
bool CMaskData::InitMaskData(char* pszMapName, const char* szBase64Data, int llen) {
	sMapMask* pMask;
	BYTE* p;
...
BYTE* CMaskData::GetMapMask(const char* pszMapName, int& size) {
	if (iMapNum == 0)
		return nullptr;
	// sMapMask* pmap;
	sMapMask* pMask;
