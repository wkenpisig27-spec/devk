		...
		case CLU_CHAR :
			{
				...
				lua_Number num = lua_tonumber(virtualMachine, -1);
				if (CLU_IS_UNSIGNED(structs[regStruct]->memberType[i])) {
					*(unsigned char*)dPointers[i] = (unsigned char)num;
				} else {
					*(char*)dPointers[i] = (char)num;
				}
			}break;
		case CLU_SHORT :
			{
				...
				lua_Number num = lua_tonumber(virtualMachine, -1);
				if (CLU_IS_UNSIGNED(structs[regStruct]->memberType[i])) {
					*(unsigned short*)dPointers[i] = (unsigned short)num;
				} else {
					*(short*)dPointers[i] = (short)num;
				}
			}break;
		case CLU_INT :
			{
				...
				lua_Number num = lua_tonumber(virtualMachine, -1);
				if (CLU_IS_UNSIGNED(structs[regStruct]->memberType[i])) {
					*(unsigned int*)dPointers[i] = (unsigned int)num;
				} else {
					*(int*)dPointers[i] = (int)num;
				}
			}break;
		case CLU_FLOAT :
			{
...
int PushStructure(int regStruct, void* data)
{
	...
	for(i = 0; i < st->numMembers; i++)
	{
		...
		case CLU_SHORT :
			{
				if (CLU_IS_UNSIGNED(st->memberType[i]))
					lua_pushnumber(virtualMachine, (double)(*(unsigned short*)marker));
				else
		...
		case CLU_INT :
			{
				if (CLU_IS_UNSIGNED(st->memberType[i]))
					lua_pushnumber(virtualMachine, (double)(*(unsigned int*)marker));
				else
	...
	case CLU_CHAR :
		{
			...
			*size = sizeof(char);
			switch( lua_type(virtualMachine,pos))
			{
				...
				{
					lua_Number num = lua_tonumber(virtualMachine, pos);
					if (CLU_IS_UNSIGNED(cType)) {
						*(unsigned char*)ret = (unsigned char)num;
					} else {
						*(char*)ret = (char)num;
					}
				}
			...
			*size = sizeof(short);
			switch( lua_type(virtualMachine,pos))
			{
				case LUA_TNUMBER :						//Makes perfect sense
				{
					lua_Number num = lua_tonumber(virtualMachine, pos);
					if (CLU_IS_UNSIGNED(cType)) {
						*(unsigned short*)ret = (unsigned short)num;
					} else {
						*(short*)ret = (short)num;
					}
				}
	...
	case CLU_INT :
		{
			...
			*size = sizeof(int);
			switch( lua_type(virtualMachine,pos))
			{
				case LUA_TNUMBER :						//Makes perfect sense
				{
					// Handle unsigned int properly - get as lua_Number (double) to preserve unsigned range
					if (CLU_IS_UNSIGNED(cType)) {
						lua_Number num = lua_tonumber(virtualMachine, pos);
						*(unsigned int*)ret = (unsigned int)num;
					} else {
						*(int*)ret = (int)lua_tointeger(virtualMachine, pos);
					}
				}
				break;
				case LUA_TBOOLEAN :						//Just a 1 or a zero
