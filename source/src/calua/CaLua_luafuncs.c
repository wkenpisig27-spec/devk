...
#ifdef _WIN64
// x64 implementation using type-based dispatching
int Handler(lua_State* vm)
{
	...
	{
		CLU_Error("Unknown function called from Lua script: %s", fName);
		return(0);
	}
	...
	unsigned long long args[16] = {0};
	// Copy stack data to aligned 64-bit array
	if (stack->dataLength > 0) {
		// Ensure we don't overflow args buffer
		int size = stack->dataLength;
		if (size > sizeof(args)) size = sizeof(args);
		memcpy(args, stack->data, size);
	}

	// Determine argument types for first 4 arguments
	int mask = 0;
	for(int i=0; i<4 && i<funcs[fIndex]->numPassArgs; i++) {
        int t = funcs[fIndex]->passType[i];
		int baseType = CLU_C_BASE_TYPE(t);
        if(baseType == CLU_FLOAT || baseType == CLU_DOUBLE)
            mask |= (1 << i);
    }

	void* fPtr = funcs[fIndex]->fPtr;
	unsigned long long retI = 0;
	double retF = 0.0;

	// I = Integer/Pointer (uint64_t), F = Float/Double (double)
	// Note: We cast all Float/Double args to 'double' for the function signature, 
	// as this covers both float (promoted in variadics or passed in XMM) and double.
	// However, for strict correctness with non-variadic float args, passing a double where float is expected *might* be an issue 
	// if the callee reads XMM0 as float (SS) vs double (SD). 
	// But in standard Windows x64 CodeGen, scalars are passed in XMM0.
	// Let's assume standard behavior where strict casting isn't 100% required if we pass via register, 
	// but we'll try to match correctness:
	// Actually, treating them as 'double' is the safest generic bet for 'double' and usually 'float' works if valid.
	
	switch(mask) {
		case 0x0: // IIII
		{
			typedef unsigned long long (*F)(unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long);
			retI = ((F)fPtr)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			// If return is float
			if (CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_FLOAT || CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_DOUBLE) {
				typedef double (*FD)(unsigned long long, ...);
				retF = ((FD)fPtr)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			}
		} break;
		case 0x1: // FIII
		{
			typedef unsigned long long (*F)(double, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long);
			retI = ((F)fPtr)(*(double*)&args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			if (CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_FLOAT || CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_DOUBLE) {
				typedef double (*FD)(double, unsigned long long, ...);
				retF = ((FD)fPtr)(*(double*)&args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			}
		} break;
		case 0x2: // IFII
		{
			typedef unsigned long long (*F)(unsigned long long, double, unsigned long long, unsigned long long, ...);
			retI = ((F)fPtr)(args[0], *(double*)&args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			if (CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_FLOAT || CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_DOUBLE) {
				typedef double (*FD)(unsigned long long, double, ...);
				retF = ((FD)fPtr)(args[0], *(double*)&args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			}
		} break;
		// Cover common cases; fallback to default for complex ones
		case 0xF: // FFFF (e.g. Camera functions)
		{
			typedef unsigned long long (*F)(double, double, double, double, unsigned long long, unsigned long long, ...);
			retI = ((F)fPtr)(*(double*)&args[0], *(double*)&args[1], *(double*)&args[2], *(double*)&args[3], args[4], args[5], args[6]);
			if (CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_FLOAT || CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_DOUBLE) {
				typedef double (*FD)(double, double, double, double, ...);
				retF = ((FD)fPtr)(*(double*)&args[0], *(double*)&args[1], *(double*)&args[2], *(double*)&args[3], args[4], args[5], args[6]);
			}
		} break;
		default: // Fallback: Treat everything as Integer/Ptr. This works for 90% of cases. 
				 // If logic hits here with floats, it might fail (pass in RCX instead of XMM0), but better than crash.
		{
			typedef unsigned long long (*F)(unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long, unsigned long long);
			retI = ((F)fPtr)(args[0], args[1], args[2], args[3], args[4], args[5], args[6], args[7]);
			if (CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_FLOAT || CLU_C_BASE_TYPE(funcs[fIndex]->retType) == CLU_DOUBLE) {
	...
	if(funcs[fIndex]->numRetArgs)
		ParseReturnArgument( funcs[fIndex]->retType, retI, retF);
...
#else
// Original x86 implementation with inline assembly
int Handler(lua_State* vm)
{
	char*			fName;
	...
	if(funcs[fIndex]->numRetArgs) {
		unsigned long long retI = ((unsigned long long)d << 32) | (unsigned int)a;
		ParseReturnArgument( funcs[fIndex]->retType, retI, f);
	}
