...
OUTPUT_META_INC = REPO_ROOT / "source" / "include" / "common" / "OpcodeMetaTable.inc"
...
BAND_CPP = {
    ...
    "SO": "OpcodeBand::SO",
    "TC": "OpcodeBand::TC",
    "out-of-band": "OpcodeBand::Unknown",
}
...
def write_opcode_meta_inc(rows: list[dict[str, str | int]]) -> None:
    lines = [
        "// AUTO-GENERATED by helper/network-tests/scripts/generate_opcode_table.py",
        "// Do not edit — regenerate from NetCommand.h.",
        "",
    ]
    for row in rows:
        band = BAND_CPP.get(str(row["band"]), "OpcodeBand::Unknown")
        is_base = "true" if row["is_base"] == "yes" else "false"
        lines.append(
            f'\t{{ {int(row["value"])}, {band}, {is_base}, "{row["name"]}" }},'
        )
    lines.append("")
    OUTPUT_META_INC.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_META_INC.write_text("\n".join(lines), encoding="utf-8")
