...
// SM4: explicit float4 rows. packoffset on float4x4 is not reliable, and DX9
// vs does dp4 against c1-c4 (rows of the transposed WVP). Reconstruct matrices
// the same way so mul(pos, ViewProj) matches the original asm.
cbuffer PKO_VS : register(b0)
{
    float4 Base                    : packoffset(c0);
    float4 ViewProj0               : packoffset(c1);
    float4 ViewProj1               : packoffset(c2);
    float4 ViewProj2               : packoffset(c3);
    float4 ViewProj3               : packoffset(c4);
    float4 LightDir                : packoffset(c5);
    float4 Ambient                 : packoffset(c6);
    float4 Diffuse                 : packoffset(c7);
    float4 UVMat00                 : packoffset(c8);
    float4 UVMat01                 : packoffset(c9);
    float4 UVMat02                 : packoffset(c10);
    float4 UVMat03                 : packoffset(c11);
    float4 UVMat10                 : packoffset(c12);
    float4 UVMat11                 : packoffset(c13);
    float4 UVMat12                 : packoffset(c14);
    float4 UVMat13                 : packoffset(c15);
    float4 UVMat20                 : packoffset(c16);
    float4 UVMat21                 : packoffset(c17);
    float4 UVMat22                 : packoffset(c18);
    float4 UVMat23                 : packoffset(c19);
